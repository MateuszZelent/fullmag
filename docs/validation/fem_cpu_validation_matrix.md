...
| Demag PBC | periodic airbox supercell comparison | primitive periodic-cell demag energy matches central-cell periodic-supercell reference and primitive-cell seam metrics stay bounded | MFEM/libCEED/Hypre runtime plus generated primitive/supercell CSV artifact | passing managed artifact; CSV acceptance and active producer are covered by `tests/fem_demag_validation/periodic_airbox_validation.py` and `tests/fem_demag_validation/test_acceptance.py`; 2026-06-27 managed 3x3 run writes `.fullmag/reports/fem-demag-periodic-airbox-validation-supercell-pbc/periodic_airbox_validation.csv`; native demag finalization projects representative values, native FEM Zarr writer emits component-major `H_demag` and scalar `demag_phi`, the periodic airbox mesher excludes lateral seam fragments from Robin mass, and the explicit 3x3 reference uses lateral PBC on the outer supercell faces. Primitive `H_demag` and `phi` seam metrics both pass with `0.0`, `robin_periodic_seam_face_count=0`, primitive CG reports 38 iterations / `8.035072644447357e-18` residual, supercell CG reports 65 iterations / `3.735510701495055e-17` residual, and primitive-vs-central-cell energy relative error is `2.4167958871934916e-3` against the `2.0e-2` tolerance. |
...
| Demag PBC periodic-airbox supercell artifact | `python3 tests/fem_demag_validation/periodic_airbox_validation.py <periodic_airbox_validation.csv>` |
| Demag PBC periodic-airbox supercell sweep summary | `python3 tests/fem_demag_validation/periodic_airbox_validation.py --summarize-sweep <3x3.csv> <5x5.csv> <7x7.csv> --sweep-output <sweep.csv>` |
