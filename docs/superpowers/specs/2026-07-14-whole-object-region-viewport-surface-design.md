...
This is a frontend scene-target classification and request-identity repair. It does not change the backend field endpoint, OpenAPI contract, field binary format, solver semantics, or visualization defaults.
...
6. Field-query identity preserves complete `part:*` mesh identifiers because the prefix is part of the backend mesh-part ID. Only the visualization prefix in `object:*` scope IDs is removed.
7. Component X uses the existing `m` request with `component=x` and renders the returned nodal values.
...
3. Add a failing field-query identity test proving that `scope_kind=part` preserves `part:film_mesh` in both canonical identity and the serialized URL, while `scope_kind=object` still normalizes `object:film` to `film`.
4. Run the focused scene-model and field-query identity tests after the minimal implementation.
5. Run a managed mixed-target browser scenario and verify:
...
6. Run Control Room typecheck, lint with zero warnings, the full test suite, viewport performance/memory gates, and React Doctor.
...
- Part-scoped field requests retain the exact manifest mesh-part ID, including `part:`.
...
- OpenAPI, endpoint, or binary codec changes.
