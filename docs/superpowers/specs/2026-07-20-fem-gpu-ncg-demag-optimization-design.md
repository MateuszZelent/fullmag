...
1. repair the GPU nonlinear-CG line search so it uses the canonical direct
   energy increment and demag refinement contract;
2. reuse the exact accepted endpoint evaluation once as the next NCG current
   evaluation;
3. build the HYPRE AMG hierarchy once per compatible demag context instead of
   resetting it for every fresh right-hand side;
4. replace the strict-GPU device-wide CUDA barrier with explicitly ordered
   Fullmag/HYPRE stream interoperation;
5. expose enough bounded telemetry to prove solve count, setup reuse, fresh
...
Introduce an isolated, version-pinned HYPRE stream interop adapter in the CUDA
demag subsystem. It borrows, never owns, the exact stream used by the pinned
HYPRE build and provides this ordering:
...
The adapter is compiled only for explicitly supported HYPRE/MFEM versions and
...
- upgrading HYPRE, MFEM, CUDA, or the managed base image except through a
