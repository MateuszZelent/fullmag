...
Blokady przed zwolnieniem agenta 4 do implementacji: uzgodnij i przetestuj stationary-only observation (obecnie pełna maska + accepted_step_count > 0), opcjonalny bit DIRECT_ENERGY_REFINEMENT w NCG przy dokładnej równości required/executed oraz PG-BB ze STT/SOT wymagającym nieuruchamianego LLG direct-torque RHS. Nie rozluźniaj globalnie walidatora. Wybierz spójny kontrakt native/Rust, zachowaj fail-closed i dodaj produkcyjne regresje. Szczegóły: docs/reports/05.09.2026/agent-1-3-integration-review.md. Do tego czasu agent 4 może prowadzić analizę; agent 5 może przygotować izolowane testy DG0 bez przejmowania sparse.
