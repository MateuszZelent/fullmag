...
| `data/mesh-region-membership/{region_id}` | realized-region membership indices for current FEM mesh parts, with explicit `object_segments` fallback when no mesh-part entry exists; topology remains owned by mesh topology resources, and projection-derived membership needs a future data-plane resource/extension |
