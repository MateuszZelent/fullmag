...
| `transport.spin.transient_drift_diffusion` | `coupled_imex_ark2.v1` | FDM / CPU / double / strict | single-grid one-way M3 decay/order/restart workloads | `reference_executable` | public ProblemIR planning, coupled session backend-state persistence, explicit CLI exact resume, and `just verify-fdm-transient-spin-m3-reference`; API restore does not auto-relaunch the separate CLI process |
