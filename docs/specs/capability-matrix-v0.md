...
| explicit fixed | FDM | CUDA | single | `production_executable` | executable | unvalidated | Dokładna realizacja single-grid to `fullmag.fdm.cuda.precision.single_storage_fp64_reduction.v1` (FP32 storage/compute/FFT, FP64 reductions). Zarządzany receipt-kandydat z rzeczywistego urządzenia obejmuje parity field/RHS/stage/step dla Heuna, RK4, RK23, DP45 i ABM3, 256-krokową trajektorię fixed-Heun oraz aktualne orakle Newella thin-film i periodic truncated-images CPU↔GPU. Nie został jeszcze promowany, ponieważ receipt jest związany z roboczym diffem; periodic exchange FP32, subcell boundary correction, termika, heterogeniczność/multilayer i szersze tuple interakcji nie dziedziczą tego dowodu. |
...
| LLG timestep qualification registry | implemented, current state `unvalidated` | implemented, current state `unvalidated` | `unsupported` | **fail-closed evidence owner; FDM/FEM GPU receipts `implemented/unvalidated`** | Exact lane identity includes integrator and timestep policy. FDM CUDA additionally binds the full storage/compute/FFT/reduction policy and requested/resolved/executed realization. The current NUM-002 actual-device receipt is qualification-candidate evidence only: it is external and dirty-diff-bound. Physical Newell thin-film and periodic truncated-images gates passed on that same diff, but must be repeated on the clean source commit before `validated_workloads` can be populated. FEM GPU requires source-bound `FULLMAG_FEM_GPU_EXECUTION_RECEIPT_ABI_V1` with `device_resident`, complete executed-device masks, zero fallback, zero strict compute transfers, matching CPU/GPU source hash, and complete operator IDs. Missing, hybrid, stale, or incomplete evidence fails closed. |
...
| Execution precision `single` | ✅ exec | planned | planned | **lane-specific public-executable** (single-grid CUDA FDM); **unvalidated** | Dokładna publiczna polityka single-grid to `fullmag.fdm.cuda.precision.single_storage_fp64_reduction.v1`: FP32 storage/compute/FFT z redukcjami FP64. Bieżący dowód-kandydat jest ograniczony do tuple wymienionych w nakładce LLG i nie promuje periodic exchange, subcell boundary correction, termiki, pól heterogenicznych ani multilayer. Bounded device-resident multilayer CUDA odrzuca FP32; referencyjny CPU FDM pozostaje wyłącznie double. |
