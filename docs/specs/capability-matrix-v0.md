...
| `RotatedInterfacialDMI` | bounded executable | bounded executable | planned | **source/contract executable on FDM/FEM CPU/GPU; bimeron qualification pending** | Python and `ProblemIR` preserve signed finite `D`; time-domain planning preserves `rotated_interfacial_dmi` for single-grid FDM, multilayer FDM and FEM without GPU-to-CPU fallback. Open magnetic boundaries require Exchange. Eigen and frequency-domain studies reject explicitly. Focused operator, energy and directional-derivative checks pass on FDM CPU and managed FDM CUDA plus FEM MFEM CPU/CUDA; full Göbel bimeron reproduction and four-lane parity remain `NOT VERIFIED`. |
