...
| explicit adaptive | FDM | CUDA | double | `source_visible` | executable | unvalidated | v2 ABI behavior, canonical PI vectors, fixed/adaptive separation, typed floor failure, and batch `dt_next` consumption are contract evidence. A bounded managed-CUDA RK23/RK45 FSAL test proves the constant-field macrospin oracle, exact 19/20 RHS savings, zero reuse for Brown thermal and dynamic Oersted, zero rollback and zero fallback; full public trajectory/time-to-accuracy qualification remains pending. |
| explicit adaptive | FDM | CUDA | single | `source_visible` | executable | unvalidated | The same bounded managed-CUDA FSAL test passes RK23/RK45 FP32 with the macrospin oracle, exact deterministic RHS savings, thermal/waveform invalidation, zero rollback and zero fallback. This does not inherit FP64 qualification or promote the public adaptive lane; broader accuracy/trajectory and time-to-accuracy evidence remains pending. |
