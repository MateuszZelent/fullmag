...
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FDM / CPU / double / strict | canonical macrospin/current-scaling workloads | `reference_executable` | corrected hbar/e SI/Gilbert oracle, signed `J_n`, target mask, CPU regression gates, and independent eight-step fixed-step trajectory prefixes; current-scaling qualification remains open |
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FDM / GPU / single or double / strict | canonical macrospin/current-scaling workloads | `production_executable` | managed native CUDA descriptor, signed `J_n`, separate target mask, one-step FP64 CPU↔device parity, and eight-step fixed-step trajectory parity pass; FP32, current-scaling, and qualification artifact remain open |
...
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FEM / GPU / double / strict | canonical target-masked local torque | `reference_executable` | managed one-step FP64 CPU↔GPU parity, signed `J dot n_stack`, complete descriptor forwarding, current reversal, and target-mask contract pass; full RK trajectory, multi-grid convergence, and cross-backend qualification remain open |
...
| `SlonczewskiSTT` / `ZhangLiSTT` | Version-dependent; see tuple rows above | Legacy FDM and FEM executable labels apply only to `*.legacy_fullmag.v0` and remain compatibility behavior. Canonical FDM GPU now executes the target-masked `slonczewski.fullmag.v2` thin-layer descriptor with managed one-step and eight-step fixed-trajectory FP64 CPU↔device parity, but is not `validated`; FP32 and current-scaling qualification remain open. Canonical FEM CPU double/strict implements target-masked `slonczewski.fullmag.v2` thin-layer and `zhang_li.fullmag.v1` central-reference contracts as `reference_executable`, not `validated`. Canonical FEM GPU is now `reference_executable` for the bounded one-step FP64 descriptor/mask/reversal contract, not production or validated; full trajectory, multi-grid, and cross-backend parity remain open. Interface-flux Slonczewski rejects rather than bulk-lowering. Public multilayer FDM and the Rust FEM reference runner remain unsupported as previously documented. `slonczewski.fullmag.v1` is read-only provenance. |
