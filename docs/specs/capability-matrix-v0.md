...
| FEM Poisson-airbox K0 modal CPU production | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated` | Future scope `modal_cpu_k0_periodic_airbox_real_shared_domain.production` is a non-promotional catalog binding. The public `BiasFieldSweep` is a physical Python/`ProblemIR`/planner/runner request, while Kittel is postsolve-only. ABI v18 imports a backend-neutral descriptor without preassembled `A_qq`; `assemble_native_magnetic_a_qq` owns bounded P1 `tet4|prism6` exchange and parallel-field assembly on the native MFEM mesh, with anisotropy/DMI unavailable and dynamic demag in the mixed Schur blocks. Materialized dimensions at or below 1024 remain validation-only; production requires measured greater-than-1024 matrix-free selected-spectrum evidence. Fresh ABI-matched managed CPU sweep convergence/parity and complete DOD evidence are still required; `executable_scope` and `validated_scope` remain null. |
| FEM GPU Poisson-airbox scalable shared-domain modal | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated` | Future scope `modal_gpu_k0_periodic_airbox_scalable.production` is a non-promotional catalog binding. The PETSc/SLEPc CUDA adapter and two-pass window certificate are source-visible, and the public physical sweep preserves requested GPU intent without fallback. Materialized dimensions at or below 1024 are validation-only and cannot qualify this scope. No current-snapshot ABI-v18 managed execution proves a measured `operator_dimension > 1024` on `matrix_free_schur_selected_spectrum`, device residency, persistent EPS/ST/KSP/BV ownership, allocation/transfer bounds, matrix-free convergence or K0-3 parity. `executable_scope` and `validated_scope` remain null until fresh managed GPU, browser and DOD gates pass. |
