...
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FEM / GPU / double / strict | canonical target-masked local torque | `reference_executable` | managed one-step plus bounded eight-step fixed-step Heun FP64 CPU↔GPU parity, signed `J dot n_stack`, complete descriptor forwarding, current reversal, and target-mask contract pass; full integrator-family, multi-grid convergence, and cross-backend qualification remain open |
