...
| FEM modal k-path dispersion | `reference_executable` for the current CPU reference/MVP modal artifact lane; production CPU and production GPU remain `unsupported` | COMSOL-style nonzero-k dispersion maps to `Eigenmodes` over a Floquet/Bloch k path, not to driven `FrequencyResponse`. `/v2/sessions/current/analysis/frequency-domain/manifest.v1` exposes `capabilities.dispersion.reference_cpu`, `capabilities.dispersion.production_cpu`, and `capabilities.dispersion.production_gpu`: the reference CPU lane can emit spectrum, branches, `dispersion.csv`, and mode-field artifacts, while production selected-spectrum CPU remains gated on sparse/matrix-free Floquet eigensolve validation and modal GPU remains gated until a real native modal GPU eigensolver plus Floquet operator exist. The driven-response GPU Floquet smoke must not be reused as modal dispersion proof. |
