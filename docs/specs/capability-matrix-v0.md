...
| `Relaxation(tangent_plane_implicit)` | semantic-only | ✅ exec on `fem_cpu_native`; GPU under development / forced unsupported | **planned** | **public-executable** (native FEM CPU/MFEM) | FEM-only tangent-plane implicit relaxation executes through the native CPU/MFEM ABI. The current operator includes `mass + step * exchange`, local anisotropy curvature, Zeeman tangent curvature, DMI weak-residual action, and demag fresh-solve linear response. Automatic FEM GPU selection falls back to `fem_cpu_native` with `fem_gpu_relaxation_algorithm_cpu_only`; forced FEM GPU fails while the full GPU/libCEED device-resident tangent-plane solve remains under development. |
