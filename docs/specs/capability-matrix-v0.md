...
| `StudyIR::FrequencyResponse` | `reference_executable` for dense FEM validation response; production CPU/GPU still `unsupported` | Public Python/API/IR semantics exist, and the current runner can emit artifact-backed dense block-real magnetic frequency-sweep validation results as `fem_frequency_response_dense_validation`. This is not the production native MFEM/hypre/libCEED frequency-domain solver. |
...
| `FrequencyResponse` | semantic-only | reference-executable dense validation path; production CPU/GPU unsupported | planned | reference-executable validation slice only | First-class `StudyIR::FrequencyResponse` and Python `fm.FrequencyResponse` serialize the driven frequency-domain contract. The current runner can execute the dense block-real magnetic frequency-sweep validation path and emit artifact-backed response sweeps, progress, diagnostics, and field payload resources. This does not promote the production native MFEM/hypre/libCEED frequency-domain solver: production CPU/GPU response, nonzero-k Floquet driven response, mode-projected response, frequency-domain elastodynamics, coupled eigenmodes, and two-way magnetoelastic response remain explicitly capability-gated. `/v2/sessions/current/analysis/frequency-domain/manifest.v1` exposes nested `frequency_domain_capabilities.v1` for precise UI gating. |
