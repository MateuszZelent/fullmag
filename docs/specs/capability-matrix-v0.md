...
| FEM periodic / Floquet spin-wave support | `partial_production_executable` for k=0 static-periodic driven response on native FEM CPU; `semantic_only`/`unsupported` for broader Floquet lanes | Static-periodic response requires `mesh.periodic_node_pairs` plus boundary-pair metadata with translation/tolerance diagnostics, and stays separate from nonzero-k Floquet/Bloch support. Floquet operator enforcement, k-path production response, and phase-aware dynamic demag remain gated. |
...
| FEM driven frequency response | `partial_production_executable` for the native FEM CPU gamma/free-boundary and k=0 static-periodic magnetic slice; `partial_production_executable` for the native FEM GPU gamma/free-boundary no-demag magnetic slice; `reference_executable` for dense FEM validation response | This row is the driven `driven_response` product only. `Frequency Response` solves `(i omega B - A) q = b` at requested frequencies. Its status does not promote modal eigensolve. The GPU slice includes exchange, Zeeman, and uniform uniaxial anisotropy through the native CUDA operator; DMI, demag, static-periodic projection, nonzero-k Floquet/Bloch response, and magnetoelastic response remain explicitly gated. |
| `StudyIR::FrequencyResponse` | `partial_production_executable` for the native FEM CPU gamma/free-boundary and k=0 static-periodic magnetic slice; `partial_production_executable` for the native FEM GPU gamma/free-boundary no-demag magnetic slice; `reference_executable` for dense FEM validation response | Public Python/API/IR semantics exist. The current runner can emit artifact-backed dense block-real magnetic frequency-sweep validation results as `fem_frequency_response_dense_validation`, exposes the limited native MFEM production CPU response engine as `fem_frequency_response_production_cpu`, and routes requested GPU frequency response to `fem_frequency_response_production_gpu` only for the no-demag gamma/free-boundary slice. Static-periodic response requires `mesh.periodic_node_pairs`; demag, nonzero-k Floquet/Bloch response, missing periodic pair metadata, DMI on GPU, magnetoelastic response, and broader production GPU response remain explicitly gated. |
...
| `FrequencyResponse` | semantic-only | reference-executable dense validation path; gated production CPU slice; gated production GPU no-demag slice | planned | partial production CPU/GPU executable, broader response still gated | First-class `StudyIR::FrequencyResponse` and Python `fm.FrequencyResponse` serialize the driven frequency-domain contract. The current runner can execute the dense block-real magnetic frequency-sweep validation path and emit artifact-backed response sweeps, progress, diagnostics, and field payload resources. The native MFEM frequency-domain production CPU lane is executable only for the gamma-point/free-boundary and k=0 static-periodic magnetic response slice with exchange, Zeeman, uniform uniaxial anisotropy, interfacial/bulk DMI on P1 tetrahedra, uniform or nodal Gilbert damping, matrix-free GMRES, progress telemetry, and partial-artifact cancellation. The native FEM production GPU lane is executable only for the gamma-point/free-boundary no-demag magnetic response slice with exchange, Zeeman, uniform uniaxial anisotropy, uniform or nodal Gilbert damping, native CUDA operator application, matrix-free GMRES, and `validation_fallback_used=false`. Static-periodic response requires `mesh.periodic_node_pairs` and boundary-pair translation/tolerance diagnostics, and is enforced by tangent-space periodic projection on CPU only. The DMI part of the CPU slice is wired through the public runner payload and native weak-residual contract but still requires managed FEM runtime proof before it is promoted to validated production. Demag, nonzero-k Floquet/Bloch response, DMI on GPU, mode-projected response, frequency-domain elastodynamics, coupled eigenmodes, and two-way magnetoelastic response remain explicitly capability-gated. `/v2/sessions/current/analysis/frequency-domain/manifest.v1` exposes nested `frequency_domain_capabilities.v1` for precise UI gating. |
