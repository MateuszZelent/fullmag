...
| FEM Poisson-airbox K0 modal CPU production | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated` | The shared-domain MFEM assembler, real-split `real_frequency_rotated` target (`tau=omega_target`), certified Schur reduction, original-unscaled residuals, cancellation and mode-field artifact path are implemented. Fresh managed CPU convergence/parity and complete DOD evidence are still required; source-visible code does not promote the cell. |
| FEM GPU Poisson-airbox bounded shared-domain modal | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated` | The production PETSc/SLEPc CUDA adapter `k0_poisson_airbox_gpu_petsc_slepc` owns the shared-domain CSR blocks, device vectors/basis, shifted Schur action and device residual certification with no CPU fallback. The bounded lane remains unvalidated until fresh managed GPU evidence, parity, convergence, performance/residency, browser and DOD gates pass. |
