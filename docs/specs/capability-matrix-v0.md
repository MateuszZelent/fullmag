...
| `transport.charge.ohmic` | `current_transport.fullmag.v1` + `fv_charge_harmonic_v1` or FEM operator | FDM/FEM / CPU/GPU / double / strict | M1 uniform/layered bar with declared BC/gauge | `semantic_only` | conservative solve, balance, convergence and managed lane evidence; FDM GPU zero-mean-per-free-component now has managed actual-device proof for balanced pure-Neumann, fail-closed unbalanced Neumann, and typed mixed anchored/free components, but has no public ProblemIR/planner/runner path |
