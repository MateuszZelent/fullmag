...
| `Demag` | ✅ exec | ✅ exec | planned | **public-executable** (FDM/FEM) | FDM uses Newell tensor FFT; executable FEM is Poisson-only (`poisson_robin` / `poisson_dirichlet`) on MFEM-native CPU/GPU paths and requires a shared-domain mesh with air. For explicit native `poisson_robin`, the managed runtime resolves directly to `hypre_pcg_boomeramg` and logs the resolved solver; when FEM CPU thread selection is left on `auto`, the runtime may resolve to a smaller effective OpenMP count for Poisson meshes and logs requested vs effective threads. |
