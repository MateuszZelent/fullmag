...
PR-15 nie promuje FDM GPU/FP64 M1. Ograniczony natywny charge-only slice ma
rzeczywisty CUDA FV/CG/AMG, immutable snapshot oraz exact-identity checkpoint
contract proof, dlatego agregat ma `implementation_state=partial`. Publiczna
ścieżka `ProblemIR`--planner--runner nadal go nie uruchamia, a steady spin,
direct SHE, mixing i torque nie mają realizacji CUDA. Dla
...
`coupling.transport_llg.one_way` tuple FDM/GPU/double/strict pozostaje
`semantic_only`, z `validation_state=unvalidated` i
`validated_workloads=[]`. Forced GPU pozostaje fail-closed bez fallbacku CPU
lub obniżenia precyzji. Tylko wiersz charge ma implementację częściową; pozostałe
cztery capability nie dziedziczą z niego stanu wykonywalnego.
...
| `transport.charge.ohmic` | `current_transport.fullmag.v1` + `fv_charge_harmonic_v1` or FEM operator | FDM/FEM / CPU/GPU / double / strict | M1 uniform/layered bar with declared BC/gauge | `semantic_only` | conservative solve, balance, convergence and managed lane evidence |
