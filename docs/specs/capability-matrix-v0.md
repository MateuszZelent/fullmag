...
| `RotatedInterfacialDMI` | FDM CPU implemented; strict FP64 CUDA validated for the Göbel fixture | FEM CPU/GPU implemented, runtime unverified; GPU limited to P1 tet4 | planned | **lane-specific implementation and qualification** | Python, `ProblemIR`, study-scoped UI authoring and script rewrite preserve signed finite `D`. Single-grid FDM CPU/CUDA and bounded FEM paths carry the operator without GPU-to-CPU fallback; conventional and rotated DMI cannot be mixed until independent native field/energy channels exist. Open magnetic boundaries require Exchange. `H_rotated_dmi` and `eden_rotated_dmi` are currently materialized only by FDM; FEM requests reject. The exact strict FP64 CUDA Göbel thin-film fixture is validated; FDM CPU and FEM bimeron runtimes remain `NOT VERIFIED`. FEM GPU rotated DMI rejects prism, pyramid and mixed topology. |
