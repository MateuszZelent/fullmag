...
identity, precision, oracle, mesh refinement, repeatability oraz dowody D4, D5 i
D6 z niepustymi, sha256-zgodnymi manifestami artefaktów. Receipt musi zawierać
zgodny hash artefaktu, `source_clean=true` oraz czysty commit/tree wraz z
czystymi zagnieżdżonymi gitlinkami; assume-unchanged i skip-worktree są
odrzucane. Pole `managed_command` może wskazywać tylko istniejącą receptę z
allowlisty z dokładnym sha256 jej kanonicznego body. Aktywne polecenia tej
recepty muszą uruchamiać zarządzany runtime i kontener, a receipt TPI wymaga
recepty jawnie dopuszczonej do `tangent_plane_implicit`. Deklarowane w wierszu
`qualification_requirements` nie zastępuje tych kanonicznych reguł.
...
| `fem.cpu.exchange_demag.mixed_p1` | `implemented` | implemented | bounded CPU/strict/double P1 exchange/Poisson/relaxation contracts accept uniform or nodal-P1 uniaxial and cubic anisotropy plus interfacial/bulk DMI on the magnetic prism oracle; evidence is source/contract only | MFEM/hypre CPU double, exchange + uniform Zeeman + uniform/nodal uniaxial/cubic anisotropy + CPU DMI + Poisson Robin/Dirichlet |
| `fem.gpu.exchange_demag.mixed_p1` | `implemented` | implemented | bounded GPU/strict/double P1 contracts accept uniform or nodal-P1 uniaxial and cubic anisotropy; DMI remains fail-closed as `gpu_dmi_kernel_not_mixed_p1`; evidence is source/contract only and no runtime qualification is claimed | MFEM/libCEED/CUDA GPU double with uniform/nodal uniaxial/cubic anisotropy on the same certified mesh; GPU DMI rejected |
...
uniform `Ms`/`Aex`, exchange, optional uniform Zeeman, uniform or nodal-P1
uniaxial/cubic anisotropy with normalized material axes, CPU-only interfacial
or bulk DMI, and Poisson Robin/Dirichlet,
...
Nodal `Ku1`/`Ku2` and `Kc1`/`Kc2`/`Kc3` fields and cubic anisotropy are part of
the bounded source-contract slice. Nodal `Ms`/`Aex`/`alpha` fields remain
rejected before backend startup. GPU DMI is rejected with the stable
`gpu_dmi_kernel_not_mixed_p1` predicate; CPU DMI is admitted by the CPU row.
Until separately qualified, all modes reject before backend startup when a
mixed-P1 request includes FEM/BEM, PBC/Floquet, GPU DMI, STT, thermal,
...
### FEM Fredkin-Köhler capability reporting reconciliation

The existing `Demag` row includes the initial body-only Fredkin-Köhler FEM/BEM
CPU source path. `RequestedFemDemagIR` and the FEM planner also recognize this
model, while native provenance has separate CPU and GPU realizations. The
runner time-domain FEM capability projection now includes `fredkin_koehler`
in `supported_demag_realizations` for CPU and GPU. Eigen and frequency-response
profiles use the common base profile and retain only Poisson realizations.
This policy-only reconciliation follows the existing
[FEM/BEM contract](../physics/0870-fem-bem-demag-open-boundary.md); it changes
neither equations nor planner legality. Its regression source has been updated,
but execution of that check and API runtime verification remain **NOT VERIFIED**.

The existing list is a static profile, not proof that a runtime is available for
every mesh or workflow. No new public status row or `supported_terms` vocabulary
is introduced; the existing API still reports generic `interaction.demag`.
GPU prerequisites include body-only P1, double precision, unconstrained true
DOFs matching mesh nodes, nonperiodic geometry and the required device stack.
Requested intent and dispatch rejection/fallback policy remain unchanged.
CPU and GPU evidence remain separate: CPU `hierarchical_h2` and GPU
`device_hypre_fem_bem` are distinct runtime realizations when selected by their
respective branches. Source presence or provenance does not promote either
realization to `production_executable` or `validated`; each needs its own
managed receipt, parity and physics gates.
...
PR-15 does not promote FDM GPU/FP64 M1. The bounded native charge-only slice
has an actual CUDA FV/CG/AMG implementation, an immutable snapshot, and an
exact-identity checkpoint contract proof, so the aggregate has
`implementation_state=partial`. The public `ProblemIR`--planner--runner path
now executes only the separately specified, bounded `CurrentTransport`
charge-only realization for FDM/CUDA/FP64/strict; steady spin, direct SHE,
mixing, and torque still have no CUDA realization. For
`transport.charge.ohmic`, `transport.spin.steady_drift_diffusion`,
`transport.spin.direct_she`, `transport.spin.mixing_conductance` i
`coupling.transport_llg.one_way`, the FDM/GPU/double/strict tuple remains
`semantic_only`, with `validation_state=unvalidated` and
`validated_workloads=[]`. Forced GPU remains fail-closed without CPU fallback
or reduced precision. Only the charge row has a partial implementation; the
other four capabilities do not inherit an executable state from it.
...
| `spin_torque.prescribed_sot` | `prescribed_sot.fullmag.v1` | FDM / CPU / double / strict | canonical signed-current macrospin and target-mask reference workload | `reference_executable` | independent SI/Gilbert algebra and CPU reference path pass; stage-time envelopes, nonlinear sweeps, and cross-backend qualification remain open |
| `spin_torque.prescribed_sot` | `prescribed_sot.fullmag.v1` | FDM / GPU / double / strict | canonical signed-current macrospin and target-mask workload | `production_executable` | managed CUDA FP64 eight-step CPU-reference trajectory and isolated `0.5×/1×/2×` target-mask scaling pass; FP32, nonlinear/stage-time, and qualification evidence remain open |
| `spin_torque.prescribed_sot` | `prescribed_sot.fullmag.v1` | FEM / CPU / double / strict | canonical signed-current macrospin, non-tabulated stage-time envelope, and target-node-mask one-step reference workload | `reference_executable` | managed MFEM CPU result matches an independent SI Heun oracle and Rust FEM reference for magnetization, `H_eff`, and max RHS at constant and sinusoidal stage times; event alignment, FEM multi-grid, long trajectory, and FEM↔FDM continuum qualification remain open |
| `spin_torque.prescribed_sot` | `prescribed_sot.fullmag.v1` | FEM / GPU / double / strict | canonical signed-current macrospin, non-tabulated stage-time envelope, target-node-mask one-step reference workload | `reference_executable` | managed CUDA interaction and native MFEM GPU steps match an independent SI Heun oracle and FEM CPU reference for `m`, `H_eff`, and max RHS at constant and sinusoidal stage times; event alignment/rollback, tabulated artifacts, FP32, long trajectories, multi-grid, and FEM↔FDM qualification remain open |
...
| `spin_torque.zhang_li` | `zhang_li.mumax3.v1` + `zl_mumax3_central_v1` | FDM / CPU / double / strict | fixed-step MuMax3 Standard Problem 5 with explicit `converged_demag` reference and central-stencil oracle | `reference_executable`; validated workload | exact CPU/GPU schedule and full-field parity pass for `dt=1e-13 s` and `dt/2`; full field passes `1e-4` against MuMax3 `DemagAccuracy=24`; literal default-accuracy golden remains a separately visible failure |
| `spin_torque.zhang_li` | `zhang_li.mumax3.v1` + `zl_mumax3_central_v1` | FDM / GPU / double / strict | same fixed-step FP64 SP5 workload on native CUDA | `production_executable`; validated workload | device execution, executed-module provenance, full trajectory CPU↔CUDA parity and `converged_demag` reference pass; adaptive, FP32, other grids and `zhang_li.fullmag.v1` remain unvalidated |
...
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FDM / CPU / double / strict | canonical macrospin/current-scaling workloads | `reference_executable` | corrected hbar/e SI/Gilbert oracle, signed `J_n`, target mask, CPU regression gates, independent eight-step fixed-step trajectory prefixes, and bounded isolated 0.5×/1×/2× current-increment scaling; nonlinear current-sweep qualification remains open |
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FDM / GPU / single or double / strict | canonical macrospin/current-scaling workloads | `production_executable` | managed native CUDA descriptor, signed `J_n`, separate target mask, one-step FP64 CPU↔device parity, eight-step fixed-step trajectory parity, and bounded isolated 0.5×/1×/2× current-increment scaling pass; FP32, nonlinear current sweep, and qualification artifact remain open |
...
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FEM / GPU / double / strict | canonical target-masked local torque | `reference_executable` | managed one-step plus bounded eight-step fixed-step Heun FP64 CPU↔GPU parity, signed `J dot n_stack`, complete descriptor forwarding, current reversal, target-mask, and isolated tangential 0.5×/1×/2× current-scaling contracts pass; full integrator-family, multi-grid convergence, and cross-backend qualification remain open |
...
| `transport.charge.ohmic` | `current_transport.fullmag.v1` + `fv_charge_harmonic_v1` or FEM operator | FDM/FEM / CPU/GPU / double / strict | M1 uniform/layered bar with declared BC/gauge | `semantic_only` | conservative solve, balance, convergence and managed lane evidence; bounded public FDM/CUDA charge-only E2E is tracked separately and does not widen this general tuple; zero-mean gauge, current-density electrodes, scaling, mesh convergence and production gates remain open |
...
| `transport.charge.magnetoresistive` | `transport_constitutive.reciprocal.fullmag.v1` + `fem_charge_spin_conforming_h1_p1.reciprocal_m2.v1` | FEM / CPU / double / strict | bounded full-domain conforming H1/P1 M2 with uniform anisotropic charge tensor and Dirichlet voltage reference | `reference_executable` | managed planner, ABI, materialization, and native reciprocal-diagnostics tests pass; heterogeneous tensors, interfaces, nonlinear iteration, mesh convergence, GPU and production gates remain open |
...
| `transport.spin.steady_drift_diffusion` | `transport_constitutive.reciprocal.fullmag.v1` + `fem_charge_spin_conforming_h1_p1.reciprocal_m2.v1` | FEM / CPU / double / strict | bounded reciprocal M2 full-domain conforming H1/P1 block with no internal spin interfaces | `reference_executable` | uniform materials, Dirichlet charge reference, no reciprocal nonlinear policy, no LLG/Oersted stage coupling; native M2 diagnostics and fail-closed planner/runner tests pass; convergence and production remain open |
...
| `transport.spin.direct_she` | `transport_constitutive.one_way.fullmag.v1` + `fv_spin_upwind_v1` | FDM / CPU / double / strict | M1 `she_1d_film_v1` structured-grid reference workflow with explicit insulating spin BC | `reference_executable` | runner-level signed `sinh/cosh` profile, charge-current, residual, spin-balance and three-resolution h-refinement gates pass; BORIS executable parity and FDM/FEM continuum comparison remain open |
| `transport.spin.direct_she` | `transport_constitutive.one_way.fullmag.v1` + `fem_charge_spin_conforming_h1_p1.transparent.v1` | FEM / CPU / double / strict | same transparent conforming H1/P1 M1 reference slice | `reference_executable` | emits revisioned canonical transport quantities; signed vector-profile, residual and three-mesh h-refinement gates pass in the managed native contract; cross-backend continuum, BORIS parity and production qualification remain open |
| `transport.spin.direct_she` | `transport_constitutive.reciprocal.fullmag.v1` + `fem_charge_spin_conforming_h1_p1.reciprocal_m2.v1` | FEM / CPU / double / strict | bounded reciprocal M2 direct-SHE source coupled to the iSHE charge block | `reference_executable` | executable native reciprocal block and diagnostic identity pass; Onsager/dissipation sweep, reciprocal mesh convergence, FDM/FEM common-limit, BORIS parity, GPU and production gates remain open |
| `transport.spin.inverse_she` | `transport_constitutive.reciprocal.fullmag.v1` | FDM / CPU / double / strict | bounded four-cell homogeneous M2 manufactured state with specified charge and spin potentials | `reference_executable` | engine constitutive oracle plus runner/ProblemIR materialization, simultaneous iSHE/direct-SHE channels, residual and balance gates; Onsager sweep, nonlinear/heterogeneous/interface, BORIS inverse parity, FEM/GPU and production gates remain open |
| `transport.spin.inverse_she` | `transport_constitutive.reciprocal.fullmag.v1` | FDM/FEM / CPU/GPU / double / strict | broader M2 reciprocal benchmark | `semantic_only` | Onsager, dissipation, nonlinear-coupling and cross-backend gates |
| `transport.spin.inverse_she` | `transport_constitutive.reciprocal.fullmag.v1` + `fem_charge_spin_conforming_h1_p1.reciprocal_m2.v1` | FEM / CPU / double / strict | bounded full-domain M2 iSHE feedback with explicit anisotropic tensor and Dirichlet voltage reference | `reference_executable` | ABI nested-prefix, planner materialization, and managed native reciprocal-diagnostics tests pass; this is not validated and excludes heterogeneous/interface/nonlinear/GPU/production workloads |
...
| `field.oersted.fem_direct_quadrature` | `fem_conservative_current_rt0_view.v1` + optional `fem_closed_current_extension.v1` + `fem_oersted_direct_tetra_quadrature.v1` + `oersted_direct_biot_savart.v1` | FEM / CPU / double / strict | small M1 volumetrically closed tetrahedral circuit or meshed lead extension; analytic return is a separate OE-F1-only additive realization | `semantic_only` | OE-T0/OE-F1 native publication and managed CPU fixture pass; bounded `steady_source_invariant.v1` exact-key cache and runner checkpoint/rollback contract exist for one-way `closed_geometry`, but native RK/FSAL callback, external-lead, OE-F2 normal runtime, GPU, cross-backend and production gates remain open |
| `field.oersted.fem_vector_potential` | `fem_conservative_current_rt0_view.v1` + optional `fem_closed_current_extension.v1` + `fem_oersted_hcurl_h1_gauge.v1` + `oersted_fem_vector_potential.v1` | FEM / CPU or GPU / double / strict | M1 volumetrically closed conductor+lead circuit contained in airbox; analytic return unsupported | `semantic_only` | Append-only OE-F2 ABI/native publication and managed CPU fixture pass on the exact immutable RT0 view, including A/gauge, compatible B/H and residual certificate; normal-runtime IR/planner method selection, stage LLG publication, exact-sequence/topology/direct-oracle/airbox/device gates and production qualification remain open; zero-mean is separate |
...
| `SlonczewskiSTT` / `ZhangLiSTT` | Version-dependent; see tuple rows above | Legacy FDM and FEM executable labels apply only to `*.legacy_fullmag.v0` and remain compatibility behavior. Canonical FDM GPU now executes the target-masked `slonczewski.fullmag.v2` thin-layer descriptor with managed one-step and eight-step fixed-trajectory FP64 CPU↔device parity plus bounded isolated 0.5×/1×/2× current-increment scaling, but is not `validated`; FP32 and nonlinear current-sweep qualification remain open. Canonical FEM CPU double/strict implements target-masked `slonczewski.fullmag.v2` thin-layer and `zhang_li.fullmag.v1` central-reference contracts as `reference_executable`, not `validated`. Canonical FEM GPU is now `reference_executable` for bounded one-step and eight-step fixed-step Heun FP64 descriptor/mask/reversal/trajectory parity plus isolated tangential 0.5×/1×/2× current scaling, not production or validated; full integrator-family, multi-grid, and cross-backend parity remain open. Interface-flux Slonczewski rejects rather than bulk-lowering. Public multilayer FDM and the Rust FEM reference runner remain unsupported as previously documented. `slonczewski.fullmag.v1` is read-only provenance. |
...
| FEM eigen frequency-window target | `semantic_only` for public authoring; `implementation_state=source_visible`, `validation_state=unvalidated` for production selected-spectrum qualification | Intended public contract: `frequency_min_hz`, `frequency_max_hz`, and `count` as max returned modes in the interval. The native CPU Schur and GPU PETSc/SLEPc adapters implement the managed real-scalar `real_frequency_rotated` pencil with `tau=omega_target`, original-block residual certification, and explicit requested/resolved provenance. Current managed production evidence is still bounded/stale relative to the dirty recovery snapshot, so no COMSOL-class large-object or production capability claim is legal. |
| FEM modal interior-window eigensolve | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated`, `validated_workloads=[]` | This row is the modal `modal_eigen` product only. `Eigenmodes` uses `L q = lambda B_alpha q`, `lambda=i omega`, tangent variables, and explicit interval targets such as `frequency_min_hz` / `frequency_max_hz`. For the strict P1/double `periodic_airbox_k0` request, planner provenance preserves requested device/precision/engine and resolves exactly one CPU `k0_poisson_airbox_cpu_schur_slepc` or GPU `gpu_modal_device_krylov` engine. The GPU token names the device-resident modal lane; `k0_poisson_airbox_gpu_petsc_slepc` remains native adapter provenance only. Explicit CPU/GPU requests never use a hidden override or fallback. Materialized descriptor dimensions at or below 1024 are `validation_only`; production uses `matrix_free_schur_selected_spectrum` and requires a measured `operator_dimension > 1024`. The future non-promotional catalog bindings are `modal_cpu_k0_periodic_airbox_real_shared_domain.production` and `modal_gpu_k0_periodic_airbox_scalable.production`. The source implements the rotated real-frequency target, shared-domain assembler, Schur reduction, and original-block residuals, but no executable or validated scope is promoted until fresh managed CPU/GPU convergence, performance, DOD, and browser evidence pass. Its status does not promote driven response. Synthetic Poisson-airbox algebra oracles remain non-production: `assembly_kind=synthetic_algebraic_oracle` must keep `production_periodic_airbox_claim=false` until a real shared-domain FEM assembly is validated. |
| FEM relax-to-modal certified handoff | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated`, `validated_workloads=[]` | `CertifiedFemEquilibriumFields.v1` and accepted completion transport exist in source, but the expanded handoff is still labeled `AcceptedFemRelaxStageHandoff.v2` and target-only signatures do not certify source material, static physics, or static BC identity. Production requires frozen-v2 migration to `AcceptedFemRelaxStageHandoff.v3`, separate source-equilibrium and modal signatures, `LinearizationState.v7`, `equilibrium_artifact.v8`, negative mutation tests, and fresh managed relax-to-eigen evidence. This row does not promote the CPU or GPU modal solver rows; `executable_scope` and `validated_scope` remain null. |
...
| FEM Poisson-airbox K0 modal CPU production | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated` | Future scope `modal_cpu_k0_periodic_airbox_real_shared_domain.production` is a non-promotional catalog binding. The public `BiasFieldSweep` is a physical Python/`ProblemIR`/planner/runner request, while Kittel is postsolve-only. Request/shared-payload ABI v19 with frozen by-value result ABI v18 imports a backend-neutral descriptor without preassembled `A_qq`; `assemble_native_magnetic_a_qq` owns bounded P1 `tet4|prism6` exchange and certificate-consistent field assembly on the native MFEM mesh, with anisotropy/DMI unavailable and dynamic demag in the mixed Schur blocks. Materialized dimensions at or below 1024 remain validation-only; production requires measured greater-than-1024 matrix-free selected-spectrum evidence. Fresh request-v19/result-v18 managed CPU sweep convergence/parity, the certified handoff row above, and complete DOD evidence are still required; `executable_scope` and `validated_scope` remain null. |
| FEM GPU Poisson-airbox scalable shared-domain modal | `source_visible`; `implementation_state=source_visible`, `validation_state=unvalidated` | Future scope `modal_gpu_k0_periodic_airbox_scalable.production` is a non-promotional catalog binding. The PETSc/SLEPc CUDA adapter and two-pass window certificate are source-visible, and the public physical sweep preserves requested GPU intent without fallback. Materialized dimensions at or below 1024 are validation-only and cannot qualify this scope. No current-snapshot request-v19/result-v18 managed execution proves a measured `operator_dimension > 1024` on `matrix_free_schur_selected_spectrum`, device residency, persistent EPS/ST/KSP/BV ownership, allocation/transfer bounds, matrix-free convergence or K0-3 parity. The caller-sized v20 attestation currently reports measurement unavailable. `executable_scope` and `validated_scope` remain null until the certified handoff, fresh managed GPU, browser and DOD gates pass. |
| FEM GPU Poisson-airbox dense G5a oracle | `source_visible`; `implementation_state=executable`, `validation_state=algebra_validated`, `validated_scope=bounded one-thread dense GPU fixture` | The truthful lane is `gpu_dense_modal_validation`. It is a separate validation oracle, not the production shared-domain adapter; it is one-shot and non-scalable and cannot emit a broad `gpu_device_resident_modal_eigensolver=true` claim. The `gpu_dense_k0_macrospin_modal_eigen` cuSolverDN exception remains limited to the no-demag macrospin scope. |
...
| FEM CPU | Target plus optional intersecting `mesh_part`/`airbox` element scope | Complete or explicitly mapped nodal P1 field on Tet4/Prism6; Pyramid5/Hex8, discontinuous, cell-centred, and higher-order carriers reject | plane, slab, depth, `object_boundary` surface | CPU | focused Tet4/Prism6 numerical and target tests; fresh managed FEM/browser/runtime/production evidence remains unqualified |
| FEM GPU | Same target/scope restrictions as FEM CPU after compatible transport | Transported nodal P1 Tet4/Prism6 field; planar metadata does not identify source device/precision | same CPU P1 postprocessor | CPU | source-compatible by code path only; no fresh GPU source/device evidence and no native GPU sampler |
...
| `planar_slab_average` | cell-volume weighted, CPU sampler | conservative native Tet4/Prism6 convex-cell measure, CPU sampler | unsupported | node-count averaging and hidden Prism6-to-Tet4 reduction forbidden |
| `planar_depth_projection` | cell-volume weighted, CPU sampler | conservative native Tet4/Prism6 convex-cell measure, CPU sampler | unsupported | unsupported reduction rejects |
| `planar_surface_projection` `object_boundary` | unsupported: boundary topology absent | native triangular/quadrilateral boundary-face measure, CPU sampler | unsupported | region/named FEM surfaces reject; folded/non-injective surfaces are diagnostic |
...
| `planar_target.domain` | full rectangular carrier | all Tet4/Prism6 elements with required mapped nodal values | unsupported | default `magnetic_only` quantities resolve `magnetic_domain`, not full-domain air |
...
| `planar_fdm_multilayer_native_layer_carrier` | implemented for static `mat_ms/mat_aex/mat_alpha` with exact `scope_kind=layer`, per-layer native grid, object/region membership and plan-first/persisted cross-check; browser unqualified | not applicable | unsupported | no flattening/common-grid/scalar fallback; stale or corrupt declared carrier returns `409`, empty target `422`; dynamic `m/H` still requires a published field-store carrier |
...
| `planar_target_boundary_overlay` | unavailable: FDM topology is not published, return `204` | `FMCS v4` exact selected-target boundary from native Tet4/Prism6 faces; browser unqualified | unsupported | `FMCS v3` is degraded only; no float-dedup inference, material-interface substitution, or monitor-rectangle fallback |
...
- `llg_overdamped`, `projected_gradient_bb`, and `nonlinear_cg` have current
  executable implementations, but their FEM demag lanes remain
  development-executable until an exact source-bound managed receipt is present.
  There is no hidden PG-BB-to-NCG fallback.
...
| explicit fixed | FDM | CUDA | single | `production_executable` | executable | physics_validated (exact workloads) | Dokładna realizacja single-grid to `fullmag.fdm.cuda.precision.single_storage_fp64_reduction.v1` (FP32 storage/compute/FFT, FP64 reductions). Czysty receipt `a8ced10bc…` kwalifikuje field/RHS/stage/step parity dla Heuna, RK4, RK23, DP45 i ABM3, 256-krokową trajektorię fixed-Heun oraz aktualne orakle Newella thin-film i periodic truncated-images CPU↔GPU. Promocja dotyczy wyłącznie workloadów zapisanych w rejestrze; periodic exchange FP32, subcell boundary correction, termika, heterogeniczność/multilayer i szersze tuple interakcji nie dziedziczą tego dowodu. |
...
| explicit adaptive | FDM | CUDA | double | `production_executable` | executable | unvalidated | Headless RK23/RK45 wykonuje transakcyjne conditional-graph batch między granicami outputu. Managed CUDA potwierdza publiczny runner E2E, analityczny oracle Gilberta, rollback, parity czasu i pola, zero fallbacku oraz co najmniej 10× mniej synchronizacji bez hot-loop control-scalar D2H. Brak czystego hash-bound artefaktu dla dokładnego tuple interakcji i pełnego time-to-accuracy, więc rejestr pozostaje fail-closed `unvalidated`. |
| explicit adaptive | FDM | CUDA | single | `production_executable` | executable | unvalidated | RK23/RK45 FP32 używa `fullmag.fdm.cuda.precision.single_storage_fp64_reduction.v1` i przechodzi zarządzany natywny oracle conditional graph, rollback, telemetrykę synchronizacji oraz budżet parity zależny od precyzji. Publiczna ścieżka headless współdzieli architekturę wykonania FP64, ale nie ma czystego publicznego artefaktu E2E związanego ze źródłem dla dokładnego adaptive interaction tuple; lane nie dziedziczy kwalifikacji FP64, fixed-step ani multilayer i pozostaje `unvalidated`. |
...
| Per-cell material fields (`ms_field`, `a_field`, `alpha_field`, `ku*_field`, `kc*_field`) | FDM CPU reference implemented for `Ms/Aex/alpha`; heterogeneous CUDA unsupported | bounded FEM exec | planned | **implemented but unqualified** (FDM CPU `Ms/Aex/alpha`); **partial-production-executable** (FEM CPU DG0 `M_s`) | Public multilayer FDM planning materializes native-layout `ms_field/a_field/alpha_field`, Task 10B wires them into CPU exchange, RHS, demag source, energy and moment, and Task 10C publishes per-layer artifacts plus plan-first/persisted native API/planar carriers. This is source/unit/API-test evidence only: no managed runtime, scientific or browser/WebGL qualification is claimed. Heterogeneous FDM CUDA remains fail-closed/no-go, and per-cell anisotropy/DMI fields remain unsupported. FEM carries material field payloads through native material-field lanes where supported. The qualified sharp element-DG0 `M_s` slice is CPU/double ordinary time evolution with mandatory consistent-mass exchange; Poisson demag and Zeeman are optional additions, not standalone DG0 owners. GPU DG0, direct relaxation, DG0 anisotropy/DMI/thermal/STT/Oersted/magnetoelastic combinations, and discontinuous DG0 `A` remain unsupported and reject without nodal fallback. |
| Object-owned authored regions (`object_regions`, region material overrides, region mesh policy, region couplings) | public multilayer planner/IR plus per-layer artifacts/API/planar membership implemented; partial single-grid CUDA exec | partial FEM executable | planned | **implemented but unqualified / partial-executable** | Authored regions are visible in Python, ProblemIR, OpenAPI, and the Control Room. Public multilayer FDM planning materializes per-layer `native_region_mask` and stable legend plus `Ms/Aex/alpha` coefficient payloads; Task 10B consumes coefficient arrays on the CPU reference lane, while Task 10C publishes native per-layer mask/legend artifacts, JSON/FMRM API resources, central client/codec and exact `scope_kind=layer` planar target filtering. Plan-first active-session data and persisted artifacts are cross-checked fail-closed; no common-grid, scalar or all-active fallback is legal. This is source/unit/API/client-test evidence, not managed runtime, scientific or browser/WebGL qualification. Heterogeneous multilayer CUDA remains fail-closed/no-go. Current single-grid CUDA FDM can materialize owner-scoped region masks for region texture overrides and explicit/disabled region-region exchange pairs; CPU reference FDM rejects executable pair overrides instead of silently ignoring them. FDM/FEM planning samples smooth `Ms/Aex` region material transitions into coefficient payloads using signed-distance weights; omitted `material_transition` defaults to `mesh_relative(cells=3, scope=boundary)` for `Ms/Aex`, while explicit `kind=sharp` remains the conformal/projection-gated discontinuous path. FEM also samples `Alpha` region overrides/fields into nodal coefficient payloads, records realization method plus min/max/mean statistics, and emits projection warnings for explicit sharp projection into runtime solver status. Python/Gmsh automatically creates conformal authored-region markers for fully contained box and cylinder regions on the OCC shared-domain path, including arbitrary cylinder axes; unsupported shapes, regions outside the owner, and overlapping conformal regions reject instead of degrading silently. Metadata-only markers do not qualify, and marker IDs cannot collide with object/domain `region_markers`. One bounded strict sharp-jump path is production-executable: conformal element-DG0 `M_s` on FEM CPU/double ordinary time evolution with mandatory consistent-mass exchange and optional Poisson demag/Zeeman. It preserves one shared P1 `m` field and uses no nodal fallback. This does not qualify discontinuous DG0 `A`, GPU DG0, other DG0 owners, region couplings, or general conformal coefficient runtime mapping. Explicit projection in extended mode remains the broader executable nodal-field path. Python `study.domain_mesh(..., object_region_markers=...)` preserves explicit precomputed markers through ProblemIR and script export. Unsupported `rkky` and `interlayer_exchange` runtime coupling diagnostics use the public snake_case kind tokens, not Rust enum debug names. Native single-grid multilayer CUDA emits explicit disabled exchange pairs between object ids so separate objects keep free-surface semantics even though intra-object region defaults use harmonic mean. |
...
| Multiple `Ferromagnet` bodies + global demag | CPU reference executable in source; heterogeneous material and native-layer carrier implemented but unqualified; bounded CUDA D-07 source path is implemented but not promoted | ✅ exec | planned | **partial-executable; heterogeneous CPU/API not managed/scientifically/browser qualified; heterogeneous CUDA unsupported** | FDM CPU uses descriptor-based multilayer convolution for eligible stacks. Task 10B wires per-layer `Ms/Aex/alpha` into CPU exchange, RHS, demag source, energy and moment. Task 10C publishes per-layer material/membership artifacts and a native API/planar carrier with object/region isolation, plan-first/persisted cross-check, hashes, revisions and ETag. Evidence remains source/unit/API/client-test only; no managed runtime, scientific or browser/WebGL qualification is claimed. The CUDA batched D-07 source path is bounded to scalar material payloads and 3-D identity descriptors; heterogeneous material fields, heterogeneous thickness, XY offset and `push_pull` must fail closed until the operator and managed device parity are qualified. A compatible native single-grid path is a distinct resolved realization and must not be presented as requested multilayer convolution without explicit provenance. Staged multilayer accepts fixed Heun/RK4/RK23 and rejects adaptive stepping, RK45 and ABM3. FP32 has no current managed qualification. Multilayer direct minimizers PG-BB/NCG are not supported; only `llg_overdamped` is legal. Thermal, spin torque, Oersted, bulk DMI and magnetoelastic remain gated. Boundary containment is planner-only: exactly omitted/`none` `boundary_correction` with both tuning parameters omitted is accepted; `volume`, `full`, every explicit `boundary_phi_floor`, and every explicit `boundary_delta_min` (including `0.0`) reject before layer construction, including explicit single-layer `strategy="multilayer_convolution"`. FEM status is independent. |
| `Exchange` | ✅ exec | ✅ exec | planned | **public-executable** (FDM/FEM) | CPU 6-point stencil in FDM and lumped-mass P1 operator in FEM |
| `Demag` | ✅ exec | ✅ exec | planned | **public-executable** (FDM/FEM, validation pending for full FEM GPU demag) | FDM uses Newell tensor FFT. Executable FEM includes Poisson airbox (`poisson_robin` / `poisson_dirichlet`) on the MFEM/libCEED/hypre CPU lane and strict MFEM/libCEED/CUDA GPU lane. Strict FEM GPU demag mode is `device_hypre_poisson`: RHS, hypre solve, warm-start, recovery `H_demag`, and demag energy stay device-resident, with `uses_gpu_poisson=true`, `hypre_execution_policy=device`, and no hot-loop demag field/magnetization round-trip. `hybrid_cpu_poisson` is only an explicit compatibility/debug mode and must not be silently selected for `study.device("gpu")`. Initial strict GPU scope is P1, double precision, shared-domain airbox Dirichlet/Robin. High-order and public Fredkin-Koehler GPU demag remain gated. The static/time-domain k=0 periodic demag slice has ordinary managed CPU/GPU periodic-antidot relaxation evidence for seam continuity and device-Poisson provenance, but remains below validated production until strict M5 z-padding and primitive-vs-supercell reports pass for the same workload; it may execute only when `ProblemIR.pbc.demag="periodic_airbox_k0"`, the mesh carries `periodic_node_pairs`, `periodic_boundary_pairs`, periodic axes that match `ProblemIR.pbc.axes`, a shared-domain airbox, at least one open axis, and accepted `periodic_pairs.v1` diagnostics proving shared-airbox coverage, magnetic coverage where the magnetic body crosses the selected seam, and opposed-normal boundary-face pairs; selected periodic axes may include `z` for non-fully-periodic cells, while the current antidot qualification fixture remains the film-specific `x/y` periodic, open-`z` case. A historical managed 3x3 primitive-vs-periodic-supercell artifact `.fullmag/reports/fem-demag-periodic-airbox-validation-supercell-pbc/periodic_airbox_validation.csv` passed with `2.4167958871934916e-3` relative energy error against the `2.0e-2` tolerance and zero primitive `H_demag`/`phi` seam mismatch for its fixture; this is supporting evidence for the reduced Poisson path, not current M5 antidot acceptance. Strict GPU k=0 static periodic demag has source-contract support and ordinary managed periodic-antidot GPU gate evidence proving `uses_cuda_kernels=true`, `uses_gpu_poisson=true`, `demag_operator_mode="device_hypre_poisson"`, and zero accepted `H_demag`/normal-flux seam mismatch for `exchange_coupled` and `air_gap`; it must still not be reported as validated production until strict M5 z-padding and primitive-vs-supercell evidence pass. Fully periodic 3D, dynamic frequency-response demag, nonzero-k Floquet magnetostatics, and broader GPU periodic demag remain gated. The public nonzero-k Floquet demag request is `magnetostatic_bc=floquet_airbox`; it is accepted as IR/DSL intent but must fail capability planning until a validated Bloch/Floquet demag-k operator exists. Native FEM source includes an initial body-only `fredkin_koehler` FEM/BEM open-boundary path in the native MFEM CPU subsystem. The time-domain public capability projection now lists `fredkin_koehler` for CPU and GPU FEM profiles; eigen/frequency profiles remain Poisson-only. The source contract and 24-test runner regression pass, while API runtime and scientific qualification remain `NOT VERIFIED`; planner/native behavior is unchanged. Poisson requires a shared-domain mesh with air; `fredkin_koehler` must not require or allocate an airbox and uses the magnetic body boundary surface instead. The Fredkin-Koehler implementation is dense-reference/validation-scale until analytic and cross-model qualification are complete. Native FEM demag exposes an explicit backend-hint `FemLinearSolverPolicy` authoring contract (`CG/GMRES`, `AMG/JACOBI/NONE`, tolerances, iteration cap) while keeping `Demag()` physics-first. For explicit native `poisson_robin`, the managed runtime resolves directly to `hypre_pcg_boomeramg`; live session views preserve requested CPU threads, resolved Rayon threads, and requested/effective OpenMP threads when the native runtime reports them. |
| `InterfacialDMI` / `BulkDMI` | single-grid FDM exec; multilayer interfacial only | planned | planned | **lane-specific executable** (FDM); **bounded energy-observation qualification** (FEM GPU); **under-qualification** (FEM frequency response) | CPU FDM computes DMI field/energy in the reference lane and exposes `H_dmi` as a derived snapshot/preview observable. Public multilayer FDM carries global interfacial DMI through its current CPU/CUDA paths, but the multilayer planner rejects `BulkDMI` instead of silently dropping it. Separate managed uniform-`M_s` native FEM GPU fixtures select interfacial `H_dmi` and bulk `H_dmi_bulk`, and compare `eden_dmi`/`eden_total` with native `E_dmi`/`E_total` at the same source step. That evidence qualifies only the bounded time-evolution energy-observation workload; it does not promote GPU DG0 or every FEM DMI execution surface. The FEM driven frequency-response CPU/GPU P1 tetrahedral tangent DMI slices remain under their separate qualification. Per-layer/per-cell DMI fields remain deferred. |
| `RotatedInterfacialDMI` | FDM CPU/CUDA implemented; current Göbel runtime qualification `NOT VERIFIED` | FEM CPU/GPU implemented, runtime unverified; GPU limited to P1 tet4 | planned | **lane-specific implementation; qualification pending** | Python, `ProblemIR`, study-scoped UI authoring and script rewrite preserve signed finite `D`. Single-grid FDM CPU/CUDA and bounded FEM paths carry the operator without GPU-to-CPU fallback; conventional and rotated DMI cannot be mixed, including object-scoped terms. Nonzero `D` at open magnetic boundaries requires Exchange and positive resolved local stiffness, including active-mask edges. FDM and FEM treat zero `D` as a no-op without Exchange, preserving explicit intent; zero FEM rDMI does not require periodic residual support. Native CUDA rejects cellwise material fields. `H_rotated_dmi` and on-demand `eden_rotated_dmi` are currently materialized only by FDM; scheduled scalar-field snapshots/autosave and FEM field requests reject. `E_dmi` includes `E_rotated_dmi`; total energy counts it once. The historical FP64 CUDA Göbel report passed 15 checks but does not qualify the current 19-check verifier. FEM rejects nonzero rDMI with PBC, and all rDMI eigenmode and frequency-response requests; FEM GPU also rejects prism, pyramid and mixed topology. |
...
| LLG timestep qualification registry | exact NUM-002 workloads `physics_validated`; all other entries retain their own status | exact NUM-002 workloads `physics_validated`; broader lanes `unvalidated` | `unsupported` | **fail-closed evidence owner with tuple-scoped promotion** | Exact lane identity includes integrator and timestep policy. FDM CUDA additionally binds the full storage/compute/FFT/reduction policy and requested/resolved/executed realization. NUM-002 is bound to clean commit `a8ced10bc56ad35d65c7549ef6574488def2fc8e`, empty-diff SHA-256 `e3b0c442…`, the RTX 3070 / CC 8.6 receipt and the exact workloads listed in the JSON registry. Physical Newell thin-film and periodic truncated-images gates passed on that clean source. FEM GPU and every broader FDM tuple keep their previous status. Missing, hybrid, stale, or incomplete evidence fails closed. |
...
| `Relaxation(nonlinear_cg)` | FDM CPU reference executable; CUDA development executable | FEM CPU/GPU development executable | **planned** | **unvalidated** | PR+ direct minimization uses the same physical energy metric, retraction, and Armijo units as PG-BB. Its accepted line-search step is in `m/A`; it owns no RK or time controls. No exact source-bound managed receipt exists, so it is not production-qualified. |
| `Relaxation(tangent_plane_implicit)` | unsupported in strict production | CPU/MFEM development-only in extended mode; forced GPU unsupported | **planned** | **unvalidated** | Strict mode rejects TPI. Extended mode may resolve it only to the CPU/MFEM development lane with explicit requested/resolved provenance and warning. Forced GPU rejects; no hidden GPU-to-CPU fallback or GPU capability claim is permitted. No source-bound managed receipt exists. |
...
| Execution precision `single` | ✅ exec | planned | planned | **lane-specific public-executable** (single-grid CUDA FDM); **physics-validated only for registered NUM-002 workloads** | Dokładna publiczna polityka single-grid to `fullmag.fdm.cuda.precision.single_storage_fp64_reduction.v1`: FP32 storage/compute/FFT z redukcjami FP64. Czysty receipt `a8ced10bc…` promuje wyłącznie tuple wymienione w rejestrze i nie promuje periodic exchange, subcell boundary correction, termiki, pól heterogenicznych ani multilayer. Bounded device-resident multilayer CUDA odrzuca FP32; referencyjny CPU FDM pozostaje wyłącznie double. |
...
| Spatial energy densities (`eden_ex`, `eden_demag`, `eden_ext`, `eden_ani`, `eden_dmi`, `eden_total`) | ✅ reference | bounded CUDA exec | bounded FEM exec | **partial-production-executable** | Canonical IDs and `n_comp=1` cell scalar payloads. FDM CUDA requires the separate full-domain demag visual buffer and scalar device snapshot; FEM keeps its existing element/quadrature source and projection provenance. Availability and cache state are separate. |
