...
| Multiple `Ferromagnet` bodies + global demag | CPU reference executable in source; heterogeneous material wiring unqualified; CUDA no-go for heterogeneous fields | ✅ exec | planned | **partial-executable; heterogeneous CPU not managed/scientifically qualified; heterogeneous CUDA unsupported** | FDM CPU uses descriptor-based multilayer convolution for eligible stacks. Task 10B wires per-layer `Ms/Aex/alpha` into CPU exchange, RHS, demag source, energy and moment, but only source/unit-test evidence exists; no managed runtime or scientific qualification is claimed. Membership publication through artifacts/API/planar monitor remains deferred to Task 10C. The CUDA batched D-07 source path is bounded to scalar material payloads and 3-D identity descriptors; heterogeneous material fields, heterogeneous thickness, XY offset and `push_pull` must fail closed until the operator and managed device parity are qualified. A compatible native single-grid path is a distinct resolved realization and must not be presented as requested multilayer convolution without explicit provenance. Staged multilayer accepts fixed Heun/RK4/RK23 and rejects adaptive stepping, RK45 and ABM3. FP32 has no current managed qualification. Multilayer direct minimizers PG-BB/NCG are not supported; only `llg_overdamped` is legal. Thermal, spin torque, Oersted, bulk DMI and magnetoelastic remain gated. Boundary containment is planner-only: exactly omitted/`none` `boundary_correction` with both tuning parameters omitted is accepted; `volume`, `full`, every explicit `boundary_phi_floor`, and every explicit `boundary_delta_min` (including `0.0`) reject before layer construction, including explicit single-layer `strategy="multilayer_convolution"`. FEM status is independent. |
