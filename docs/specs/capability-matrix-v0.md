...
| `spin_torque.slonczewski` | `slonczewski.fullmag.v1` plus one registered realization | FEM / CPU or GPU / any precision / any mode | canonical macrospin/current-scaling workloads | `semantic_only` | oriented `J_n`, realization identity, SI/Gilbert and lane gates; legacy slices are not v1 evidence |
