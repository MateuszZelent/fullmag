...
| `spin_torque.prescribed_sot` | `prescribed_sot.fullmag.v1` | FDM or FEM / CPU or GPU / any precision / any mode | canonical signed-current macrospin | `semantic_only` | SI/Gilbert/signed-current/mask/stage-time gates and lane proof; managed FDM CUDA now has bounded FP64 eight-step CPU-reference and isolated `0.5×/1×/2×` target-mask evidence, but FEM/FP32/nonlinear/cross-backend qualification remains open |
