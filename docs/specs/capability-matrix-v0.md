...
| `spin_torque.prescribed_sot` | `prescribed_sot.fullmag.v1` | FEM / CPU / double / strict | canonical signed-current macrospin, constant envelope, and target-node-mask one-step reference workload | `reference_executable` | managed MFEM CPU result matches an independent SI Heun oracle and Rust FEM reference for magnetization, `H_eff`, and max RHS; FEM multi-grid, long trajectory, and FEM↔FDM continuum qualification remain open |
| `spin_torque.prescribed_sot` | `prescribed_sot.fullmag.v1` | FEM / GPU / double / strict | canonical signed-current macrospin, constant envelope, target-node-mask one-step reference workload | `reference_executable` | managed CUDA interaction and native MFEM GPU one-step match an independent SI Heun oracle and FEM CPU reference for `m`, `H_eff`, and max RHS; FP32, stage-time envelopes, long trajectories, multi-grid, and FEM↔FDM qualification remain open |
