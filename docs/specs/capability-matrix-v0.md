...
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FDM / CPU / double / strict | canonical macrospin/current-scaling workloads | `reference_executable` | corrected hbar/e SI/Gilbert oracle, signed `J_n`, target mask, and CPU regression gates; full trajectory/current-scaling qualification remains open |
| `spin_torque.slonczewski` | `slonczewski.fullmag.v2` + `slonczewski_thin_layer_homogenized.v1` | FDM / GPU / single or double / strict | canonical macrospin/current-scaling workloads | `production_executable` | managed native CUDA descriptor, signed `J_n`, separate target mask, and one-step FP64 CPU↔device parity pass; FP32, full trajectory, and qualification artifact remain open |
