...
| Multiple `Ferromagnet` bodies + global demag | CPU reference executable; CUDA source-visible/unsafe for heterogeneous assisted scope | ✅ exec | planned | **partial-executable; not production-qualified for FDM CUDA** | FDM CPU uses descriptor-based multilayer convolution for eligible stacks. The CUDA batched D-07 source path is bounded to 3-D identity descriptors and the whole step remains host-authoritative; the assisted `two_d_stack` builder does not yet use the same source/destination-thickness and transfer descriptors as CPU. It must therefore fail closed for heterogeneous thickness, XY offset and `push_pull` until the canonical operator is shared and managed device parity passes. A compatible native single-grid path is a distinct resolved realization and must not be presented as requested multilayer convolution without explicit provenance. Staged multilayer accepts fixed Heun/RK4/RK23 and rejects adaptive stepping, RK45 and ABM3. FP32 has no current managed qualification. Multilayer direct minimizers PG-BB/NCG are not supported; only `llg_overdamped` is legal. Thermal, spin torque, Oersted, bulk DMI and magnetoelastic remain gated. Boundary containment is planner-only: exactly omitted/`none` `boundary_correction` with both tuning parameters omitted is accepted; `volume`, `full`, every explicit `boundary_phi_floor`, and every explicit `boundary_delta_min` (including `0.0`) reject before layer construction, including explicit single-layer `strategy="multilayer_convolution"`. This source-tested preflight does not prove CPU/CUDA runtime or device parity. FEM status is independent. |
