...
| `RotatedInterfacialDMI` | `semantic_only` | `semantic_only` | planned | **semantic-only on FDM/FEM CPU/GPU until operator qualification** | Python and `ProblemIR` preserve signed finite `D`; time-domain planning preserves `rotated_interfacial_dmi` for single-grid FDM, multilayer FDM and FEM without GPU-to-CPU fallback. Open magnetic boundaries require Exchange. Eigen and frequency-domain studies reject explicitly. No lane is executable or validated until its dedicated FDM CPU/CUDA or FEM MFEM/CUDA field, energy, natural-boundary, directional-derivative and managed-runtime gates pass. |
