...
| Transakcja kroku | `backends/fdm/api/c_api.cpp` | `execute_single_grid_step_transaction` | sterowanie krokiem i telemetryką transferów | FDM GPU | host-transfer callsite manifest | dowód statyczny; runtime wymaga GPU |
| Błąd RK23 | `backends/fdm/gpu/cuda/integrators/llg_rk23_fp64.cu` | `rk23_error_kernel` | per-cell embedded error | FDM GPU | `adaptive_error_reduction_contract` | pipeline testowany statycznie |
| Błąd DP45 | `backends/fdm/gpu/cuda/integrators/llg_dp45_fp64.cu` | `dp45_error_kernel` | per-cell embedded error | FDM GPU | `adaptive_error_reduction_contract` | pipeline testowany statycznie |
| Redukcja adaptacyjna | `backends/fdm/gpu/cuda/runtime/reductions_fp64.cu` | `reduce_adaptive_error_policy` | hierarchiczne maksimum i polityka kroku | FDM GPU | `adaptive_error_reduction_contract`; lokalizowany parity nadal wymagany | semantyka pipeline potwierdzona, parity niepotwierdzone |
| Trwały state setup | `backends/fdm/gpu/cuda/runtime/context.cu` | `context_alloc_device` | alokacja buforów urządzenia | FDM GPU | repeated-step allocation counter | dowód statyczny; profil wymagany |
| Gate precyzji | `backends/fdm/api/c_api.cpp` | `valid_precision` | walidacja FP32/FP64 | FDM GPU | parity per precision | dowód statyczny; qualification częściowa |
| Rollback FSAL | `backends/fdm/gpu/cuda/integrators/fsal_policy.hpp` | `context_reject_staged_step` | odrzucenie staged step | FDM GPU | `fsal_retry_transaction_contract` | test kontraktu |
