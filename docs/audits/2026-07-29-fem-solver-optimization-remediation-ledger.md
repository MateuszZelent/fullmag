...
## Independent P1 demag root-cause gate

The delegated fixed-mesh gate is recorded in
`docs/audits/2026-08-03-fem-demag-p1-root-cause-gate.json`. It is a diagnostic
root-cause result, not a production promotion: the delegated payload did not
carry an exact source commit or managed runtime manifest. On the identical
managed CPU FEM state, the independent `mu0/2 b^T u` reconstruction and the
production demag energy agree at relative gaps `8.97e-15` (snapshot step 0) and
`5.16e-15` (terminal step 1). The remaining `5.2258788%` difference from the
FDM/Newell oracle is therefore classified as `p1_approximation_error` in the
P1 potential space. P2 remains unpromoted until same-run analytic and telemetry
...
| T4 | completed | 59a030ca047cc3aa8f2b70609c10aad5484aba50 | aea0050b51ff820f3394ca517e8a551a9496ae2c7eeeb4214eb6404270ed32a0 | docs/audits/2026-08-03-fem-t4-equilibrium-parity-qualification.json; docs/audits/2026-08-02-fem-t4-coarse-parity-measurement.json; scripts/validate_fem_relaxation_equilibrium_parity.py; scripts/test_validate_fem_relaxation_equilibrium_parity.py; scripts/verify_fem_relaxation_equilibrium_parity_semantics.py; scripts/analysis/fem_gpu_benchmark.py; backends/fem/tests/relaxation_source_contract.cpp; docs/physics/0580-canonical-relaxation-equilibrium-contract.md; justfile | promote | 59a030ca047cc3aa8f2b70609c10aad5484aba50 | Managed schema-v3 runtime is exact (`MFEM 4.9`, `HYPRE 3.1.0`, CC 8.9, clean source commit `59a030ca047cc3aa8f2b70609c10aad5484aba50`, tree `69d05ab6...`, source snapshot `8aa6217a...`, source inputs `aa7e5625...`, manifest `aea0050b...`). The fresh `FULLMAG_EQUILIBRIUM_SCOPE=coarse FULLMAG_EQUILIBRIUM_REPEAT=1 just verify-fem-relaxation-equilibrium-parity` exited 0 and the revalidated equilibrium gate reported `pass` for all 4 pairs. Demag PG-BB and NCG have same tolerance, converged torque stop, exact accepted-step parity, and sub-1e-15 final-state differences. Exchange-only NCG retains a one-step executed-count delta (3 versus 2) while passing the existing comparator; the overall direct-minimizer report remains `coverage_only`, so no broad performance promotion is claimed. |
...
| T8 | in_progress | b21e9888195a2ccc2d1ce6adf6d777dde0142e9e |  | docs/audits/2026-08-03-fem-demag-analytic-qualification-gate.json; docs/audits/2026-08-03-fem-demag-p1-root-cause-gate.json; scripts/validate_fem_demag_analytic_qualification.py; scripts/test_validate_fem_demag_analytic_qualification.py; scripts/validate_fem_demag_mesh_airbox_convergence.py; scripts/test_validate_fem_demag_mesh_airbox_convergence.py; examples/assets/fem_demag_analytic_qualification_suite_v1.json; justfile | blocked_by_measurement | b21e9888195a2ccc2d1ce6adf6d777dde0142e9e | The analytic gate is fail-closed and cannot promote a relaxed box or synthetic artifact. Focused source tests pass (`43` tests, `8` subtests), but there is no canonical one-shot prescribed-uniform `compute_fields` producer or managed CPU/GPU analytic artifact. The managed Poisson contract is blocked by the missing PETSc include path `/usr/lib/petscdir/petsc3.15/x86_64-linux-gnu-real/include`; the current exact runtime must be rebuilt after that container issue is repaired. Physics pages remain unpromoted until the analytic artifact exists. |
| T9 | in_progress | bbbf2c627d5b6cbe27372ceb3f8b7cbf06ace794 | 20744e2dfe7198b42fd0593fcaec1d755368ca2a46f4c74f34aed3df38b5f339 | benchmarks/fem-llg/qualification-registry-v1.json; scripts/validate_llg_qualification_registry.py; scripts/test_validate_llg_qualification_registry.py; backends/fem/tests/llg_time_domain_qualification.cpp; docs/audits/evidence/task-9/task-9-managed-qualification-source-gate.json; crates/fullmag-runner/src/timestep_qualification.rs; crates/fullmag-runner/src/lib.rs; crates/fullmag-runner/src/artifacts.rs; crates/fullmag-api/src/schemas/diagnostics.rs; apps/control-room/src/modules/footer/FooterDiagnostics.tsx; docs/physics/0960-canonical-llg-time-domain-solver-and-qualification-contract.md | blocked_by_measurement | bbbf2c627d5b6cbe27372ceb3f8b7cbf06ace794 | Managed schema-v3 runtime is exact (`clean`, compute capability 8.9, HYPRE 3.1.0, source snapshot `b90fc210...`, runtime source inputs `daf75f9e...`). The current CMake qualification rerun passes CPU FP64, GPU FP64, and CPU/GPU parity with artifact hashes recorded in the T9 evidence report. Registry promotion remains fail-closed because the qualification JSON does not embed source/runtime identity and the harness does not execute the exported managed binary. |
