...
- Frequency response is executable only for explicitly proven lanes: dense FEM validation, current native MFEM production CPU/GPU gamma/free-boundary ordinary dynamic demag through the backend tangent provider, and k = 0 static-periodic magnetic slices may emit response artifacts; CPU periodic-airbox k = 0 demag remains a diagnostic slice until the static equilibrium and solved response gates pass; GPU periodic-airbox demag, nonzero-k Floquet demag, and magnetoelastic response remain rejected until separately qualified.
