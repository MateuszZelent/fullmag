...
- Strict M5 now has artifact-backed report entry points for z-padding, central-cell extraction, and primitive-vs-supercell comparison. `verify-fem-static-pbc-demag-equilibrium-runtime` rejects missing comparison report paths before running the ordinary CPU/GPU relaxation gates, the report writer rejects self-comparison or incompatible-workload artifact roots, and the periodic-antidot validator requires report `workload` to match the accepted run and independently rejects over-threshold metrics even when a report declares `status="ok"`. The supercell report now also requires explicit `fem_static_pbc_supercell_central_cell.v1` extraction provenance, so global supercell averages cannot masquerade as central-cell agreement.
...
| M5 | Static PBC equilibrium and demag stabilization for magnonic-crystal unit cells | 75% | Static PBC intent now flows through `ProblemIR.pbc`/`metadata.pbc`, mesh option propagation is explicit, planner axes must match `ProblemIR.pbc`, `periodic_pairs.v1` now carries domain-aware magnetic/airbox coverage and boundary-face orientation diagnostics through artifacts and the v2 mesh resource, same-step `H_demag`, gauge-invariant `demag_phi`, and runtime-emitted `diagnostics/fem_static_pbc_demag_seams.v1.json` flux/charge diagnostics are required, GPU minimizer trial states and recovered GPU `H_demag` are projected onto periodic classes, and the ordinary managed CPU/GPU periodic-antidot gates pass for `exchange_coupled` and `air_gap`; strict M5 now has artifact-backed comparison report targets and rejects missing, self-comparison, incompatible-workload, workload-mismatched, over-threshold, or missing-central-extraction reports, but strict acceptance still requires fresh z-padding and primitive-vs-supercell reports for the same workload. |
...
- `examples/fem_periodic_antidot_relax_exchange_coupled_supercell_3x3.py` and `just verify-fem-static-pbc-demag-supercell-runtime` now provide the canonical managed path to produce primitive/3x3 supercell artifacts for the exchange-coupled workload and write the strict supercell report from explicitly supplied central-cell extraction inputs. `just prepare-fem-static-pbc-demag-supercell-runtime-artifacts` can now produce the unit/supercell runtime artifact roots before those extraction inputs are available.
- `scripts/write_fem_static_pbc_supercell_central_cell_artifact.py` and `just write-fem-static-pbc-demag-supercell-central-cell-artifact` now provide the explicit central-cell extraction artifact producer from supplied magnetic-node indices, field-cell indices, central-cell demag energy, and central-cell torque residual; the producer validates index ranges against `m_final.json`, `H_demag.zarr`, and `demag_phi.zarr`, rejects central scalar values above global supercell `E_demag` or final torque, accepts literal index lists or index files, and does not infer central energy from the total supercell energy.
