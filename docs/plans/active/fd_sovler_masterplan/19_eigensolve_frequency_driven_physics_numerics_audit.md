...
qualification. `runtime_verified` is used only for previously existing runtime
evidence and is not used for this update unless a runtime was actually rerun,
which it was not.
...
| F-17 | MEDIUM_HIGH | Driven residuals and preconditioners | Driven response needs true unpreconditioned block residuals and preconditioner quality, not only tracked GMRES residual. | Publish tracked and recomputed residuals, block partitions, preconditioner probes and stop reasons; accept only true residual against the original operator. | resolved_in_docs | implemented | not_run | `docs/plans/active/fd_sovler_masterplan/11_runtime_telemetry_performance.md`; `.fullmag/reports/frequency-domain-periodic-airbox-gpu-runtime/artifacts/response/diagnostics/solver.v1.json`; `backends/fem/src/frequency_domain/driven_response_solver.cpp` |
...
| F-22 | MEDIUM | GPU callback/readiness levels | GPU operator callbacks, shifted apply probes and dense macrospin eigensolve are different readiness levels and must not be collapsed. | Keep labels separate: `gpu_operator_host_krylov`, `gpu_dense_contract_eigensolver`, `gpu_dense_k0_macrospin_modal_eigen`, `gpu_device_krylov`, `gpu_modal_device_krylov`. | resolved_in_docs | implemented | not_run | `docs/plans/active/fd_sovler_masterplan/17_eigen_k0_gpu_readiness_audit.md`; `docs/plans/active/fd_sovler_masterplan/25_frequency_domain_readiness_matrix.json`; `backends/fem/gpu/cuda/frequency_domain/driven_response_gpu.cu` |
...
- `modal_eigen/gpu/k0/periodic_airbox_k0` has dense/apply algebra evidence but
  no scalable modal GPU solver.
