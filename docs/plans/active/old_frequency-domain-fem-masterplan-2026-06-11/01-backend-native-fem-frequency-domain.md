...
- Driven frequency response is executable as dense FEM validation and as limited native MFEM production CPU/GPU slices for gamma-point/free-boundary and k=0 static-periodic magnetic response with exchange, Zeeman, uniaxial anisotropy, interfacial/bulk DMI on supported P1 tetrahedral meshes, Gilbert damping, and ordinary k=0 dynamic demag through the backend demag-tangent provider. Static-periodic response requires validated `mesh.periodic_node_pairs` and is enforced by tangent-space periodic projection. GPU periodic-airbox Poisson, nonzero-k Floquet/Bloch demag, Floquet phase enforcement beyond the validated no-demag scalar slice, magnetoelastic response, and device-resident GPU demag Poisson remain explicitly unsupported.
