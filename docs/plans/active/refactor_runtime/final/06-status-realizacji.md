...
Data ostatniego przyrostu: 26.09.2026. Checkout: lokalny `master`, HEAD `93f11dbc564c00b725d174ccb2fd0ff9a96493c9` (dirty; bieżące zmiany P2/P3a, P4 i ekstrakcja B/FDM pozostają w working tree). Receipt'y i starsze smoke'y zachowują własną przypiętą tożsamość źródła.
...
heartbeat/cancel/retry, orphan reconciliation po awarii supervisora, pełnego
subprocess E2E oraz kwalifikacji pozostałych lane'ów. P3 wynosi **58%**, P5
około **5%**, a całość około **28%**. Szczegóły:
...
i [`p3/29-accepted-worker-supervisor.md`](p3/29-accepted-worker-supervisor.md).
## Tabela zbiorcza
...
| **P5** | Izolowany runtime, fencing, cancel/recovery, live steering | **5%** | Częściowa baza P5-B: `DurableWorkerInbox`, trwały pending/applied, pojedynczy slot supervisora, spawn/wait procesu, reconciliation terminalnego ACK oraz fenced zwolnienie lease; regresje źródłowe i procesowe PASS. Pełny runtime pozostaje niezakwalifikowany. | `fullmag-cli RunJson` nadal omija accepted RunSpec i wywołuje solver bezpośrednio; brakuje automatycznego fenced wyboru taska/attemptu, transportu CLI, wyścigów anulowania/retry, heartbeat-generation fencing, orphan recovery po awarii supervisora, pełnego process E2E i steering qualification. |
