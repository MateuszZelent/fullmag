...
Data ostatniego przyrostu: 26.09.2026. Baza przyrostu: `master@8a95167820594d577cde1a8d5d68a497e0e78d46`; końcowe receipt'y przypinają także hash treści bieżącego przyrostu. Receipt'y i starsze smoke'y zachowują własną przypiętą tożsamość źródła.
...
terminalny barrier → potwierdzony exit → release dokładnego lease: **1/1 PASS**,
receipt `00ebd08b3e1d46b5b5b754a2779261e4`, `source_changed_during_run=false`.
...
heartbeatów, operator cancel, retry policy i orphan reconciliation po awarii
supervisora oraz kwalifikacji pozostałych lane'ów. P3 wynosi **61%**, P5 około
**10%**, a całość około **28%**. Szczegóły:
...
i [`p3/31-supervisor-worker-e2e.md`](p3/31-supervisor-worker-e2e.md).
...
| **P5** | Izolowany runtime, fencing, cancel/recovery, live steering | **10%** | Częściowa baza P5-B: `DurableWorkerInbox`, trwały pending/applied, pojedynczy slot supervisora, spawn/wait procesu, wymagany jawny timeout, potwierdzone zakończenie potomka przed rekoncyliacją, reconciliation terminalnego ACK, fenced zwolnienie lease oraz pełne process E2E ograniczonego FDM CPU; regresje źródłowe i procesowe PASS. Pełny runtime pozostaje niezakwalifikowany. | `fullmag-cli RunJson` nadal omija accepted RunSpec i wywołuje solver bezpośrednio; brakuje automatycznego fenced wyboru taska/attemptu, transportu CLI, operator cancel i wyścigów anulowania/retry, heartbeat-generation fencing, orphan recovery po awarii supervisora, process E2E pozostałych lane'ów i steering qualification. |
