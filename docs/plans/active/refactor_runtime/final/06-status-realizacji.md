...
Data ostatniego przyrostu: 26.09.2026. Baza przyrostu: `master@c0f2c1de150cb579875f725abce066c56aec6520`; końcowe receipt'y przypinają także hash treści bieżącego przyrostu. Receipt'y i starsze smoke'y zachowują własną przypiętą tożsamość źródła.
...
heartbeatów, operator cancel, retry policy, orphan reconciliation po awarii supervisora, pełnego
subprocess E2E oraz kwalifikacji pozostałych lane'ów. P3 wynosi **59%**, P5
około **8%**, a całość około **28%**. Szczegóły:
...
i [`p3/30-supervisor-timeout.md`](p3/30-supervisor-timeout.md).
## Tabela zbiorcza
...
| **P5** | Izolowany runtime, fencing, cancel/recovery, live steering | **8%** | Częściowa baza P5-B: `DurableWorkerInbox`, trwały pending/applied, pojedynczy slot supervisora, spawn/wait procesu, wymagany jawny timeout, potwierdzone zakończenie potomka przed rekoncyliacją, reconciliation terminalnego ACK oraz fenced zwolnienie lease; regresje źródłowe i procesowe PASS. Pełny runtime pozostaje niezakwalifikowany. | `fullmag-cli RunJson` nadal omija accepted RunSpec i wywołuje solver bezpośrednio; brakuje automatycznego fenced wyboru taska/attemptu, transportu CLI, operator cancel i wyścigów anulowania/retry, heartbeat-generation fencing, orphan recovery po awarii supervisora, pełnego process E2E i steering qualification. |
