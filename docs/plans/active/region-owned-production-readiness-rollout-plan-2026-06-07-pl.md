...
| Zablokowac region poza parentem w importowanych payloadach | Gotowe. `validate_scene_document()` odrzuca full-scene/imported object-frame box/cylinder/sphere regiony wychodzace poza owner bounds z deterministycznym `REGION_OUTSIDE_OWNER_BOUNDS`; testy `scene_document_validation_rejects_object_frame_region_outside_owner_bounds` i `scene_document_validation_uses_full_oblique_cylinder_extent` pokrywaja oversized, przesuniety i oblique cylinder payload. |
...
| Field assets | Czesciowo gotowe. Data-plane material-field detail konsumuje typed `MaterialFieldAssetIR` z runtime metadata (`asset_id`, opcjonalny `artifact_path`, mesh identity, location, component count, source kind, algorithm, timing, values/statystyki). Testy `material_field_data_resource_uses_realized_material_field_asset_metadata` i `material_field_data_resource_reads_file_backed_realized_asset` pokrywaja inline metadata i file-backed asset z aktywnego artifact dir. Runner zapisuje teraz FEM material-field artifacty `material-fields/{asset_id}.json` dla zmaterializowanych `Ms/Aex/alpha` node fields oraz `Ms/Aex` element fields i publikuje lekkie referencje w `metadata.material_field_assets`; test `write_artifacts_persists_fem_material_field_asset_files` pokrywa trwaly zapis file-backed assetu. Pozostaje kwalifikacja pozostalych parametrow i szersza backend parity/provenance. |
| Data-plane resource | Czesciowo gotowe. `GET /v2/sessions/current/data/material-fields` kataloguje authored material-field payloady, a `GET /v2/sessions/current/data/material-fields/{field_id}` zwraca per-assignment realized samples/statystyki oraz typed asset metadata z current runtime fields/assets albo runner-written file-backed material-field artifact; `GET /v2/sessions/current/data/mesh-region-membership/{region_id}` zwraca membership indices z `mesh_parts`, fallbackiem `object_segments` oraz geometryczna projekcja typed Box/Cylinder/Sphere regionu na FEM nodes/elements/boundary-face centroidy, gdy region nie ma osobnego mesh part/segment. `GET /v2/sessions/current/data/mesh-region-memberships` zwraca listowy snapshot dostepnych memberships dla enabled authored object regions oraz owner-qualified `unresolved_regions[]` dla regionow bez wspieranej membership realizacji. Projection respektuje object-frame owner translation i zwraca `realization_method` plus warning, ze to fallback centroidowy, nie conformal mesh part. Object-frame projection jest capability-gated dla non-identity rotation albo non-unit scale, zeby nie publikowac falszywych membership indices; control-room viewport konsumuje listowy resource dla wszystkich authored projected/non-segmented kandydatow. Pelna transformowana projekcja i pelna provenance kwalifikacja pozostaja otwarte. |
...
| FEM boundary markers | Czesciowo gotowe. `GET model/couplings` raportuje marker-backed boundary faces dla surface endpointow i zwraca `missing_boundary_markers`, gdy geometria powierzchni jest znaleziona tylko jako mesh triangle preview bez boundary markerow wymaganych przez runtime coupling. Test `authoring_coupling_resource_blocks_surface_without_boundary_markers` pokrywa blokade preview-only. Runtime operator nadal pozostaje niezaimplementowany. |
...
| Realized membership resource | Czesciowo gotowe. `GET /v2/sessions/current/data/mesh-region-membership/{region_id}` zwraca `mesh_id`, `mesh_revision`, `mesh_part_ids`, `element_indices`, `node_indices` i `boundary_face_indices` dla jednego regionu. `GET /v2/sessions/current/data/mesh-region-memberships` zwraca listowy snapshot dla enabled authored object regions: `memberships[]` plus owner-qualified `unresolved_regions[]`. Oba zasoby obsluguja regiony zrealizowane jako mesh-backed FEM parts, regiony dostepne przez `object_segments` oraz typed Box/Cylinder/Sphere regiony projektowane geometrycznie na aktualny FEM mesh. Projection uwzglednia translacje ownera dla object-frame regionow i publikuje `realization_method="shape_centroid_geometry_projection_v1"` oraz ostrzezenie o fallbacku centroidowym. CSG i object-frame non-identity rotation/non-unit scale nie sa projektowane jako fallback membership; pelna transformowana projekcja i pelna provenance kwalifikacja pozostaja otwarte. |
| Realized overlay | Mesh-backed conformal region parts renderuja sie przez prawdziwy mesh i maja data-plane membership indices. Viewport ma teraz typed data facade/resource hook dla listowego `mesh-region-memberships`; resource hook filtruje memberships dla authored projected/non-segmented regionow, ktore nie sa juz mesh-backed w manifest, i zamienia membership bez `mesh_part_ids` na synthetic realized mesh overlay parts z `element_indices`/`node_indices`. |
...
- Dodano podstawowa geometryczna projekcje membership dla non-segmented
  Box/Cylinder/Sphere regionow bez mesh part/segment. Test
  `mesh_region_membership_projects_authored_region_shape_without_mesh_part`
  potwierdza `source="geometry_projection"` oraz zwrot element/node/boundary-face
  indices. Test
  `mesh_region_membership_projects_object_frame_region_with_owner_translation`
  potwierdza object-frame sampling z owner translation. Projection response niesie
  teraz `realization_method="shape_centroid_geometry_projection_v1"` i warning,
  ze nie jest to conformal mesh part. Object-frame non-identity rotation albo
  non-unit scale jest teraz capability-gated jako brak supported projection, zeby
  nie zwracac falszywych indices; pelna transformowana projekcja, pelna
  provenance kwalifikacja pozostaje otwarta.
- Dodano frontendowy typed facade/resource hook dla
  `GET /v2/sessions/current/data/mesh-region-membership/{region_id}` oraz
  `GET /v2/sessions/current/data/mesh-region-memberships`. Resource hook
  konsumuje listowy backend resource dla wszystkich authored
  projected/non-segmented regionow, ktore nie sa mesh-backed w aktualnym
  manifest. Viewport adapter mapuje membership `element_indices`/`node_indices`
  na synthetic mesh overlay part. Testy `ControlRoomApi.test.ts`,
  `geometryLifecycleResources.test.ts` i `useViewport3DSceneModel.test.ts`
  pilnuja centralnego path/facade, deterministycznego batch key/revision oraz
  modelowego wejscia realized overlay bez zaleznosci od selekcji.
