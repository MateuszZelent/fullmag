...
| Realized membership resource | Czesciowo gotowe. `GET /v2/sessions/current/data/mesh-region-membership/{region_id}` zwraca `mesh_id`, `mesh_revision`, `mesh_part_ids`, `element_indices`, `node_indices` i `boundary_face_indices` dla regionow zrealizowanych jako mesh-backed FEM parts, regionow dostepnych przez `object_segments` oraz typed Box/Cylinder/Sphere regionow projektowanych geometrycznie na aktualny FEM mesh. Projection uwzglednia translacje ownera dla object-frame regionow i publikuje `realization_method="shape_centroid_geometry_projection_v1"` oraz ostrzezenie o fallbacku centroidowym. CSG, rotation/scale transform i pelna provenance kwalifikacja pozostaja otwarte. |
| Realized overlay | Mesh-backed conformal region parts renderuja sie przez prawdziwy mesh i maja data-plane membership indices. Data-plane potrafi juz zwrocic projected/non-segmented membership dla typed Box/Cylinder/Sphere, ale membership-color overlay w viewport nadal nie konsumuje tej sciezki jako osobnego realized overlay. |
...
- Dodano podstawowa geometryczna projekcje membership dla non-segmented
  ...
  ze nie jest to conformal mesh part. Rotation/scale transform, pelna provenance
  kwalifikacja i viewport overlay consumption pozostaja otwarte.
