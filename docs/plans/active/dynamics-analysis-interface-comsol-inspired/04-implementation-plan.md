...
- [ ] Define TypeScript types:
  ...
  ```
- [ ] Implement `phasorAdapter(convention: PhasorConvention)` returning `{ decayRateSign: 1 | -1, phaseAnimationDirection: 1 | -1 }`.
- [ ] Implement `floquetPhaseAdapter(convention: FloquetSpatialConvention)` returning `{ spatialPhaseSign: 1 | -1 }`.
- [ ] Integrate adapter into `AnalysisFieldOverlayController` so the viewport shader never hard-codes a sign.
- [ ] Write unit tests with both convention values verifying correct sign extraction.
- [ ] Run:
...
- [ ] Add validation check in the Driven Frequency Response Inspector: if no `δh` source is configured, display a blocking status card:
  ```
...
- [ ] Add or update Explorer node models for:
...
- [ ] Write failing tests in the existing Explorer builder tests proving each semantic node has a distinct `kind` and selection ref.
- [ ] Implement the tree builder changes.
- [ ] Verify every new node routes to a specific Inspector panel ID.
- [ ] Run:
...
- [ ] Add failing tests for each node kind:
...
- [ ] Implement dedicated panels or panel-model branches.
- [ ] For Study Setup nodes, ensure that inputs trigger stage draft updates and dispatch transactions through the canonical authoring transaction path, verifying capability-gated status.
- [ ] For Results nodes, ensure panels render read-only properties from revision-locked run artifacts.
- [ ] Each panel must show resource identity, revision/status, provenance, available actions, and unsupported/degraded reason where applicable.
- [ ] Run:
...
- [ ] Register `object.mode_visualization`, `object.mode_visualization.group`, `object.mode_visualization.field`, `object.mode_visualization.view` kinds in `inspectorRegistry.tsx`.
- [ ] Create `ModeVisualizationInspectorPanel` that reuses `FrequencyDomainModeDisplayControls` as its core control card, sharing appearance model with `ObjectVisualizationPanel`.
- [ ] The panel must show: source (driven/eigenmode), frequency/sample/mode indices, active fieldId and resource status, Real/Imag/Abs/Phase/PhaseRotatedReal toggle, phase slider, animation toggle, surface/vectors/colormap controls inherited from object visualization, and capability warnings.
- [ ] Write failing test: selecting an `object.mode_visualization.view` node renders the mode visualization inspector, not the placeholder.
- [ ] Run:
...
- [ ] Add model tests proving modal chart points preserve sample index, raw mode index, frequency, damping/imaginary frequency, residual, tangent leakage, and mode field ID.
- [ ] Implement calculated complex frequency fields in the inspector via a solver/artifact sign convention adapter:
...
- [ ] Add UI tests proving controls exist for component and complex view.
- [ ] Add mode selection actions: inspect, plot 3D, animate phase, compare.
- [ ] Ensure chart click maps to a canonical frequency-domain mode selection ref.
- [ ] Keep y-axis units explicit and avoid mixed diagnostic units on one axis unless split or dual-axis mode is explicit.
- [ ] Run:
...
- [ ] Add tests for observable/component selection: `delta mx`, `delta my`, `delta mz`, `|delta m|`.
- [ ] Add tests for quantity selection: amplitude, phase, absorbed power, susceptibility.
- [ ] Filter chart series by selected observable and selected quantity while preserving original row identity for selection.
- [ ] Remove hidden valid-point truncation from action lists; use scrollable or table-backed point inspection instead.
- [ ] Format tooltips as named fields with units. Never show raw arrays/tuples.
- [ ] Run:
...
- [ ] Add a peak table model for modal peaks, driven peaks, and paired nearest modal/driven comparisons.
- [ ] Implement spatial overlap coefficient ($\eta_j$) calculation for comparison pairs using the Hermitian inner product with FEM mass-matrix weighting:
  ...
  - [ ] Handle missing field payload states (render `degraded` and request link).
- [ ] Add tests for detuning, overlap coefficient, source links, missing modal data, missing driven data, and unsupported comparison.
- [ ] Add Inspector panels for selected peak and comparison pair (rendering $\eta_j$ value).
- [ ] Add chart peak markers with exact frequency labels and selection refs.
- [ ] Ensure peak labels use display units while internal values remain Hz.
- [ ] Run:
...
- [ ] Add tests for analysis-field overlay query updates: field ID, component, view, phase, wavevector $\mathbf{k}_F$, source label.
- [ ] Extend `AnalysisFieldOverlayState` type with `wavevectorKf?: [number, number, number]` (SI, rad/m) and `cellOrigin?: [number, number, number]` (SI, meters). The viewport shader requires these for Floquet phase projection. Without this extension, Floquet mode visualization is impossible.
- [ ] Add `floquetSpatialConvention?: FloquetSpatialConvention` field to `AnalysisFieldOverlayState`. The viewport must read the sign from this field through the `floquetPhaseAdapter`, not from a hard-coded formula. Values: `"dst_equals_src_exp_minus_i_k_dot_delta_r"` (COMSOL manual convention) or `"dst_equals_src_exp_plus_i_k_dot_delta_r"`.
- [ ] Add `phasorConvention: PhasorConvention` field to `AnalysisFieldOverlayState`. Required for correct decay rate sign extraction and phase animation direction.
- [ ] Implement spatial phase propagation in the viewport shader/renderer for Floquet modes:
...
- [ ] Support tiled/periodic unit-cell phase offsets $\mathbf{k}_F \cdot \mathbf{R}$ where cell translation $\mathbf{R}$ is non-zero.
- [ ] Route mode/response plot actions through kernel commands or the existing analysis overlay controller.
- [ ] Do not import viewport internals into inspector or charts.
- [ ] Ensure missing field resources disable plot actions with explicit reason.
- [ ] Browser smoke must assert visible canvas, nonzero drawing buffer, and no lost WebGL context for a selected analysis field.
- [ ] Run:
  `pnpm --dir apps/control-room test -- --run AnalysisFieldOverlay`
- [ ] Implement field buffer eviction policy: maintain an LRU cache of at most 10 decoded analysis field buffers. When the 11th mode is selected, evict the least-recently-used buffer. Do not hold GPU texture references for evicted buffers.
- [ ] Implement ECharts instance lifecycle budget: no more than 4 concurrent ECharts instances across all analysis plot surfaces. Dispose the oldest instance when the limit is exceeded.
...
- [ ] Add route-aware chart composition for modal spectrum, response sweep, dispersion, and comparison.
- [ ] Implement Apache ECharts configurations:
...
- [ ] Implement a Brillouin Zone Path Editor UI to author high-symmetry point sweeps ($\Gamma - X - M - \Gamma$, etc.) with segment densities.
- [ ] Add "Trigger Field Calculation" button in the Inspector for dispersion points that lack saved field payloads, triggering background solver execution.
- [ ] Add resource-status and capability badges.
- [ ] Add exact point table or docked data inspector for selected chart point.
- [ ] Ensure ECharts instances (`echarts.init`) and resize observers are cleanly disposed on unmount to prevent memory leaks.
- [ ] Run chart lifecycle and adapter tests.
...
- [ ] Implement capability gating on all commands: if the active solver backend does not support the required capability (e.g., nonzero-k Floquet demag, coupled solid mechanics), disable the command and show the rejection reason.
- [ ] Add DMI boundary condition warning: if DMI is enabled in frequency-domain studies and the backend cannot validate the DMI boundary operator, the Inspector must render diagnostic `frequency_domain.dmi_boundary_condition_uncertain` with severity `warning`. This follows COMSOL manual guidance that frequency-domain DMI boundary conditions are not yet fully resolved.
- [ ] Add command tests for available, disabled, unsupported (gated), rejected, and completed states.
- [ ] Main menu, ribbon, context menu, and command palette must render the same command registry entries.
- [ ] Run:
...
- [ ] Use shared shadcn/ui-style primitives for tabs, dropdowns, segmented
  controls, tooltips, dialogs, switches, and menus.
- [ ] Keep charts dense and readable: tabular numbers, units, clear legends,
  hover/focus states, stable dimensions.
- [ ] Avoid nested cards and decorative surfaces.
- [ ] Verify light/dark Catppuccin themes.
- [ ] Run visual regression snapshot tests for light/dark Catppuccin themes.
...
- [ ] Run API hygiene checks:
  `rg "fetch\\(" apps/control-room/src`
- [ ] Run module boundary checks:
  `rg "from ['\\\"]\\.\\./" apps/control-room/src/modules`
- [ ] Run browser smoke for analysis-field overlay if viewport behavior changed.
- [ ] Verify Explorer tree virtualizes correctly with 1000+ frequency sweep points (no DOM explosion).
- [ ] Verify field buffer eviction: clicking through >20 modes must not exceed GPU memory budget.
- [ ] Verify ECharts dispersion chart renders <5000 data points without frame drops.
- [ ] Verify chart unit invariant: every analysis chart Y-axis represents one selected observable with one unit. Mixed-unit series on one axis is a correctness bug. Diagnostics (residual norms, convergence) may appear in tooltip/table, never as hidden mixed-axis chart series.
- [ ] Update this plan with completed tasks and any deferred capability gaps.

## Acceptance Checklist

- [ ] Explorer exposes all dynamics result/resource families.
- [ ] Every Explorer node has a dedicated Inspector.
- [ ] Modal and driven products are visibly distinct.
- [ ] Component and complex-view selectors are first-class controls.
- [ ] Spectrum charts show one selected quantity per axis unless split/dual-axis
  mode is explicit.
- [ ] Peak selection synchronizes chart, Inspector, table, and viewport overlay.
- [ ] Unsupported/degraded states are visible and actionable.
- [ ] No direct module `fetch()`.
- [ ] No module imports another module's internals.
- [ ] ECharts and viewport resources dispose correctly.
