# FDM-CPU-NUM-002 — Jawny kontrakt projected RK i kwalifikacja rzędu
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-CPU-NUM-002`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Każdy etap RK jest normalizowany do sfery przed kolejną oceną RHS. Jest to inna metoda niż klasyczny tableau w przestrzeni otaczającej; embedded estimator jest budowany z RHS w punktach po projekcji.

### Skutek

Deklarowany rząd może nie być zachowany, estimator może niedoszacować błędu fazowego, a AoS/SoA/GPU mogą różnić się kolejnością kombinacja–projekcja–RHS.

## 3. Docelowy kontrakt

Projection policy jest częścią resolved plan i provenance. Solver albo kwalifikuje projected RK jako osobną metodę z własnym order oracle, albo przechodzi na metodę geometryczną/tangent-plane.
...
| `crates/fullmag-ir/src/plan.rs` | planowanie, walidacja i transport konfiguracji |
...
- Dodać enum `ProjectionPolicy` i wersję realizacji integratora.
- Zdefiniować, czy embedded error używa kandydatów przed czy po projekcji; jedna reguła dla wszystkich lane.
- Oddzielić `combine_stage` od `project_stage`, aby testować obie operacje.

### Etap C — integracja i migracja

- W przypadku projected RK stroić regulator na obserwowany order, nie na nazwę tableau.
- Rozważyć końcową projekcję tylko accepted state lub tangent-plane update jako osobną capability.
- Zapisywać maksymalny defect normy przed projekcją i maksymalny kąt projekcji.
...
- Temporal convergence macrospin dla kolejnych `dt` i każdego integratora.
- Phase-error test przy `alpha=0` i energy-decay test przy `alpha>0`.
- Porównanie pre/post projection estimator z niezależnym step-doubling oracle.
- AoS/SoA parity kolejności projekcji.
- Test silnego bezpośredniego torque, gdzie defect normy jest mierzalny.
...
- observed order
- max pre-projection norm defect
...
- Zmiana projection policy zmienia trajektorie i wymaga jawnej migracji.
- Usunięcie projekcji może pogorszyć stabilność bez alternatywy geometrycznej.

## 11. Poza zakresem

- Dowód matematyczny wszystkich kombinacji interakcji.

## 12. Definition of Done

Ustalenie `FDM-CPU-NUM-002` jest zamknięte dopiero wtedy, gdy:
