# FDM-CPU-PHY-002 — Przestrzenne `Ms` i `alpha` w termice oraz lokalnych kontraktach
...
| Zależności | `FDM-CPU-TRX-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-CPU-PHY-002`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Ścieżka termiczna używa skalarnego `Ms/alpha`, mimo że model i ABI posiadają pola przestrzenne. W heterogenicznym materiale wariancja szumu powinna być liczona lokalnie.

### Skutek

Niewłaściwa temperatura efektywna i relaksacja w regionach o różnych parametrach; możliwa rozbieżność CPU/GPU oraz niepoprawna fizyka na interfejsach.

## 3. Docelowy kontrakt

Wszystkie człony odczytują parametry przez jeden `MaterialAccessor` o jednoznacznej semantyce komórki, regionu i interfejsu. Termiczne sigma jest per-cell i zależy od lokalnej objętości, `Ms`, `alpha`, `gamma` i `dt`.
...
| `crates/fullmag-engine/src/fdm/shared/terms.rs` | modyfikacja lub weryfikacja kontraktu |
...
- Wprowadzić `MaterialFieldView` zapewniający scalar fallback bez branchu w każdej interakcji.
- Precompute per-cell static factor termiczny, a przy zmianie `dt` aktualizować wyłącznie wspólny mnożnik `dt^-1/2`.
- Walidować `Ms>0`, `alpha>=0`, skończoność i aktywną maskę przed uruchomieniem.

### Etap C — integracja i migracja

- Ujednolicić źródło lokalnych parametrów dla LLG, energii, STT/SOT i observables.
- W provenance zapisać digest pól materiałowych i fallback policy.
...
- Dwa regiony z różnym `Ms/alpha`: stosunek wariancji zgodny z analityką.
- Inactive cells i `Ms=0` nie generują szumu ani NaN.
- CPU/GPU statistical parity w granicach przedziału ufności.
- Restart determinism dla stałego seeda.
- Test jednorodny potwierdzający brak regresji względem scalar path.
...
- Per-cell współczynniki zwiększą footprint i bandwidth.
- Należy jasno rozstrzygnąć Itô/Stratonovich oraz sposób integracji szumu.

## 11. Poza zakresem

- Temperatura zależna od czasu i sprzężony solver ciepła.

## 12. Definition of Done

Ustalenie `FDM-CPU-PHY-002` jest zamknięte dopiero wtedy, gdy:
