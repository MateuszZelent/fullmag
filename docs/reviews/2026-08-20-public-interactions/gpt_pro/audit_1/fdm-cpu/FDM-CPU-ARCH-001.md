# FDM-CPU-ARCH-001 — Jedna semantyka fizyki dla AoS i SoA
...
| Zależności | `FDM-CPU-PHY-002`, `FDM-CPU-PHY-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-CPU-ARCH-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Ścieżki AoS i SoA nie mają identycznego pokrycia DMI, termiki, STT/SOT, energii, frozen spins i cache FSAL. Wybór reprezentacji może więc zmienić rozwiązywane równanie, a nie tylko wydajność.

### Skutek

Silent physics downgrade, trudne do wykrycia różnice trajectory oraz benchmarkowanie szybkiej ścieżki o innym zakresie fizyki niż produkcja.

## 3. Docelowy kontrakt

AoS i SoA są jedynie layoutami danych nad jednym katalogiem operatorów i jednym kontraktem RHS. Brak implementacji danego członu powoduje fail-closed, nie jego pominięcie.
...
- Zbudować capability matrix operator × layout i sprawdzać ją w plannerze.
- Wyodrębnić wspólne funkcje matematyczne oraz layout-specific iterators/views.
- Zapewnić identyczną kolejność składania H_eff i direct torque.

### Etap C — integracja i migracja

- Ujednolicić maski, frozen spins, PBC, termikę, FSAL i obserwable.
- Dodać runtime receipt `state_layout` oraz listę rzeczywiście wykonanych operatorów.
- Usunąć automatyczny fallback AoS↔SoA w środku kroku; ewentualna konwersja tylko na granicy sesji.
...
- Field/RHS/stage/step parity dla każdego aktywnego członu.
- Property tests losowych małych siatek z maskami i PBC.
- Trajectory parity przez wiele kroków dla fixed i adaptive integratorów.
- Test fail-closed dla członu celowo niezaimplementowanego w jednym layoucie.
- Benchmark dowodzący, że parity testuje ten sam resolved plan.
...
- layout conversions/session
- operator parity max error
...
- Abstrakcja może pogorszyć wektoryzację, jeśli nie zostanie oparta o monomorfizowane views.

## 11. Poza zakresem

- Usunięcie AoS z publicznych snapshotów.

## 12. Definition of Done

Ustalenie `FDM-CPU-ARCH-001` jest zamknięte dopiero wtedy, gdy:
