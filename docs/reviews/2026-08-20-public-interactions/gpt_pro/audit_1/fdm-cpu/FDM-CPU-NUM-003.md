# FDM-CPU-NUM-003 — ABM3: stały krok albo poprawne współczynniki variable-step
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-CPU-NUM-003`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Historia ABM przechowuje wartości `dt`, natomiast predictor/corrector używa klasycznych współczynników dla stałego kroku. Po zmianie kroku historia może być formalnie niezgodna.

### Skutek

Utrata rzędu, niestabilność lub błąd fazy po zmianie `dt`, restarcie albo zmianie polityki output/zdarzeń.

## 3. Docelowy kontrakt

ABM3 jest jawnie fixed-step i resetuje historię po każdej zmianie kroku albo implementuje współczynniki zależne od stosunków kolejnych kroków z testem rzędu.
...
- Najpierw wprowadzić bezpieczny gate: `|dt-dt_history|/dt <= tolerance`; inaczej wyczyścić historię i wykonać startup.
- Oznaczyć ABM3 jako fixed-step w capability/planner, dopóki variable-step nie zostanie zakwalifikowany.
- Jeśli potrzebny variable-step, wyprowadzić współczynniki z interpolacji Lagrange’a dla rzeczywistych czasów historii.

### Etap C — integracja i migracja

- Checkpoint musi przechowywać czasy/RHS history i schema version.
- Przy zmianie fizyki, maski lub dynamicznego źródła unieważnić historię.
...
- Order test dla stałego kroku.
- Skok `dt` 2×: fixed-step gate resetuje startup i nie używa starej historii.
- Po implementacji variable-step: convergence dla nieregularnej sekwencji kroków.
- Restart z historią daje identyczny kolejny krok.
- Dynamic source discontinuity invalidates history.
...
- Variable-step ABM jest wrażliwy na gwałtowne zmiany kroku i może wymagać limiterów.

## 11. Poza zakresem

- Adaptacyjny ABM z własnym embedded estimatorem w pierwszej iteracji.

## 12. Definition of Done

Ustalenie `FDM-CPU-NUM-003` jest zamknięte dopiero wtedy, gdy:
