# FDM-CPU-PHY-001 — Kanoniczny naturalny warunek brzegowy DMI dla masek i obiektów
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-CPU-PHY-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Brakujący sąsiad przy granicy aktywnej maski jest zastępowany spinem centralnym. To odpowiada zerowej pochodnej zwykłej, lecz nie ogólnemu naturalnemu BC wynikającemu z wariacji energii DMI. Semantyka różni się od słabej postaci FEM i części korekt CUDA.

### Skutek

Zła amplituda lub znak boundary twist, zależność wyników od sposobu wokselizacji i rozbieżność FDM–FEM.

## 3. Docelowy kontrakt

Jeden udokumentowany kontrakt BC dla interfacial i bulk DMI na zewnętrznej granicy, granicy wewnętrznej maski, interfejsie materiałowym i PBC; każda implementacja musi przejść ten sam oracle.
...
|---|---|
| `crates/fullmag-engine/src/fdm/cpu/fields.rs` | operator fizyczny, pole lub energia |
...
| `docs/physics/fem_dmi.md` | modyfikacja lub weryfikacja kontraktu |
| `docs/physics` | modyfikacja lub weryfikacja kontraktu |
...
- Wyprowadzić BC z przyjętego funkcjonału energii i konwencji normalnej.
- Wprowadzić jawny typ boundary realization zamiast ukrytego central-spin fallbacku.
- Dla mask boundary zbudować link table z normalną/face fraction lub odrzucić niekwalifikowaną geometrię.

### Etap C — integracja i migracja

- Rozdzielić interfacial i bulk DMI oraz warunki na interfejsie dwóch wartości D.
- Zapewnić zgodność energii i pola przez directional derivative test.
- Dodać tę samą fixture do FDM GPU i FEM.
...
- Analityczny 1D boundary twist dla obu znaków D.
- Directional derivative `dE/dε` versus `-μ0 Ms H·δm`.
- Mask boundary versus równoważna prostokątna granica.
- PBC bez sztucznego boundary twist.
- FDM CPU/GPU i FEM CPU/GPU parity po refinement.
...
- boundary-angle error
- energy derivative residual
- CPU/GPU/FEM discrepancy
...
- BC zależy od dokładnie przyjętej definicji energii DMI.
- Sub-cell normalne mogą zwiększyć koszt pamięci.

## 11. Poza zakresem

- Pełna rekonstrukcja krzywizny dowolnej powierzchni w pierwszej wersji.

## 12. Definition of Done

Ustalenie `FDM-CPU-PHY-001` jest zamknięte dopiero wtedy, gdy:
