# FDM-CPU-TRX-001 — Atomowy commit termicznego RNG i stanu integratora
...
| Zależności | `FDM-CPU-NUM-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-CPU-TRX-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Licznik realizacji termicznej może zostać przesunięty po wywołaniu integratora także wtedy, gdy krok kończy się błędem. Stan magnetyzacji i czas pozostają wtedy niezmienione, ale kolejna próba używa innego szumu. Podobnego rollbacku wymagają FSAL, historia ABM i cache pól.

### Skutek

Nieudana operacja zmienia późniejszy wynik naukowy, restart nie jest deterministyczny, a retry nie reprezentuje tego samego przyrostu Wienera.

## 3. Docelowy kontrakt

Semantyka transakcyjna: skutki uboczne są modyfikowane wyłącznie w `commit_accepted_step`; każdy błąd lub reject pozostawia serializowany solver state bitowo równy stanowi wejściowemu, z wyjątkiem jawnej telemetryki próby.
...
|---|---|
| `crates/fullmag-engine/src/fdm/shared/problem.rs` | modyfikacja lub weryfikacja kontraktu |
...
- Wprowadzić `StepTransaction` przechowujący tylko stan konieczny do commit/rollback: czas, `dt`, thermal interval index, FSAL validity i indeks historii.
- Przenieść `advance_thermal_step()` do jednej gałęzi po `Ok(accepted_report)`.
- Dla adaptive retry zachować ten sam surowy draw normalny i przeskalować amplitudę zgodnie z nowym `dt`; nie losować nowego incrementu.

### Etap C — integracja i migracja

- FSAL i historia ABM muszą być aktualizowane dopiero po accepted endpoint RHS.
- Output counters, autosave revision i progress sequence aktualizować po commit, nie po próbie.
- Dodać serializowalny digest stanu transakcyjnego do testów fault-injection.
...
- Fault injection przed polem, po pierwszym RHS, po normalizacji i przed obserwablami.
- Porównanie SHA-256 checkpointu przed i po nieudanym kroku.
- Retry termiczny: ten sam seed i interval index dają ten sam `xi`, niezależnie od liczby rejectów.
- Restart po rejected attempt daje tę samą dalszą trajektorię co przebieg bez błędu.
- Dokładnie jeden increment RNG na accepted step.
...
- Błędne współdzielenie bufora losowego między równoległymi sesjami.
- Niezgodność starych checkpointów bez migracji schema.

## 11. Poza zakresem

- Zmiana modelu stochastycznego LLG.
- Gwarancja bitwise parity między różnymi bibliotekami libm.

## 12. Definition of Done

Ustalenie `FDM-CPU-TRX-001` jest zamknięte dopiero wtedy, gdy:
