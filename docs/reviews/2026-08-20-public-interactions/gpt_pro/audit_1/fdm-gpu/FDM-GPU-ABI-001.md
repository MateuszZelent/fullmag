# FDM-GPU-ABI-001 — Usunięcie driftu `fullmag_fdm_plan_desc` i obowiązkowy build feature CUDA
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-GPU-ABI-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Nagłówek C i Rust FFI zawierają rozszerzony descriptor (`ms_field`, `a_field`, `alpha_field`, pola DMI, formuły STT, maski i rozszerzenia frozen spins), natomiast konstruktor runnera tworzy literal tego typu bez kompletnego, jawnego mapowania wszystkich pól. Domyślny CI nie musi budować feature `cuda`, więc drift ABI może pozostać niewykryty.

### Skutek

Utrzymywana ścieżka może nie kompilować się po włączeniu CUDA albo, po częściowym wyrównaniu, przenosić niezamierzone wartości zerowe i wykonywać inną fizykę niż plan.

## 3. Docelowy kontrakt

Descriptor ma jednego właściciela schema, generowane lub automatycznie sprawdzane bindingi i kompletną funkcję konwersji z IR. Każda zmiana C ABI łamie CI, dopóki Rust i testy kontraktowe nie zostaną zaktualizowane.
...
| `crates/fullmag-fdm-sys/src/lib.rs` | modyfikacja lub weryfikacja kontraktu |
...
| `backends/fdm/tests/region_owned_abi_contract.cpp` | testy, kwalifikacja lub automatyzacja |
...
- Nadać descriptorowi jawny `abi_version` i `struct_size`; dla append-only rozszerzeń stosować osobne wersjonowane struktury.
- Zaimplementować `TryFrom<&FdmPlanIR> for OwnedNativePlanDesc`, który zero-inicjalizuje całość, posiada backing storage i mapuje każde pole w jednym miejscu.
- Dodać compile-time `static_assert(sizeof/offsetof)` po stronie C++ oraz Rust layout tests dla każdego pola.
- Wygenerować bindings z nagłówka lub generować oba artefakty z jednego schema; ręczne duplikowanie pozostawić tylko z testem hash.

### Etap C — integracja i migracja

- W CI uruchamiać `cargo check -p fullmag-runner --features cuda` i natywny build z CUDA stub/toolkit.
- Dodać test round-trip descriptoru z niezerową wartością każdego append-only pola.
- Fail-closed dla nieznanej wersji/rozmiaru; nie interpretować częściowego descriptoru.
...
- C/Rust size, alignment i offset parity.
- Feature-build CI dla debug/release i dwóch konfiguracji precision.
- Descriptor sentinel test ustawiający unikalne wartości w każdym polu.
- Negative test niezgodnego `abi_version`/`struct_size`.
- Planner→runner→C API test sprawdzający exact resolved values.
...
- Zmiana ABI wymaga kompatybilności z istniejącymi binariami.
- Automatyczne bindings mogą wymagać dodatkowego toolchainu w release.

## 11. Poza zakresem

- Przeprojektowanie wszystkich publicznych ABI FullMag naraz.

## 12. Definition of Done

Ustalenie `FDM-GPU-ABI-001` jest zamknięte dopiero wtedy, gdy:
