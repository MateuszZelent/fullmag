# FDM-GPU-QUAL-001 — Sprzętowe CI, Compute Sanitizer i time-to-accuracy gate
...
| Zależności | `FDM-GPU-ABI-001`, `FDM-GPU-ARCH-001`, `FDM-GPU-PERF-001`, `FDM-GPU-TRX-001`, `FDM-GPU-PHY-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-GPU-QUAL-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Domyślny build może omijać feature CUDA, a statyczna obecność kerneli nie potwierdza kompilacji, rezydencji ani przyspieszenia na docelowym urządzeniu.

### Skutek

Regresje ABI, wycieki, silent fallback i synchronizacje trafiają do mastera bez dowodu. Steps/s może premiować mniej dokładną precision/integrator policy.

## 3. Docelowy kontrakt

Warstwowy CI: feature compile bez GPU, natywne testy na GPU, sanitizer lane i scheduled performance qualification z exact hardware identity.
...
| `justfile` | testy, kwalifikacja lub automatyzacja |
| `backends/fdm/tests` | testy, kwalifikacja lub automatyzacja |
| `crates/fullmag-runner/src/fdm/gpu/cuda/native/tests.rs` | testy, kwalifikacja lub automatyzacja |
...
- Dodać obowiązkowy `cargo check --features cuda` oraz build CMake dla wspieranych architektur.
- Hardware CI uruchamia macrospin, field/RHS/stage/trajectory parity, adaptive reject i thermal restart.
- Compute Sanitizer memcheck/racecheck/initcheck dla małych fixture.

### Etap C — integracja i migracja

- Performance lane zbiera Nsight Systems counters, transfer audit i time-to-accuracy.
- Fail job, jeżeli executed receipts nie są strict device-resident.
- Przechowywać baseline per GPU model/driver/toolkit z kontrolowanym progiem regresji.
...
- Repeated create/step/snapshot/destroy.
- OOM/preflight and unsupported capability.
- FP32/FP64 qualification matrix.
- No-allocation/no-full-readback contract.
- Long-run leak and deterministic restart.
...
- Sprzętowy CI jest kosztowny i wymaga stabilnej maszyny.
- Driver updates wymagają rebaseliningu.

## 11. Poza zakresem

- Gwarancja identycznej wydajności na wszystkich GPU.

## 12. Definition of Done

Ustalenie `FDM-GPU-QUAL-001` jest zamknięte dopiero wtedy, gdy:
