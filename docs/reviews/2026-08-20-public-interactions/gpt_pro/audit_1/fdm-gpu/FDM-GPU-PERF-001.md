# FDM-GPU-PERF-001 — Usunięcie hostowej synchronizacji z decyzji adaptacyjnej
...
| Zależności | `FDM-GPU-ARCH-001`, `FDM-CPU-NUM-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-GPU-PERF-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Adaptive RK redukuje błąd na urządzeniu, ale host odczytuje skalary i synchronizuje strumień, aby podjąć decyzję accept/reject oraz wyliczyć `dt_next`.

### Skutek

Co najmniej jedna bariera CPU–GPU na próbę; dla małych/średnich siatek latency może dominować nad kernelami LLG i ograniczać możliwość CUDA Graph capture.

## 3. Docelowy kontrakt

Redukcja, decyzja PI, aktualizacja `dt`, retry counter i wybór ścieżki są urządzeniowe. Host otrzymuje batched telemetry po accepted step lub zgodnie z output stride.
...
- Przechowywać `error`, `previous_error`, `dt`, decyzję i licznik retry w małej strukturze device control.
- Dodać kernel kontrolera PI zgodny bit-for-bit z kanoniczną polityką CPU w granicach tolerancji.
- Użyć device-side conditional launch przez graph conditional nodes, persistent loop lub ograniczony host launch bez D2H.

### Etap C — integracja i migracja

- Odczytywać jeden pakiet diagnostyczny po accepted step; w trybie stats none nie synchronizować hosta.
- Zachować typed `dt_min_exhausted` i interrupt polling przez oddzielny rzadki kanał kontrolny.
- Księgować każdy control-scalar D2H i synchronizację w telemetryce.
...
- Golden vectors CPU/device dla kontrolera PI.
- Wymuszony reject pokazujący zero control-scalar D2H w hot loop.
- Attempt trace parity CPU/GPU.
- Test `dt_min_exhausted`, NaN i max_reject.
- Benchmark małej siatki mierzący latency przed/po.
...
- Persistent device loop utrudnia szybkie przerwanie przez użytkownika.
- Różnice `pow` CPU/GPU mogą minimalnie zmienić `dt_next`.

## 11. Poza zakresem

- Przeniesienie output writera na GPU.

## 12. Definition of Done

Ustalenie `FDM-GPU-PERF-001` jest zamknięte dopiero wtedy, gdy:
