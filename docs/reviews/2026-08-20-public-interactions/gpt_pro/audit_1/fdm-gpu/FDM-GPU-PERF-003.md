# FDM-GPU-PERF-003 — Fuzja kerneli lokalnych i ograniczenie launch overhead
...
| Zależności | `FDM-GPU-PERF-002`, `FDM-GPU-PERF-004` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FDM-GPU-PERF-003`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Składanie H_eff, lokalne interakcje, LLG RHS, normalizacja i redukcje mogą być realizowane jako wiele krótkich kerneli na każdy stage. Dla mniejszych siatek launch latency i ruch pamięci dominują.

### Skutek

Niska occupancy, wielokrotne odczyty/zapisy tych samych wektorów i słaby break-even wobec CPU.

## 3. Docelowy kontrakt

Planner wybiera wyspecjalizowany pipeline dla aktywnego zestawu interakcji. Lokalne pola i RHS są fuzowane tam, gdzie nie narusza to reuse demag/exchange i kwalifikacji fizycznej.
...
|---|---|
| `backends/fdm/gpu/cuda/integrators/llg_fp32.cu` | integracja czasu i stan próby |
| `backends/fdm/gpu/cuda/integrators/llg_fp64.cu` | integracja czasu i stan próby |
| `backends/fdm/gpu/cuda/interactions` | operator fizyczny, pole lub energia |
| `backends/fdm/include/kernels.hpp` | ABI, stan runtime lub deklaracje |
...
- Zebrać timeline kernel-by-kernel i zidentyfikować memory-bound sekwencje.
- Fuzować anisotropy, Zeeman, Oersted envelope, magnetoelastic i thermal addition z H_eff/RHS, gdy dane są dostępne.
- Połączyć normalizację z stage update i block reductions.

### Etap C — integracja i migracja

- Generować ograniczoną liczbę wariantów przez feature mask/template, unikając runtime branch explosion.
- Dla stałej topologii capture CUDA Graph; update parametrów `dt/time` przez graph parameters.
- Zachować osobne oracles dla każdego wariantu fuzji.
...
- Kernel count regression test dla typowych konfiguracji.
- Parity fused/unfused field, RHS i trajectory.
- Nsight Compute/System benchmark małej, średniej i dużej siatki.
- Graph recapture po zmianie physics revision.
- Test occupancy/register pressure i brak spill regresji.
...
- Nadmierna fuzja zwiększy register pressure.
- Liczba wariantów binarnych może eksplodować.

## 11. Poza zakresem

- Fuzja kosztownego demag FFT w jeden kernel.

## 12. Definition of Done

Ustalenie `FDM-GPU-PERF-003` jest zamknięte dopiero wtedy, gdy:
