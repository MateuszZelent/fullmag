# FEM-GPU-PERF-002 — Trwałe device assembly, restrictions, quadrature i preconditionery
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-002`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Repeated RK apply nie może odbudowywać CSR, restrictions, mesh geometry, quadrature data, essential DOF, Hypre workspace ani event pools. Ich lifecycle musi być revision-driven.

### Skutek

Wysoki setup per step, fragmentacja VRAM i brak graph reuse.

## 3. Docelowy kontrakt

Kompletny FemGpuState jest budowany raz na dependency key. Hot loop wykonuje wyłącznie kernels/solver apply; każda rebuild ma reason receipt.
...
| `backends/fem/gpu/cuda/demag_poisson` | modyfikacja lub weryfikacja kontraktu |
| `backends/fem/gpu/cuda/runtime/gpu_state_runtime.cpp` | modyfikacja lub weryfikacja kontraktu |
...
- Zdefiniować dependency key mesh/order/material/BC/PBC/precision/device.
- Preallocate fields, RK buffers, reductions, CUB temp storage i transaction buffers.
- Upload geometry/material coefficients tylko po revision change.

### Etap C — integracja i migracja

- Prepare Hypre solver/preconditioner i CUDA events przed hot loop.
- Wprowadzić allocation/assembly audit counters i hard gate.
- Atomowo przełączać nowy state po udanym rebuild.
...
- Zero cudaMalloc/operator build po warm-up.
- Invalidation matrix.
- OOM fail-before-step.
- Repeated create/destroy under sanitizer.
- Preconditioner reuse count.
...
- Błędny dependency key daje stale device data.
- Duży preallocation zwiększa minimalny VRAM.

## 11. Poza zakresem

- Dynamiczna zmiana mesh w środku pojedynczej próby.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-002` jest zamknięte dopiero wtedy, gdy:
