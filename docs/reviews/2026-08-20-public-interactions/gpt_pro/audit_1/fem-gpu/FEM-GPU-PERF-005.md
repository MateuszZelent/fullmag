# FEM-GPU-PERF-005 — Jawna lokalizacja i lifecycle preconditionera
...
| Zależności | `FEM-GPU-ARCH-001`, `FEM-GPU-PERF-002` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-005`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
GPU operator z hostowym preconditionerem/sparse solve jest hybrydą. Sam Hypre execution policy nie wystarcza bez receipt faktycznego memory location, setup i apply.

### Skutek

Niejawne transfery, mylący status GPU i nieprzewidywalne iteracje.

## 3. Docelowy kontrakt

Preconditioner descriptor zawiera type, location, precision, setup revision i reuse count. Strict GPU wymaga device apply.
...
|---|---|
| `backends/fem/gpu/cuda/demag_poisson/hypre_device_solver.cpp` | modyfikacja lub weryfikacja kontraktu |
...
- Rozdzielić solver i preconditioner receipts.
- Fail strict preflight, jeśli preconditioner nie ma device capability.
- Cache setup po operator dependency key.

### Etap C — integracja i migracja

- Dodać degradation policy na podstawie iteration growth, ale rebuild poza aktywnym stage.
- Raportować iterations/residual/setup/apply timing.
...
- Device location receipt.
- No-host-transfer apply.
- Rebuild on operator change only.
- Iteration degradation trigger.
- CPU/GPU tolerance parity.
...
- Nie wszystkie preconditionery Hypre są dostępne/efektywne na każdym GPU.

## 11. Poza zakresem

- Jeden preconditioner dla wszystkich realizacji demag.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-005` jest zamknięte dopiero wtedy, gdy:
