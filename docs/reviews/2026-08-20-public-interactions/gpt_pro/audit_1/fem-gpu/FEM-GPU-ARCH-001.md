# FEM-GPU-ARCH-001 — Executed-device proof i ścisła klasyfikacja FEM GPU
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-ARCH-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Plan GPU może reprezentować strict device Poisson, hybrid CPU Poisson albo wybrane kernelle CUDA. Bez per-operator receipts nazwa `FEM GPU` nie rozróżnia pełnego kroku urządzeniowego od operator acceleration.

### Skutek

Silent fallback/hybrid execution, błędne benchmarki i brak odtwarzalności faktycznej ścieżki.

## 3. Docelowy kontrakt

Resolved class: `device_resident`, `gpu_operator_host_solver`, `hybrid_cpu_poisson` lub `cpu`. Tryb strict dopuszcza wyłącznie pierwszy; receipts obejmują exchange, demag, local fields, LLG, RK, redukcje i preconditioner.
...
|---|---|
| `backends/fem/gpu/cuda/integrators/rk/rk_plan.cpp` | planowanie, walidacja i transport konfiguracji |
| `backends/fem/gpu/cuda/integrators/rk/rk_step_preflight.cu` | integracja czasu i stan próby |
| `crates/fullmag-plan/src/fem.rs` | planowanie, walidacja i transport konfiguracji |
| `crates/fullmag-runner/src/fem/engine.rs` | orkiestracja, provenance i publikacja |
| `native/include/fullmag_fem.h` | ABI, stan runtime lub deklaracje |
...
- Rozszerzyć capability/receipt ABI o execution class i operator locations.
- Preflight porównuje requested policy z faktycznym planem i fail-closed przed krokiem.
- Hybrid mode musi być nazwany jawnie w provenance i UI/CLI.

### Etap C — integracja i migracja

- Strict test blokuje hostowe operator apply i hostowe preconditioner path.
- Publikować device, driver, toolkit, MFEM/Hypre execution policy i source-of-truth.
...
- Strict no-fallback.
- Receipt matrix dla wszystkich składników.
- Negative test braku device Hypre/preconditionera.
- Hybrid classification test.
- Hardware executed-device smoke.
...
- Część dotychczasowych planów zostanie przeklasyfikowana jako hybrid/experimental.

## 11. Poza zakresem

- Usunięcie jawnie wybranego hybrid debug mode.

## 12. Definition of Done

Ustalenie `FEM-GPU-ARCH-001` jest zamknięte dopiero wtedy, gdy:
