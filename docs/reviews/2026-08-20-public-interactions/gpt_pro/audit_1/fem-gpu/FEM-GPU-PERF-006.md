# FEM-GPU-PERF-006 — Device-side decyzja adaptacyjna bez `cudaStreamSynchronize`
...
| Zależności | `FEM-GPU-PERF-004`, `FEM-CPU-NUM-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-006`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
`rk_adaptive_decision_readback` kopiuje trzy skalary i `rk_scalar_readback` synchronizuje stream na każdą próbę.

### Skutek

Host fence per adaptive attempt i brak pełnego graph/persistent execution.

## 3. Docelowy kontrakt

Device controller PI i control record; host odczytuje attempt trace po accepted step lub diagnostycznie.
...
| `backends/fem/gpu/cuda/integrators/rk/rk_attempt_loop.cu` | integracja czasu i stan próby |
| `backends/fem/gpu/cuda/integrators/rk/rk_adaptive_runtime.cu` | integracja czasu i stan próby |
...
- Dodać device PI controller zgodny z CPU.
- Przechowywać decision/reason/dt_next w device record.
- Warunkować retry bez D2H; bounded max_reject na device.

### Etap C — integracja i migracja

- Batchować trace.
- Zachować rzadki interrupt channel.
...
- Golden decision parity.
- Zero control sync/reject.
- dt_min/NaN/max_reject.
- Attempt trace parity.
- Latency benchmark.
...
- Graph conditional support zależy od wersji CUDA.
- Persistent loop komplikuje watchdog/interrupt.

## 11. Poza zakresem

- Brak hostowej kontroli lifecycle.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-006` jest zamknięte dopiero wtedy, gdy:
