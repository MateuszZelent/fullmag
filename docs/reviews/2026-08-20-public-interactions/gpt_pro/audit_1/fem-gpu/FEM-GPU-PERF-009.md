# FEM-GPU-PERF-009 — Eliminacja hybrid CPU Poisson round-trip per stage
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-009`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Tryb `FULLMAG_FEM_GPU_DEMAG_HYBRID_CPU_POISSON` pobiera stage magnetization do host AoS, liczy demag CPU i wysyła H_demag na GPU dla każdej oceny RHS.

### Skutek

Pełny O(N) D2H+H2D oraz host solve per stage; taka ścieżka nie jest produkcyjnym GPU LLG i może być dramatycznie wolna.

## 3. Docelowy kontrakt

Strict GPU używa device Hypre Poisson lub innej device demag. Hybrid jest jawny compatibility/debug mode, odrzucany przez strict i benchmarkowany osobno.
...
|---|---|
| `backends/fem/gpu/cuda/integrators/rk/rk_demag_dispatch.cu` | integracja czasu i stan próby |
...
- Zmienić nazwę/capability hybrid lane i usunąć ją z strict selection.
- Dokończyć device Poisson RHS/solve/recovery/energy dla wszystkich wspieranych BC/PBC.
- Preflight wymaga ready device workspace.

### Etap C — integracja i migracja

- Transfer audit fail przy full stage field roundtrip.
- Hybrid output ma ostrzeżenie i exact byte counters.
- Dodać break-even, ale nie promować jako GPU.
...
- Strict rejects hybrid.
- Device Poisson field/energy parity.
- Zero full-vector transfers strict.
...
- Device Hypre wymaga konkretnego MFEM/MPI/Hypre build.
- Nie wszystkie BC mogą być od razu wspierane.

## 11. Poza zakresem

- Usunięcie CPU Poisson jako reference oracle.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-009` jest zamknięte dopiero wtedy, gdy:
