# FEM-GPU-QUAL-001 — Sprzętowy CI, sanitizer, leak i time-to-accuracy qualification
...
| Zależności | `FEM-GPU-ARCH-001`, `FEM-GPU-PERF-001`, `FEM-GPU-PERF-002`, `FEM-GPU-PERF-004`, `FEM-GPU-PERF-009`, `FEM-GPU-PHY-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-QUAL-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Bez rzeczywistego GPU nie można potwierdzić kompilacji wszystkich lanes, executed-device, stream interop, Hypre device policy, wycieków i przyspieszenia.

### Skutek

P0/P1 regresje mogą pozostać niewidoczne; static tests nie wykrywają transferów ani timeline.

## 3. Docelowy kontrakt

Obowiązkowe compile gates oraz hardware qualification dla strict device lane; scheduled performance/long-run.
...
- Build MFEM+MPI+Hypre GPU+CUDA matrix z exact versions.
- Run operator/RHS/stage/trajectory parity and no-fallback.
- Compute Sanitizer i repeated session leak tests.

### Etap C — integracja i migracja

- Nsight timeline/transfer audit artifact.
- Performance baseline per GPU/device stack i time-to-accuracy.
- Promotion registry rozróżnia source-present, public-executable i physics-qualified.
...
- Strict device Poisson.
- Adaptive rejects and transaction faults.
- Thermal restart.
- PBC/Airbox/material fixtures.
- Long-run VRAM leak.
...
- Koszt utrzymania GPU runnera i wrażliwość na driver/toolkit updates.

## 11. Poza zakresem

- Benchmark na każdym modelu GPU w każdym PR.

## 12. Definition of Done

Ustalenie `FEM-GPU-QUAL-001` jest zamknięte dopiero wtedy, gdy:
