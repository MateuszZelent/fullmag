# FEM-GPU-NUM-003 — Zgodność adaptive error z projected high-order candidate
...
| Zależności | `FEM-CPU-NUM-001`, `FEM-GPU-PERF-006` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-NUM-003`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
GPU capture pre-normalization candidate do `m_stage`, następnie normalizuje accepted state i może obliczać endpoint RHS. Norm defect/rotation oraz embedded combination muszą jednoznacznie odnosić się do kandydata przed lub po projekcji.

### Skutek

Błędna decyzja accept/reject i rozbieżność CPU/GPU, szczególnie dla dużego defect normy.

## 3. Docelowy kontrakt

Wersjonowana projection/error policy identyczna z FEM CPU. Error kernel otrzymuje dokładnie zdefiniowany high-order candidate i osobno mierzy projection correction.
...
| `backends/fem/gpu/cuda/integrators/rk/rk_attempt_setup.cu` | integracja czasu i stan próby |
| `backends/fem/gpu/cuda/integrators/rk/adaptive_error_kernels.cu` | integracja czasu i stan próby |
...
- Nazwać bufory `candidate_unprojected` i `candidate_projected`.
- Zdefiniować estimator target w solver policy.
- Error kernel używa właściwego bufora; projection norm/angle jako guards.

### Etap C — integracja i migracja

- CPU implementacja stosuje tę samą kolejność.
- Provenance zapisuje policy version.
...
- Synthetic large norm defect.
- CPU/GPU error norm parity.
- Temporal order.
- Projection guard threshold.
- Frozen/PBC candidate handling.
...
- Zmiana polityki zmieni adaptive trajectories.
- Post-projection estimator może wymagać ponownej kalibracji order.

## 11. Poza zakresem

- Dowód geometrycznego RK.

## 12. Definition of Done

Ustalenie `FEM-GPU-NUM-003` jest zamknięte dopiero wtedy, gdy:
