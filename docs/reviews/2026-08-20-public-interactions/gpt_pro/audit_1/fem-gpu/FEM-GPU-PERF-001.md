# FEM-GPU-PERF-001 — Usunięcie hostowego integratora i hostowego Krylov z hot loop
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Pełny device lane wymaga, aby stage state, effective fields, LLG algebra, Krylov/preconditioner i decyzje pozostawały na GPU. Każde mapowanie MFEM Vector/GridFunction na host w stage/iteration niszczy rezydencję.

### Skutek

PCIe/NVLink latency i synchronizacje mogą uczynić GPU wolniejszym od FEM CPU, szczególnie dla mniejszych DOF.

## 3. Docelowy kontrakt

Device jest source-of-truth przez cały accepted step. Host uruchamia jedynie coarse orchestration i odczytuje batched scalars/output zgodnie z harmonogramem.
...
| `backends/fem/gpu/cuda/integrators/rk/rk_rhs_runtime.cu` | integracja czasu i stan próby |
...
| `backends/fem/cpu/mfem/runtime/mfem_host_access.hpp` | ABI, stan runtime lub deklaracje |
...
- Utrzymywać device component fields dla wszystkich etapów i operatorów.
- Zakazać audited host read/write w `TransferAuditScope::HotLoop` z wyjątkiem zatwierdzonych control scalars.
- Zapewnić Hypre device execution i device preconditioner; host API call nie może kopiować danych.

### Etap C — integracja i migracja

- Przenieść accept/reject i stop control na device lub ograniczyć do jednej batched granicy accepted step.
- Dodać timeline receipts oraz fail przy pełnym field roundtrip.
...
- Transfer audit zero full-vector H2D/D2H.
- Device source-of-truth przez cały step.
- CPU-disabled strict run.
- Nsight timeline bez host gaps per stage.
- Small/medium/large break-even.
...
- Niektóre Hypre wywołania host API nadal sterują device kernels; trzeba mierzyć, nie zakładać.

## 11. Poza zakresem

- Całkowite usunięcie hosta z lifecycle sesji.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-001` jest zamknięte dopiero wtedy, gdy:
