# FEM-GPU-NUM-001 — Mixed precision i tolerancje solverów jako time-to-accuracy policy
...
| Zależności | `FEM-GPU-PERF-004`, `FEM-GPU-PERF-005` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-NUM-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Niższa precyzja może zmienić redukcje, iteracje Krylov i accept/reject. Precyzja storage, operator, solver i akumulacji musi być jawna.

### Skutek

Pozorne przyspieszenie pojedynczego kroku przy gorszej trajektorii lub większej liczbie iteracji/kroków.

## 3. Docelowy kontrakt

Precision policy jest wersjonowana i kwalifikowana per workload; krytyczne normy/energie mogą akumulować w FP64.
...
| `backends/fem/gpu/cuda/integrators` | integracja czasu i stan próby |
...
- Rozdzielić state/operator/solver/reduction precision.
- Dopasować linear tolerances do temporal error.
- FP32/mixed capability tylko po field/RHS/trajectory qualification.

### Etap C — integracja i migracja

- Deterministic reduction policy i tolerancje publikować.
- Planner uwzględnia iteration inflation w wyborze.
...
- Precision sweep time-to-accuracy.
- Adaptive decision parity.
- Long-run energy/norm drift.
- Krylov iteration sweep.
...
- Mixed precision może wymagać iterative refinement.
- Różne GPU mają różne FP64 ratios.

## 11. Poza zakresem

- Bitwise parity różnych precisions.

## 12. Definition of Done

Ustalenie `FEM-GPU-NUM-001` jest zamknięte dopiero wtedy, gdy:
