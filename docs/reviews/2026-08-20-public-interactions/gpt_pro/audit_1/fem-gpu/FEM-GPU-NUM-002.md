# FEM-GPU-NUM-002 — Device tangent-plane/IMEX dla sztywnych siatek
...
| Zależności | `FEM-CPU-NUM-002`, `FEM-GPU-PERF-003`, `FEM-GPU-PERF-005` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-NUM-002`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
GPU przyspiesza RHS, ale explicit RK nadal ma `dt ~ h_min^2`. Lokalny refinement może uczynić trajectory niepraktyczną mimo wysokiego throughput.

### Skutek

Duża liczba etapów i Poisson solves; brak time-to-solution przewagi.

## 3. Docelowy kontrakt

Planner używa stiffness estimate i oferuje zakwalifikowaną device-resident metodę tangent-plane/semi-implicit/IMEX.
...
| `backends/fem/gpu/cuda/relaxation` | modyfikacja lub weryfikacja kontraktu |
...
- Przenieść stiffness estimate z FEM CPU do wspólnego planu.
- Zdefiniować device linearized operator i preconditioner reuse.
- Zachować norm constraint przez tangent-space solve/retraction.

### Etap C — integracja i migracja

- Porównać liczbę demag solves i linear iterations.
- Fail/warn dla explicit plan poza kwalifikowanym zakresem.
...
- Explicit/implicit parity.
- Temporal order.
- Time-to-accuracy.
- Large locally refined mesh.
...
- Implicit device implementation jest znaczącym projektem.
- Preconditioner może ograniczyć zysk.

## 11. Poza zakresem

- Natychmiastowe wycofanie RK4/RK45.

## 12. Definition of Done

Ustalenie `FEM-GPU-NUM-002` jest zamknięte dopiero wtedy, gdy:
