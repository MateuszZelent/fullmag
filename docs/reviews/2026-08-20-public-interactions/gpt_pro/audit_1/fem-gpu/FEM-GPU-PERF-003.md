# FEM-GPU-PERF-003 — Planner matrix-free/partial assembly zamiast wymuszonego legacy sparse
...
| Zależności | `FEM-GPU-PERF-002`, `FEM-CPU-PERF-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-003`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
GPU RK preflight wymaga trybu `legacy_sparse_gpu` dla exchange. Dla repeated high-order FEM apply matrix-free/partial assembly może mieć mniejszy footprint i lepszą przepustowość.

### Skutek

Nadmierny CSR memory traffic, ograniczenie rozmiaru problemu i brak optymalnej ścieżki dla wyższego order.

## 3. Docelowy kontrakt

Planner wybiera operator representation na podstawie order, DOF, device memory, liczby stage i benchmarków. Każda reprezentacja ma parity i performance receipts.
...
| `backends/fem/gpu/cuda/exchange/exchange_plan.cpp` | planowanie, walidacja i transport konfiguracji |
| `backends/fem/gpu/cuda/exchange` | modyfikacja lub weryfikacja kontraktu |
...
- Dodać enum operator mode: legacy sparse, partial assembly, matrix-free.
- Zaimplementować device kernels/restrictions zgodne z MFEM quadrature data.
- Preflight akceptuje qualified modes, nie jeden hardcoded string.

### Etap C — integracja i migracja

- Memory admission model uwzględnia każdy mode.
- Autotuning tylko w setup i z reproducible selection receipt.
- Fallback mode jest jawny, nigdy cichy.
...
- Operator apply parity dla mesh/order/material fields.
- Energy derivative parity.
- Memory footprint i throughput sweep.
- Planner selection golden cases.
- Repeated apply no-allocation.
...
- Partial assembly dla niektórych form/wersji MFEM może mieć ograniczenia.
- Wiele modes zwiększa koszt utrzymania.

## 11. Poza zakresem

- Usunięcie legacy sparse jako oracle/fallback.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-003` jest zamknięte dopiero wtedy, gdy:
