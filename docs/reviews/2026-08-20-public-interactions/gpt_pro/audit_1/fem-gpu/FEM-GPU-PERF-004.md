# FEM-GPU-PERF-004 — Device reductions i stop criteria bez synchronizacji per stage
...
| Zależności | `FEM-GPU-PERF-001` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-004`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Normy, torque, energy i adaptive metrics są redukowane na GPU, lecz ich odczyt może synchronizować strumień. Stop criteria nie powinny wymagać pełnego field readback ani wielu scalar fences.

### Skutek

Globalna bariera na każdy stage/próbę i ograniczona asynchroniczność.

## 3. Docelowy kontrakt

Wszystkie redukcje i decyzje control są device-side. Host otrzymuje jeden pakiet scalar telemetry po accepted step lub rzadziej; pełne pola tylko na snapshot.
...
|---|---|
| `backends/fem/gpu/cuda/reductions` | modyfikacja lub weryfikacja kontraktu |
| `backends/fem/gpu/cuda/integrators/rk/rk_scalar_readback.cu` | integracja czasu i stan próby |
| `backends/fem/gpu/cuda/integrators/rk/rk_step_stats.cu` | integracja czasu i stan próby |
| `backends/fem/gpu/cuda/integrators/rk/rk_adaptive_decision_readback.cu` | integracja czasu i stan próby |
...
- Zdefiniować device control record z error, torque, energy plateau i reason.
- Batchować scalars do pinned ring buffer z event readiness.
- Nie wywoływać `cudaStreamSynchronize` dla każdej pojedynczej wartości.

### Etap C — integracja i migracja

- Stop decision wykonywać na GPU lub na accepted-step boundary.
- W trybie stats-none pominąć final energy reductions, jeżeli nie są wymagane do stopu.
- Księgować sync reason.
...
- Zero scalar sync per non-output stage.
- Stop reason parity CPU/GPU.
- Adaptive retry without host fence.
- Pinned ring backpressure.
- Stats schedule operator counts.
...
- Opóźniona telemetryka nie może opóźnić krytycznego stopu.
- Asynchroniczny ring wymaga kontroli lifetime.

## 11. Poza zakresem

- Brak jakiegokolwiek scalar readbacku w całej sesji.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-004` jest zamknięte dopiero wtedy, gdy:
