# FEM-GPU-PERF-007 — Usunięcie podwójnego RK23 endpoint RHS i ogólna polityka FSAL
...
| Zależności | `FEM-GPU-NUM-003`, `FEM-GPU-PERF-009` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-GPU-PERF-007`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Adaptive BS23 liczy k3 w accepted normalized candidate dla embedded error, a final refresh ponownie oblicza RHS/H_eff dla tego samego endpointu; komentarz uznaje redundancję za benign.

### Skutek

Jedna dodatkowa pełna ewaluacja exchange+demag+local fields na każdą próbę/zaakceptowany krok.

## 3. Docelowy kontrakt

K3 staje się jednocześnie embedded endpoint RHS i FSAL/final cache, jeśli state/time/source revision są identyczne.
...
|---|---|
| `backends/fem/gpu/cuda/integrators/rk/rk23_adaptive_k3.cu` | integracja czasu i stan próby |
| `backends/fem/gpu/cuda/integrators/rk/rk_stage_schedule.cu` | integracja czasu i stan próby |
| `backends/fem/gpu/cuda/integrators/rk/rk_final_refresh.cu` | integracja czasu i stan próby |
| `backends/fem/gpu/cuda/integrators/rk/rk_fsal_policy.cpp` | integracja czasu i stan próby |
...
- Zapisać endpoint cache validity po k3.
- Final refresh reuse k3/H_eff, gdy accepted candidate nie został zmieniony po jego ocenie.
- Jeśli projekcja/frozen/source zmienia state, jawnie invalidować.

### Etap C — integracja i migracja

- Unieważniać FSAL dla termiki i incompatible dynamic sources.
- Liczyć rhs_evaluations bez podwójnego księgowania.
...
- RK23 operator-count expected.
- Endpoint field/RHS parity.
- Thermal FSAL disabled.
...
- K3 używa candidate przed/po projekcji; musi być exact identity.

## 11. Poza zakresem

- FSAL dla wszystkich integratorów.

## 12. Definition of Done

Ustalenie `FEM-GPU-PERF-007` jest zamknięte dopiero wtedy, gdy:
