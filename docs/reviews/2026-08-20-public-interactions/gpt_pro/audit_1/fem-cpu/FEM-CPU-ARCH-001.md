# FEM-CPU-ARCH-001 — Kanoniczna reprezentacja true/local/element/quadrature state
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-CPU-ARCH-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
FEM operuje między AoS host state, MFEM GridFunction/Vector, true DOF, periodic reduced nodes, element fields i quadrature/material coefficients. Niejawne konwersje i kopie utrudniają określenie źródła prawdy.

### Skutek

Nadmiarowy ruch pamięci, ryzyko podwójnej projekcji PBC, błędne uwzględnienie Airbox DOF oraz trudne parity CPU/GPU.

## 3. Docelowy kontrakt

Dla każdego typu danych istnieje jeden owner, jawna mapa i revision. LLG state obejmuje wyłącznie magnetyczne DOF; Airbox jest auxiliary field space. Adaptery nie zmieniają semantyki.
...
| `backends/fem/cpu/mfem/runtime/aos_field.cpp` | modyfikacja lub weryfikacja kontraktu |
| `backends/fem/cpu/mfem/runtime/state_io.cpp` | modyfikacja lub weryfikacja kontraktu |
| `backends/fem/core/fem_state.cpp` | modyfikacja lub weryfikacja kontraktu |
| `backends/fem/core/fem_material_fields.cpp` | operator fizyczny, pole lub energia |
...
- Udokumentować state model: authoritative `m`, local/true mappings, periodic representatives i auxiliary spaces.
- Wprowadzić typed views zamiast surowych `std::vector<double>` wszędzie, gdzie pomyłka space jest możliwa.
- Precompute gather/scatter/restriction maps i stosować je in-place.

### Etap C — integracja i migracja

- Airbox nodes muszą mieć zero RHS i być wykluczone z norm/stop/thermal.
- Material fields mają określoną lokalizację (element/quadrature/nodal) i kontrolowaną projekcję.
- Publikować representation receipts w debug provenance.
...
- Round-trip local↔true↔AoS.
- PBC representative consistency.
- Airbox exclusion oracle.
- Elementwise material discontinuity fixture.
- CPU/GPU representation parity.
...
- representation copies/stage
- gather/scatter bytes
- invalid-space assertions
...
- Typed wrappers wymagają szerokiej migracji API.
- Nodal projection może wygładzić ostre elementowe parametry.

## 11. Poza zakresem

- Jedna przestrzeń FE dla wszystkich pól fizycznych.

## 12. Definition of Done

Ustalenie `FEM-CPU-ARCH-001` jest zamknięte dopiero wtedy, gdy:
