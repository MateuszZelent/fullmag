# FEM-CPU-NUM-002 — Stiffness-aware wybór integratora dla `h_min` i exchange
...
| Zależności | `FEM-CPU-NUM-001`, `FEM-CPU-PERF-004` |

## 1. Cel dokumentu

Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-CPU-NUM-002`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Jawne RK zachowuje ograniczenie stabilności rosnące jak `h_min^2` dla exchange. Lokalny refinement i wyższy order mogą wymuszać bardzo mały krok niezależnie od kosztu pojedynczego RHS.

### Skutek

Miliony kroków, wysoki odsetek rejectów i brak korzyści z optymalizacji operatora.

## 3. Docelowy kontrakt

Planner estymuje stiffness i wybiera/zaleca explicit RK tylko w kwalifikowanym zakresie. Dla sztywnych problemów udostępnia tangent-plane, semi-implicit lub IMEX z poprawnym solver tolerance contract.
...
| `backends/fem/cpu/mfem/integrators` | integracja czasu i stan próby |
| `backends/fem/cpu/mfem/relaxation` | modyfikacja lub weryfikacja kontraktu |
...
- Obliczyć conservative exchange timescale z lokalnego h_min, A, Ms i gamma.
- Dodać planner warning/rejection lub integrator recommendation.
- Zbudować benchmark explicit versus semi-implicit/IMEX dla tego samego błędu.

### Etap C — integracja i migracja

- Dla implicit operatorów cache’ować linearization/preconditioner i kontrolować nonlinear/linear residual.
- Rozdzielić dynamikę precesyjną od pure-damping relaxation.
...
- Manufactured exchange mode temporal stability.
- Refinement study h_min versus accepted dt.
- Explicit/implicit trajectory parity w wspólnym zakresie.
- Time-to-accuracy benchmark.
- Fail/warn contract dla ekstremalnie sztywnej siatki.
...
- Estymator stiffness może być nadmiernie konserwatywny.
- Implicit method wymaga nowych preconditionerów.

## 11. Poza zakresem

- Zastąpienie wszystkich jawnych RK.

## 12. Definition of Done

Ustalenie `FEM-CPU-NUM-002` jest zamknięte dopiero wtedy, gdy:
