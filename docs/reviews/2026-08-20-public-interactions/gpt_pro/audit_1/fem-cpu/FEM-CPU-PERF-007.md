# FEM-CPU-PERF-007 — Jedna polityka wątków, NUMA i bibliotek solverowych
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-CPU-PERF-007`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
MFEM, OpenMP, Hypre/AMG, BLAS i zewnętrzny runner mogą niezależnie uruchamiać pule wątków. Bez jednego budżetu powstaje oversubscription i remote NUMA traffic.

### Skutek

Niestabilny wall-clock, spadek skalowania i duża wariancja benchmarków.

## 3. Docelowy kontrakt

Session-level `CpuExecutionPolicy` ustala affinity, liczbę wątków dla każdej biblioteki, first-touch allocation i strategię równoległości operator versus solver.
...
|---|---|
| `backends/fem/cpu/mfem/runtime/mfem_device.cpp` | modyfikacja lub weryfikacja kontraktu |
...
- Wykrywać physical cores/NUMA nodes i przyjmować jawny user override.
- Ustawić OMP/MKL/Hypre thread counts przed setup.
- Unikać nested parallel regions; dokumentować owner wątków per phase.

### Etap C — integracja i migracja

- First-touch duże wektory zgodnie z affinity.
- Publikować effective thread policy i affinity w provenance.
- Dodać scaling benchmark 1..N wątków.
...
- Nested parallelism disabled contract.
- NUMA placement smoke na wielosocketowym hoście.
- Scaling efficiency benchmark.
- Determinism/tolerance przy różnej liczbie wątków.
...
- Affinity zachowuje się różnie w kontenerach/SLURM.
- Hypre policy zależy od buildu.

## 11. Poza zakresem

- Ręczny tuning każdego modelu CPU bez automatycznego baseline.

## 12. Definition of Done

Ustalenie `FEM-CPU-PERF-007` jest zamknięte dopiero wtedy, gdy:
