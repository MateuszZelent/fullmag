# FEM-CPU-PERF-004 — Preconditioned, blokowy solve macierzy masy
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-CPU-PERF-004`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Consistent-mass projection może wykonywać CG bez jawnego preconditionera; wariant okresowy buduje hostowe reduced vectors i własny CG. Trzy komponenty pola są rozwiązywane osobno.

### Skutek

Wiele iteracji i hostowych przebiegów na każdy stage; koszt rośnie z order/mesh conditioning i może dominować exchange.

## 3. Docelowy kontrakt

Planner wybiera lumped mass jako szybki, zwalidowany default lub consistent mass z trwałym preconditionerem i blokowym solve dla x/y/z. PBC korzysta z precomputed reduced operator, nie składania hostowego per apply.
...
|---|---|
| `backends/fem/cpu/mfem/interactions/exchange_mass_projection.cpp` | operator fizyczny, pole lub energia |
...
- Zbudować preconditioner Jacobi/AMG/Cholesky odpowiedni dla SPD mass matrix i cache’ować go.
- Użyć block/multiple RHS solve albo wspólnego operator apply dla trzech komponentów.
- Precompute PBC restriction/prolongation i reduced mass operator.

### Etap C — integracja i migracja

- Tolerancję solve powiązać z błędem czasowym/polowym; nie rozwiązywać znacznie dokładniej niż wymagane.
- Raportować iterations i residual per stage.
- Zweryfikować energy derivative dla lumped i consistent realizacji.
...
- Manufactured projection z analitycznym wynikiem.
- Lumped/consistent field i energy convergence.
- Iteration-count regression dla regularnych i zniekształconych tetraedrów.
- PBC parity.
- Tolerance sweep versus trajectory error.
...
- Zbyt luźna tolerancja może zepsuć order LLG.
- Block solve może zwiększyć footprint.

## 11. Poza zakresem

- Jedna polityka masy dla wszystkich orderów i workloadów bez benchmarku.

## 12. Definition of Done

Ustalenie `FEM-CPU-PERF-004` jest zamknięte dopiero wtedy, gdy:
