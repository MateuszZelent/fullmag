# FEM-CPU-PERF-001 — Rozdzielenie setup/assembly od wielokrotnego operator apply
...
Ten plik jest wykonawczym planem zamknięcia ustalenia `FEM-CPU-PERF-001`. Nie opisuje jedynie kierunku refaktoryzacji: definiuje docelowy invariant, właścicieli danych, wymagane zmiany w kodzie, testy regresyjne, dowody fizyczne i numeryczne oraz kryteria promocji do statusu produkcyjnego.
...
Exchange i mass forms są montowane jako LEGACY, a runtime ma rozbudowane ścieżki operatorów, ograniczeń okresowych i demag. Każda odbudowa formy, sparsity, essential DOF lub preconditionera w etapie RK byłaby dominującym kosztem.

### Skutek

Koszt O(nnz) lub droższy per stage, utrata skalowania i brak przewidywalnej wydajności po wielu krokach.

## 3. Docelowy kontrakt

Kontekst ma jawny lifecycle `build -> finalize -> repeated apply`. Wszystkie obiekty zależne wyłącznie od mesh/material/boundary revision są trwałe; step może jedynie stosować operator i rozwiązywać układ.
...
|---|---|
| `backends/fem/cpu/mfem/interactions/exchange_operator.cpp` | operator fizyczny, pole lub energia |
| `backends/fem/cpu/mfem/interactions/exchange_runtime.cpp` | operator fizyczny, pole lub energia |
| `backends/fem/cpu/mfem/interactions/demag_poisson.cpp` | operator fizyczny, pole lub energia |
| `backends/fem/cpu/mfem/runtime/mfem_context.cpp` | modyfikacja lub weryfikacja kontraktu |
...
- Wprowadzić `OperatorDependencyKey` obejmujący mesh topology/geometry, FE order, material coefficient revision, BC/PBC i device mode.
- Każdy operator posiada osobne `setup()` i `apply()` oraz liczniki build/reuse/invalidation.
- Zakazać `Assemble/Finalize/SetOperator` w hot-loop scope przez audit hook/test.

### Etap C — integracja i migracja

- Zmiany wartości czasu zależnych od źródła nie mogą unieważniać operatorów geometrycznych.
- Preconditioner odbudowywać tylko po zmianie operator key lub po jawnej polityce degradation.
- Publikować setup versus apply time oraz rebuild reason.
...
- 100-step contract: assembly/setup count pozostaje 1.
- Zmiana tylko magnetyzacji nie unieważnia operatora.
- Zmiana mesh/material/BC unieważnia dokładnie wymagane zależności.
- Fault injection podczas rebuild nie publikuje częściowo nowego operatora.
- Benchmark setup i steady-state apply osobno.
...
- Niepełny dependency key może użyć stale operatora.
- Nadmierne invalidation zniweluje zysk.

## 11. Poza zakresem

- Natychmiastowe przejście wszystkich operatorów na partial assembly.

## 12. Definition of Done

Ustalenie `FEM-CPU-PERF-001` jest zamknięte dopiero wtedy, gdy:
