...
On a host enrolled with `Fullmag_build_runner`, use `local-build-runner` for full builds of master and task worktrees. Reuse the single coordinator and explicitly select the source checkout and commit/snapshot; a new worktree does not authorize another runner or bypassing the queue. Lightweight tests and scientific runtime gates remain separate.
