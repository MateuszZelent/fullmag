...
async function main() {
  ...
  assertMagneticTargetsAvailable(scene, [targetAId, targetBId, targetCId]);
...
function assertMixedTargetVisualizationState(state, targetBSource) {
  ...
  const targetC = targets.get(targetCId)?.settings;
  const airbox = state.targets?.airbox?.settings;
  if (targetA?.surface_color_source !== "orientation" || !targetA.vectors_visible) {
    throw new Error(`Target A was not patched to orientation+vectors: ${targetAId}`);
  }
  if (
  ...
  if (targetC?.surface_color_source !== "solid" || !targetC.vectors_visible) {
...
async function waitForMixedTargetProof({
  ...
  try {
    return await poll("mixed-target field requests", async () => {
      ...
      const targetCRequest = fieldRequests.find(
        (entry) =>
          entry.quantityId === "m" &&
          entry.params.component === "full" &&
...
async function firstColorbarHandle(page) {
  const handle = await page.locator(".fm-viewport-3d__colorbar").first().elementHandle();
  if (!handle) {
