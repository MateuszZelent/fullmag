...
describe("compute performance smoke script", () => {
  it("is exposed as a strict browser smoke for runtime compute commands", () => {
    ...
    expect(smokeScript).toContain("verifyViewport3DGesturesDuringSolve");
    expect(smokeScript).toContain("VIEWPORT_3D_CANVAS_SELECTOR");
    expect(smokeScript).toContain(".fm-viewport-3d canvas");
    expect(smokeScript).toContain("COMPUTE_VIEWPORT_GESTURE_FORBIDDEN_REQUEST_PREFIXES");
    expect(smokeScript).toContain("compute-viewport-camera-gestures");
    expect(smokeScript).toContain("viewportGestureProof");
    expect(smokeScript).toContain("readCanvasContextState");
    expect(smokeScript).toContain("recordViewportGestureRequests");
    expect(smokeScript).toContain("assertViewportGestureDoesNotFetch");
    expect(smokeScript).toContain("unexpectedViewportGestureRequests");
    expect(smokeScript).toContain("previousCameraSignature");
    expect(smokeScript).toContain("nextCameraSignature = await waitForCameraSignatureChange(");
    expect(smokeScript).toContain("return nextCameraSignature;");
