...
describe("viewport proof manifest", () => {
  ...
  it("writes canonical JSON and validates the stored manifest", async () => {
    const root = await fixtureRoot();
    const path = join(root, "proof-manifest-v1.json");

    await writeProofManifest(qualificationManifest(), path, root);

    expect(JSON.parse(await readFile(path, "utf8"))).toEqual(qualificationManifest());
