...
/**
 * S-07 — CPU-side (float64) normalization for the `scalarValues` GPU
 * attribute; see the identical rationale in viewport3dFieldMapping.ts's
 * copy of this helper. NaN / overflow sentinels pass through unnormalized
 * so the fragment shader's existing bad-value detection keeps working.
 */
...
export const normalizeScalarValueForTests = normalizeScalarValue;
export const normalizeScalarValueForShaderAttributeForTests = normalizeScalarValueForShaderAttribute;
export const resolveScalarRangeForFieldForTests = resolveScalarRangeForField;
