...
describe("viewport 3D mixed-target smoke script", () => {
  it("codifies the production mixed-target proof gate from the field-data report", () => {
    ...
    expect(smokeScript).toContain('targetBSource : "orientation"');
    ...
    expect(smokeScript).toContain("getBoundingClientRect");
    expect(smokeScript).toContain("isContextLost");
    expect(smokeScript).toContain("assertMixedTopologyManifest");
    expect(smokeScript).toContain("assertAirboxFullWireframeBuildEvidence");
    expect(smokeScript).toContain("edge-source=volumeEdges");
    expect(smokeScript).toContain("render-semantic=hiddenEdges");
    expect(smokeScript).toContain("assertNoTopologyRebuildAfterFieldSwitch");
    expect(smokeScript).toContain("assertMixedMeshFamiliesSelectable");
    expect(smokeScript).toContain("globalCellOrdinal");
  ...
  it("has a lightweight runtime fixture for the mixed-target live proof", () => {
    ...
    expect(fixture).toContain('name="film"');
