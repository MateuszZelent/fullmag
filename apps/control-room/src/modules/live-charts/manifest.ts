...
export const liveChartsManifest: ModuleManifest = {
  ...
  contributes: { commands: [
    ...
    { id: "live-charts.fit", title: "Fit Live Chart", group: "live-charts", category: "View", scope: "viewport", run: () => ({ status: "completed" as const }) },
    ...(["csv", "tsv", "png"] as const).map((format) => ({ id: `live-charts.export.${format}`, title: `Export Live Chart ${format.toUpperCase()}`, group: "live-charts", category: "Analysis", scope: "viewport" as const, run: () => ({ status: "completed" as const }) })),
  ] },
};
