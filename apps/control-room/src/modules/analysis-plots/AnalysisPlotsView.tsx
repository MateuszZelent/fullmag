...
export function AnalysisPlotsView(props: AnalysisPlotsViewInput) {
  const { activeSurface, comparisonDatasetRef = null, comparisonSelectedSeriesKeys = [], comparisonTable = null, comparisonTableStatus = "idle", comparisonTableUnsupportedReason = null, comparisonVisibleRevision = null, datasetRefs = [], displayUnits = {}, dynamicStructureFactor = null, dynamicStructureFactorStatus = "idle", frequencyDomainProvenance = null, frequencyDomainSeries = [], frequencyDomainStatus = "idle", frequencyDomainTitle = "Frequency domain", frequencyDomainUnavailableReason = null, hasComparisonSelection = false, kernel, onDatasetRefChange = () => undefined, onComparisonDatasetRefChange = () => undefined, onComparisonSelectedSeriesKeysChange = () => undefined, onSurfaceChange = () => undefined, range = null, selectedDatasetRef = null, selectedPoint = null, selectedSeriesIds = [], selectedStageId = null, spinWaveGamma = null, spinWaveGammaStatus = "idle", surfaceProvenance = {}, table = null, tableStatus = "idle", tableUnsupportedReason = null, xAxisId: selectedXAxisId = null } = props;
  ...
  return <div className="fm-analysis-plots">
    ...
    <section className="fm-analysis-plots__panel fm-analysis-plots__panel--primary">
      ...
      {surface === "dynamics" ? (datasetPrompt ?? <AnalysisTableSurface chartId={chartId} chartSeries={chartSeries} displayUnits={displayUnits} kernel={kernel} onPointSelect={onPointSelect} onRangeChange={onRangeChange} onSelectedSeriesIdsChange={onSelectedSeriesIdsChange} range={range} selectedPoint={selectedPoint} selectedSeriesIds={selectedSeriesIds} status={tableUnsupportedReason ? "unsupported" : resolvedTableStatus} table={resolvedTable} unsupportedReason={tableUnsupportedReason} xAxisId={xAxisId} xAxisLabel={formatXAxisLabel(chartSeries, xAxisId)} />) : null}
      ...
      {surface === "comparison" ? <div className="fm-analysis-plots__comparison">
        ...
        {!resolvedDatasetRef ? <div className="fm-analysis-plots__empty" role="status">Select a published dataset before comparison.</div> : !comparisonDatasetRef ? <div className="fm-analysis-plots__empty" role="status">Select a second published dataset compatible with {resolvedDatasetRef} to compare series.</div> : <><strong>Compatible series</strong><span>{resolvedDatasetRef} · revision {resolvedTable?.revision ?? "unknown"}</span><span>{comparisonDatasetRef} · revision {comparisonVisibleRevision ?? comparisonTable?.revision ?? "unknown"}</span>{comparisonTableStatus !== "ready" || comparisonTableUnsupportedReason ? <AnalysisTableSurface chartId={`comparison:${comparisonDatasetRef}`} chartSeries={[]} kernel={kernel} onPointSelect={onPointSelect} onRangeChange={onRangeChange} onSelectedSeriesIdsChange={onComparisonSelectedSeriesKeysChange} range={range} selectedPoint={selectedPoint} selectedSeriesIds={[]} status={comparisonTableUnsupportedReason ? "unsupported" : comparisonTableStatus} table={comparisonTable} unsupportedReason={comparisonTableUnsupportedReason} xAxisId={comparisonTable?.columns[0]?.column_id ?? "x"} xAxisLabel={formatXAxisLabel(comparisonSeries, comparisonTable?.columns[0]?.column_id ?? "x")} /> : compatibleSeries.length === 0 ? <div role="status">No compatible quantity and unit series are published by both datasets.</div> : <div className="fm-analysis-plots__comparison-panes"><AnalysisTableSurface chartId={chartId} chartSeries={compatibleSeries} kernel={kernel} onPointSelect={onPointSelect} onRangeChange={onRangeChange} onSelectedSeriesIdsChange={(ids) => onComparisonSelectedSeriesKeysChange(comparisonKeysForSeriesIds(ids, compatibleSeries))} range={range} selectedPoint={selectedPoint} selectedSeriesIds={selectedPrimaryComparisonSeries.map((series) => series.id)} status={resolvedTableStatus} table={resolvedTable} xAxisId={xAxisId} xAxisLabel={formatXAxisLabel(compatibleSeries, xAxisId)} /><AnalysisTableSurface chartId={`comparison:${comparisonDatasetRef}`} chartSeries={comparisonSeries.filter((series) => availableComparisonSeriesKeys.includes(comparisonSeriesKey(series)))} kernel={kernel} onPointSelect={onPointSelect} onRangeChange={onRangeChange} onSelectedSeriesIdsChange={(ids) => onComparisonSelectedSeriesKeysChange(comparisonKeysForSeriesIds(ids, comparisonSeries))} range={range} selectedPoint={selectedPoint} selectedSeriesIds={selectedSecondaryComparisonSeries.map((series) => series.id)} status={comparisonTableStatus} table={comparisonTable} unsupportedReason={comparisonTableUnsupportedReason} xAxisId={comparisonTable?.columns[0]?.column_id ?? "x"} xAxisLabel={formatXAxisLabel(comparisonSeries, comparisonTable?.columns[0]?.column_id ?? "x")} /></div>}</>}</div> : null}
    </section>
  </div>;
}
