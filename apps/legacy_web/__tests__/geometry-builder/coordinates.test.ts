...
describe("physical <-> scene coordinates", () => {
  it("position round-trip is stable", () => {
    ...
    expect(scene).toEqual([1.25, -3.5, 8.75]);
  ...
  it("delta round-trip keeps axis mapping", () => {
    ...
    expectVec3Close(physicalDelta, [0.1e-9, 0.2e-9, 0.3e-9]);
  });
  ...
  it("scale round-trip is stable", () => {
    ...
    expect(scaleScene).toEqual([2.0, 0.5, 1.5]);
    expectVec3Close(back, scalePhysical);
  });

  it("dimensionless transform scale stays in physical XYZ order", () => {
  ...
  it("quaternion round-trip is stable", () => {
    ...
    expectQuatClose(qScene, qPhysical);
