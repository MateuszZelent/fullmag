...
- Follow NeuralMag's progressive executable path: import and setup, geometry and material, state, interaction, stage, observables, output, and a complete end-to-end script. Keep examples in copyable `# %%` cells, in execution order, and make each cell understandable when run from a clean session.
...
- Make the first user-facing example workflow-oriented and runnable. Put reference/API detail after the working path; never leave an isolated parameter table without showing where the parameter is used.
...
4. Add a complete Python authoring chapter: every public parameter, a copyable executable `python` example organized with `# %%` cells, validation/failure behavior, and backend support. The primary runnable user example MUST use the public study pipeline (`fm.study(...)` with `study.stages.add_*`); it must not present a direct `fm.Problem(...)` construction as the normal execution workflow. A direct `fm.Problem(...)` example is allowed only in an explicitly labelled low-level section for stage snapshots, canonical lowering, or `ProblemIR` inspection, and the page must state that distinction. If the current public stage builder cannot represent the interaction graph, do not fabricate a runnable stage example: document the exact API boundary, provide the valid low-level lowering example, and mark the missing stage integration as an explicit limitation.
