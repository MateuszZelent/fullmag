...
verify-fdm-gpu-strict-residency:
    python3 -c 'import json, pathlib; p=pathlib.Path(".fullmag/verification/fdm-gpu-strict-residency/execution-receipt.v1.json"); p.parent.mkdir(parents=True, exist_ok=True); p.write_text(json.dumps({"schema_version":"fdm_gpu_execution_receipt_evidence.v1","validation_state":"unvalidated","runtime_check":"unavailable","requested":"device_resident","resolved":"unavailable","executed":"none","fallback_count":0,"transfer_counts":{"setup_full_vector_h2d_count":0,"setup_full_vector_h2d_bytes":0,"hot_loop_full_vector_h2d_count":0,"hot_loop_full_vector_h2d_bytes":0,"hot_loop_full_vector_d2h_count":0,"hot_loop_full_vector_d2h_bytes":0,"hot_loop_host_compute_count":0,"hot_loop_host_sync_count":0,"hot_loop_control_scalar_d2h_bytes":0,"hot_loop_control_scalar_host_sync_count":0}}, indent=2)+"\n")'
    docker compose --profile fem-gpu run --rm --no-deps -v "${FULLMAG_MANAGED_NATIVE_ROOT:-/mnt/fullmag-zfn2-native}:${FULLMAG_MANAGED_NATIVE_ROOT:-/mnt/fullmag-zfn2-native}" -e FULLMAG_MANAGED_NATIVE_ROOT="${FULLMAG_MANAGED_NATIVE_ROOT:-/mnt/fullmag-zfn2-native}" -e CMAKE_BUILD_PARALLEL_LEVEL="${FULLMAG_NATIVE_BUILD_JOBS:-2}" -e FULLMAG_CUDA_ARCHITECTURES="${FULLMAG_CUDA_ARCHITECTURES:-native}" fem-gpu bash -lc 'set -euo pipefail; cd /workspace; build_dir="${FULLMAG_MANAGED_NATIVE_ROOT:-/mnt/fullmag-zfn2-native}/fdm-gpu-strict-residency"; evidence="/workspace/.fullmag/verification/fdm-gpu-strict-residency/execution-receipt.v1.json"; tmp="$evidence.tmp"; mkdir -p "$build_dir" "$build_dir/cargo-home" "$build_dir/cargo-target"; rm -f "$tmp"; cmake -S native -B "$build_dir/native" -DCMAKE_CUDA_ARCHITECTURES="$FULLMAG_CUDA_ARCHITECTURES" -DFULLMAG_ENABLE_CUDA=ON -DFULLMAG_ENABLE_FEM_GPU=OFF -DFULLMAG_USE_MFEM_STACK=OFF -DFULLMAG_FEM_WITH_SLEPC=OFF; cmake --build "$build_dir/native" --target fullmag_fdm fdm_device_residency_receipt_contract; FULLMAG_FDM_GPU_RESIDENCY_EVIDENCE_PATH="$tmp" "$build_dir/native/backends/fdm/fdm_device_residency_receipt_contract"; test -s "$tmp"; python3 -m json.tool "$tmp" >/dev/null; mv "$tmp" "$evidence"; ctest --test-dir "$build_dir/native/backends/fdm" --output-on-failure --no-tests=error -R "^fdm_device_residency_receipt_contract$"; export FULLMAG_FDM_LIB_DIR="$build_dir/native/backends/fdm"; export LD_LIBRARY_PATH="$FULLMAG_FDM_LIB_DIR${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"; export CARGO_HOME="$build_dir/cargo-home"; export CARGO_TARGET_DIR="$build_dir/cargo-target"; export CARGO_INCREMENTAL=0; cargo test -p fullmag-runner --features cuda fdm::gpu::cuda::native::residency::tests -- --nocapture; cargo test -p fullmag-runner --features cuda fdm::gpu::cuda::route::tests -- --nocapture'
