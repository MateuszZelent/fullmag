...
verify-fem-demag-fem-bem-native-contract:
    docker compose --profile fem-gpu run --rm --no-deps \
      -e CMAKE_BUILD_PARALLEL_LEVEL="${FULLMAG_NATIVE_BUILD_JOBS:-2}" \
      -e FULLMAG_CUDA_ARCHITECTURES="${FULLMAG_CUDA_ARCHITECTURES:-native}" \
      fem-gpu bash -lc 'set -euo pipefail; cd /workspace; build_dir=/tmp/fullmag-fem-bem-native-contract; cmake -S native -B "$build_dir" -DCMAKE_CUDA_ARCHITECTURES="$FULLMAG_CUDA_ARCHITECTURES" -DFULLMAG_ENABLE_CUDA=ON -DFULLMAG_ENABLE_FEM_GPU=ON -DFULLMAG_USE_MFEM_STACK=ON -DFULLMAG_FEM_WITH_SLEPC=OFF; cmake --build "$build_dir" --target fem_demag_fem_bem_contract fem_demag_fem_bem_gpu_contract fem_cuda_demag_timing_contract fem_hypre_validation_policy_contract fem_gpu_sparse_apply_contract fem_gpu_operator_fusion_contract; ctest --test-dir "$build_dir/backends/fem" --output-on-failure --no-tests=error -R "^fem_(demag_fem_bem(_gpu)?|cuda_demag_timing|hypre_validation_policy|gpu_sparse_apply|gpu_operator_fusion)_contract$"'
