use fullmag_ir::{
    FdmGridCertificateIR, FieldEnvelopeIR, FieldSpatialProfileIR, FieldTargetIR, GeometryEntryIR, RegionalFieldDriveIR,
    ResolvedRegionalFieldDriveBasisIR,
};
...
pub(crate) fn resolve_fdm_regional_field_drives(
    ...
    for drive in drives.iter().filter(|drive| drive.enabled) {
        ...
        for (index, point) in sample_points.iter().enumerate() {
            ...
            if !target_regions.is_empty() && !target_regions.contains(&region_mask.get(index).copied().unwrap_or(0)) {
                continue;
            }
...
fn sinc_value(axis: [f64; 3], period_m: f64, center_m: f64, width_m: Option<f64>, window: &str, point: [f64; 3]) -> f64 {
    let coordinate = point[0] * axis[0] + point[1] * axis[1] + point[2] * axis[2] - center_m;
    if width_m.is_some_and(|width| coordinate.abs() > 0.5 * width) { return 0.0; }
    let x = std::f64::consts::PI * coordinate / period_m;
    let sinc = if x.abs() <= 1e-4 {
        let x2 = x * x; 1.0 - x2 / 6.0 + x2 * x2 / 120.0
    } else { x.sin() / x };
...
enum CellRelation { Outside, Inside, Boundary }
...
fn spatial_point_value(profile: &FieldSpatialProfileIR, point: [f64; 3], geometry: &[GeometryEntryIR]) -> Result<f64, PlanError> {
    match profile {
        FieldSpatialProfileIR::Uniform {} => Ok(1.0),
        FieldSpatialProfileIR::Sinc { axis, period_m, center_m, width_m, window } => {
            Ok(sinc_value(*axis, *period_m, *center_m, *width_m, window, point))
        }
        FieldSpatialProfileIR::GeometryMask { object_id, envelope } => {
            let entry = geometry.iter().find(|entry| entry.name() == object_id).ok_or_else(|| PlanError { reasons: vec![format!(
                "FDM RegionalFieldDrive geometry mask '{}' is absent", object_id)] })?;
            if !geometry_contains(entry, point)? { return Ok(0.0); }
            Ok(match envelope {
                FieldEnvelopeIR::Uniform {} => 1.0,
                FieldEnvelopeIR::Sinc { axis, period_m, center_m, width_m, window } =>
...
fn tensor_average<const N: usize>(
    profile: &FieldSpatialProfileIR, center: [f64; 3], half: [f64; 3], geometry: &[GeometryEntryIR],
    points: [f64; N], weights: [f64; N],
) -> Result<f64, PlanError> {
    let mut sum = 0.0;
    for ix in 0..N { for iy in 0..N { for iz in 0..N {
        let point = [center[0] + half[0] * points[ix], center[1] + half[1] * points[iy], center[2] + half[2] * points[iz]];
        sum += weights[ix] * weights[iy] * weights[iz] * spatial_point_value(profile, point, geometry)?;
    }}}
    Ok(sum / 8.0)
}

fn adaptive_cell_average(
    profile: &FieldSpatialProfileIR, center: [f64; 3], half: [f64; 3], geometry: &[GeometryEntryIR], depth: u32,
) -> Result<f64, PlanError> {
    const P4: [f64; 4] = [-0.8611363115940526, -0.3399810435848563, 0.3399810435848563, 0.8611363115940526];
    const W4: [f64; 4] = [0.34785484513745385, 0.6521451548625461, 0.6521451548625461, 0.34785484513745385];
    match profile {
        FieldSpatialProfileIR::Sinc { .. } if depth == 0 => {
        ...
        FieldSpatialProfileIR::GeometryMask { object_id, envelope } => {
            ...
            match geometry_cell_relation(entry, center, half)? {
                ...
                CellRelation::Inside if matches!(envelope, FieldEnvelopeIR::Uniform {}) => return Ok(1.0),
                CellRelation::Inside => return tensor_average(profile, center, half, geometry, P4, W4),
                CellRelation::Boundary => {}
            }
        }
        _ => {}
    }
    let low = tensor_average(profile, center, half, geometry,
        [-0.5773502691896257, 0.5773502691896257], [1.0, 1.0])?;
    let high = tensor_average(profile, center, half, geometry, P4, W4)?;
    if (high - low).abs() <= 1.0e-6 { return Ok(high); }
    if depth == 10 { return Err(PlanError { reasons: vec![
    ...
    for sx in [-1.0, 1.0] { for sy in [-1.0, 1.0] { for sz in [-1.0, 1.0] {
        let child = [center[0] + sx * child_half[0], center[1] + sy * child_half[1], center[2] + sz * child_half[2]];
        sum += adaptive_cell_average(profile, child, child_half, geometry, depth + 1)?;
    }}}
    Ok(sum / 8.0)
}

fn spatial_cell_average(profile: &FieldSpatialProfileIR, center: [f64; 3], cell_size: [f64; 3], geometry: &[GeometryEntryIR]) -> Result<f64, PlanError> {
    if matches!(profile, FieldSpatialProfileIR::Uniform {}) { return Ok(1.0); }
    adaptive_cell_average(profile, center, [cell_size[0]*0.5, cell_size[1]*0.5, cell_size[2]*0.5], geometry, 0)
}
...
mod tests {
    ...
    fn geometry_mask_uses_cell_volume_average_instead_of_centroid_sampling() {
        let geometry = vec![GeometryEntryIR::Translate {
            name: "half-cell".into(),
            base: Box::new(GeometryEntryIR::Box { name: "box".into(), size: [1.0, 1.0, 1.0] }),
            by: [-0.5, 0.0, 0.0],
        }];
        let profile = FieldSpatialProfileIR::GeometryMask {
            object_id: "half-cell".into(), envelope: FieldEnvelopeIR::Uniform {},
        };
    ...
    fn spatial_sinc_is_integrated_over_the_cell() {
        let profile = FieldSpatialProfileIR::Sinc {
            axis: [1.0, 0.0, 0.0], period_m: 1.0, center_m: 0.0,
            width_m: None, window: "none".into(),
        };
