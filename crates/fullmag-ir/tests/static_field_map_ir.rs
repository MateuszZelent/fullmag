...
fn static_field_map_is_a_typed_zeeman_ir_term() {
    ...
    problem.energy_terms = vec![EnergyTermIR::Exchange, EnergyTermIR::StaticFieldMap {
    ...
    let decoded: ProblemIR = serde_json::from_value(encoded).expect("static field map deserializes");
