...
mod tests {
    use super::*;
    use fullmag_ir::{DriveActivationIR, FieldDriveKindIR, FieldSpatialProfileIR, FieldTargetIR, FieldTimeOriginIR, TimeDependenceIR};
    ...
    fn manifest_carries_revision_stage_events_and_provenance() {
        let drive = RegionalFieldDriveIR {
            ...
            waveform: TimeDependenceIR::Pulse { t_on: 1e-12, t_off: 2e-12 },
        ...
        let artifact = regional_field_drive_artifact(
            &[drive],
            &TimeStageContextIR { active_stage_id: Some("run".into()), start_time_s: 10e-12 },
            3e-12,
            &[],
            &ExecutionProvenance { execution_engine: "native_fem_cpu".into(), precision: "double".into(), ..Default::default() },
        ).unwrap().unwrap();
