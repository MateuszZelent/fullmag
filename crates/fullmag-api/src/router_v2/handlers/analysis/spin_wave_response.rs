...
fn bound_dynamic_structure_factor(
    mut resource: DynamicStructureFactorResource,
    max_cells: usize,
    ...
    let project = |values: &[f64]| {
        f_indices
            ...
            .flat_map(|f| {
                k_indices
                    .iter()
                    .map(move |k| values.get(f * original_nk + k).copied().unwrap_or(0.0))
            })
            .collect::<Vec<_>>()
    };
    ...
    resource.frequency_hz = f_indices
        .iter()
        .map(|index| resource.frequency_hz[*index])
        .collect();
    resource.k_rad_per_m = k_indices
        .iter()
        .map(|index| resource.k_rad_per_m[*index])
        .collect();
    resource.invalid_probe_mask = k_indices
        .iter()
        .map(|index| {
            resource
                .invalid_probe_mask
        ...
        .collect();
    resource.frequency_count = f_indices.len();
    resource.wavevector_count = k_indices.len();
    resource.original_frequency_count = original_nf;
...
mod tests {
    ...
    fn finite_k_json_projection_is_bounded_and_keeps_artifact_reference() {
        ...
        let resource = DynamicStructureFactorResource {
            ...
            artifact_ref: "analysis/full.json".into(),
            bounded: false,
            original_frequency_count: 100,
            original_wavevector_count: 100,
            wavevector_unit: "rad/m".into(),
            frequency_unit: "Hz".into(),
            x_m: (0..100).map(|i| i as f64).collect(),
            time_s: (0..198).map(|i| i as f64).collect(),
            k_rad_per_m: (0..100).map(|i| i as f64).collect(),
            frequency_hz: (0..100).map(|i| i as f64).collect(),
            power: vec![1.0; count],
            spectrum_real: vec![0.0; count],
            spectrum_imag: vec![0.0; count],
            source_power: vec![1.0; count],
            source_spectrum_real: vec![0.0; count],
            source_spectrum_imag: vec![0.0; count],
            source_observable: "H_drive".into(),
            source_unit: "A/m".into(),
            component: "my".into(),
            propagation_axis: "x".into(),
            phase_convention: "exp[-i(k*x-2*pi*f*t)]".into(),
            normalization: "canonical".into(),
            spatial_window: vec![1.0; 100],
            temporal_window: vec![1.0; 198],
            spatial_window_power_sum: 100.0,
            temporal_window_power_sum: 198.0,
            mesh_probe_signature: "signature".into(),
            invalid_probe_mask: vec![false; 100],
        ...
        assert_eq!(
            bounded.power.len(),
            bounded.frequency_count * bounded.wavevector_count
        );
        assert_eq!(bounded.artifact_ref, "analysis/full.json");
        assert_eq!(
            (
                bounded.original_frequency_count,
                bounded.original_wavevector_count
            ),
            (100, 100)
        );
