...
{
    ctx.zeeman.h_ext_xyz.resize(static_cast<size_t>(ctx.mesh.n_nodes) * 3u);
    ...
    for (uint32_t i = 0; i < ctx.mesh.n_nodes; ++i) {
        ...
        ctx.zeeman.h_ext_xyz[base + 0] = ctx.zeeman.external_field_am[0];
        ctx.zeeman.h_ext_xyz[base + 1] = ctx.zeeman.external_field_am[1];
        ctx.zeeman.h_ext_xyz[base + 2] = ctx.zeeman.external_field_am[2];
    }
}
