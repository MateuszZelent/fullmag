#!/usr/bin/env python3
"""Validate a GPU Poisson parity artifact for PA-G1 gating."""
...
def fail(message: str) -> NoReturn:
    raise SystemExit(f"invalid GPU Poisson parity artifact:\n{message}")
...
def require_nonnegative_int(payload: dict[str, object], key: str) -> int:
    actual = payload.get(key)
...
def validate_payload(payload: dict[str, object]) -> None:
    require_equal(payload, "schema_version", "gpu_poisson_parity.v1")
    ...
    parity = payload.get("gpu_poisson_parity")
    if not isinstance(parity, dict):
        fail("gpu_poisson_parity must be a JSON object")
    ...
    h2d_count = require_nonnegative_int(parity, "h2d_count")
    d2h_count = require_nonnegative_int(parity, "d2h_count")
    if h2d_count != 0:
        fail(f"h2d_count must be 0 for PA-G1 hot-loop parity, got {h2d_count}")
    if d2h_count != 0:
        fail(f"d2h_count must be 0 for PA-G1 hot-loop parity, got {d2h_count}")


def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: verify_fem_gpu_poisson_parity_artifact.py "
            "<gpu_poisson_parity.v1.json>",
            file=sys.stderr,
        )
        return 2
