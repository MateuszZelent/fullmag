...
# The storage adapter validates FULLMAG_PROJECT_STORAGE_ROOT through
# scripts/fullmag_storage.py before creating any build or cache directory.
param(
...
$FrontendRoot = if ($env:FULLMAG_WINDOWS_FRONTEND_ROOT) {
  ...
  Join-Path ([string]$StorageLayout.frontend_root) "fem-$StorageDevice"
}
