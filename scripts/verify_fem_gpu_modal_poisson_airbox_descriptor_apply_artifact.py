#!/usr/bin/env python3
"""Validate a GPU Poisson-airbox modal descriptor-apply artifact for GPU-G5b."""
...
def fail(message: str) -> NoReturn:
    raise SystemExit(
        "invalid GPU Poisson-airbox modal descriptor-apply artifact:\n" + message
    )
...
def require_finite_nonnegative_number(payload: dict[str, object], key: str) -> float:
    ...
    if not isinstance(actual, (int, float)) or isinstance(actual, bool):
        fail(f"{key} must be a finite nonnegative number")
    value = float(actual)
...
def validate_payload(payload: dict[str, object]) -> None:
    require_equal(payload, "schema_version", "gpu_modal_poisson_airbox_descriptor_apply.v1")
    ...
    require_equal(payload, "execution_lane", "gpu_device_modal_descriptor_apply_contract")
    ...
    require_equal(payload, "algebraic_action", "A*x")
    require_equal(payload, "matrix_format", "csr_device_apply")
    ...
    require_equal(payload, "gpu_device_resident_operator_apply", True)
    require_equal(payload, "cpu_fallback", "disabled")
    require_equal(payload, "fallback_used", False)
    require_equal(payload, "setup_h2d_count", 14)
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "usage: verify_fem_gpu_modal_poisson_airbox_descriptor_apply_artifact.py "
