#!/usr/bin/env python3
"""Verify the bounded public FDM GPU charge fixture against its 1D SI oracle."""
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: verify_fdm_gpu_public_charge_output.py OUTPUT_DIR")
    ...
    assert provenance["charge_transport"]["fallbacks_triggered"] == []
    ...
    # Two cell-centre samples for 0 V -> 0.1 V over 20 nm.
    assert_close(potential[0], 0.025, 1.0e-14, "V[0]")
    assert_close(potential[1], 0.075, 1.0e-14, "V[1]")
    # J_x = -sigma * dV/dx = -(4e6 S/m)*(0.1 V)/(20 nm).
    for cell in range(2):
    ...
    assert transport["execution"]["resolved_engine"] == "cuda_fdm_charge_only"
    assert transport["execution"]["physical_residual"] < 1.0e-12
    return 0
