#!/usr/bin/env python3
"""Tests for GPU Poisson-airbox modal shifted descriptor-apply artifact validation."""
...
VALIDATOR = (
    REPO_ROOT
    / "scripts"
    / "verify_fem_gpu_modal_poisson_airbox_shifted_descriptor_apply_artifact.py"
)
...
def test_validator_accepts_gpu_modal_shifted_descriptor_apply_artifact(tmp_path: Path) -> None:
    artifact = tmp_path / "gpu_modal_poisson_airbox_shifted_descriptor_apply.v1.json"
...
def test_validator_rejects_unshifted_action(tmp_path: Path) -> None:
    artifact = tmp_path / "gpu_modal_poisson_airbox_shifted_descriptor_apply.v1.json"
    artifact.write_text(json.dumps(payload(algebraic_action="A*x")), encoding="utf-8")

    result = run_validator(artifact)

    assert result.returncode != 0
    assert "algebraic_action" in (result.stderr + result.stdout)


def test_validator_rejects_dense_matrix_format(tmp_path: Path) -> None:
    artifact = tmp_path / "gpu_modal_poisson_airbox_shifted_descriptor_apply.v1.json"
    artifact.write_text(json.dumps(payload(matrix_format="dense_host_shifted_apply")), encoding="utf-8")
...
def test_validator_rejects_per_iteration_host_roundtrip(tmp_path: Path) -> None:
    artifact = tmp_path / "gpu_modal_poisson_airbox_shifted_descriptor_apply.v1.json"
    artifact.write_text(json.dumps(payload(per_iteration_h2d_count=1)), encoding="utf-8")
