...
- The CPU descriptor path now has a kernel catalog and reusable FFT workspace; this closes an
  implementation gap, not the qualification gap for every BORIS kernel family or for CUDA.
...
| Common-cell pitch and normalization | BORIS derives `h_common = convolution_rect / n_common` and uses the maximum cell size when normalizing transfer dimensions. | Fullmag keeps common-cell size, native-cell size, and transform layout as separate resolved descriptor fields; a mismatch becomes explicit transfer or rejection. | Equal counts do not prove equal cell volumes or transfer accuracy; each cell-size class needs its own field/energy evidence. |
...
| Kernel catalog and reuse | BORIS catalog declarations in `DemagKernelCollection.h` (L108-L131) cover 2-D/3-D self, Z-shifted, X-shifted, and complex-full families; `KernelAlreadyComputed` (L130-L131) checks reuse. | Fullmag `KernelReuseKey::from_pair_with_layout` plus the CPU `kernel_catalog`/`pair_bindings` runtime deduplicate one tensor per full key and reuse one FFT/workspace allocation across refreshes; telemetry reports hit/miss, pair/FFT counts, and cold/warm bytes. | CPU descriptor-path reuse is implemented and fail-closed, but it does not qualify all BORIS kernel families, reduced/full representations, CUDA, or device residency. |
...
| PBC images | BORIS `demag_pbc_images` and `Set_PBC` apply the same image counts to every participating mesh in either supermesh or multilayer convolution. | Fullmag's multilayer boundary mode is `open`; PBC is rejected by the planner and cannot silently fall back to another strategy. | PBC remains an explicit gap requiring Python/ProblemIR fields, image kernels, energy/provenance rules, and separate CPU/CUDA qualification. |
| Empty cells, masks, and energy | BORIS tracks `non_empty_cells` and `total_nonempty_volume`, excludes mesh regions as configured, and normalizes multilayer energy over participating volume. | Fullmag keeps native active masks, uses volume-weighted demag energy, and publishes target-only Airbox `H_demag` while keeping solver masking separate from visualization fields. | Mask, volume, energy, and Airbox claims require separate runtime artifacts; source tests do not close the full matrix. |
| Antiferromagnetic and atomistic meshes | BORIS tracks antiferromagnetic participation and forces transfer for atomistic meshes. | Fullmag's public multilayer contract is for named ferromagnetic FDM objects; AFM and atomistic mesh semantics are not silently mapped into it. | This is an explicit scope gap requiring a separate material, mask, transfer, energy, and qualification contract. |
| Reconfiguration and invalidation | BORIS rebuilds per-mesh demag modules when mesh geometry, `n_common`, force mode, or PBC changes. | Fullmag resolves a new plan from topology fingerprints and invalidates the CPU catalog/workspace when descriptor geometry changes. | An end-to-end session replan and post-replan parity receipt is still missing; a fingerprint alone is not a runtime qualification. |
...
| BORIS rectangle collection | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-rect-collection` | Traceability for per-layer rectangles and scratch alignment. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS common counts | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-common-n` | Traceability for `n_common` defaults and forced 2-D Z count. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS force-mode header | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-header-force-mode` | Traceability for the documented `force_2d_convolution` values 0, 1, and 2. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS explicit common counts | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-set-n-common` | Traceability for the explicit common convolution-count setter. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS convolution rectangle | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-convolution-rect` | Traceability for the adjusted scratch rectangle. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS multilayer initialization | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-mconv-init` | Traceability for per-module convolution scratch setup. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS multilayer update | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-mconv-update` | Traceability for forward/pair/inverse stages. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS 2-D mode switch | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-2d-mode` | Traceability for `force_2d_convolution`. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS multilayer toggle | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-multilayer-toggle` | Traceability for the multilayer-versus-supermesh choice. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS kernel catalog | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-kernel-catalog` | Traceability for self/shifted/full kernel families. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS kernel reuse | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-kernel-reuse` | Traceability for shift and cell-size reuse. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS kernel storage | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-kernel-storage` | Traceability for `KerType` storage flags, cell sizes, and FFT dimensions. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS pair multiplication | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-kernel-multiply` | Traceability for ordered multiple-input multiplication. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS 3-D pair multiplication | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-kernel-multiply-3d` | Traceability for ordered 3-D multiple-input multiplication. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS irregular thickness | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-irregular-thickness` | Traceability for unequal Z thickness. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS weighted transfer | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-weighted-transfer` | Traceability for weighted mesh transfer. | BORIS reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS CUDA update | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-cuda-update` | Traceability for CUDA multilayer staging. | BORIS CUDA reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
| BORIS CUDA initialization | `docs/physics/multilayer_convolution/boris-behavioral-spec.v1.md` | `DOC-ANCHOR:boris-cuda-init` | Traceability for CUDA scratch initialization. | BORIS CUDA reference | clean-room behavioral specification; manifest hash | traceability only | Repository anchor |
