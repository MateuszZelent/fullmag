...
| `FDM.demag` | `FDMDemag \| None` | `None` | $1$ | `FDM.__init__` has no explicit type check; a valid `FDMDemag` is required during lowering. | Demagnetizing-field realization policy. | FDM CPU/GPU authoring. | `backend_policy.discretization_hints.fdm.demag` |
...
| `FDMDemag.explain` | `bool` | `True` | $1$ | The raw script builder requires a Boolean; the constructor itself does not type-check it. | Requests a readable plan explanation; it is not physical data and is not serialized by `to_ir()`. | FDM authoring helper. | `not serialized` |
