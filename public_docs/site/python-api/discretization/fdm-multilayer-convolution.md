...
| `FDM.demag` | `FDMDemag \| None` | `None` | $1$ | `FDM.__init__` has no explicit type check; a valid `FDMDemag` is required during lowering. | Attaches requested demagnetization topology and common-grid policy. | FDM CPU/GPU. | `backend_policy.discretization_hints.fdm.demag` |
