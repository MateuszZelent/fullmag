...
(python-api-interactions-thermal-noise-problem-statement)=
## Problem statement

Temperature and optional reproducibility seed are explicit stochastic inputs.
...
(python-api-interactions-thermal-noise-governing-equations)=
## Governing equations

```{math}
:label: eq-thermal_noise

q_{\mathrm{IR}} = \mathrm{thermal_noise}(\text{qualified inputs})
```

The physical term or constraint is represented by the canonical IR object thermal_noise. The exact discrete operator, quadrature, mesh treatment, and solver selection are backend responsibilities; this page does not replace their qualification evidence.

(python-api-interactions-thermal-noise-symbols-and-si-units)=
...
(python-api-interactions-thermal-noise-assumptions-and-validity)=
...
(python-api-interactions-thermal-noise-python-api)=
## Python API

### Constructor or function

fm.ThermalNoise(temperature, seed=None)

### Parameters

| Python name | Type | Default | SI unit | Validation | Meaning | FEM/FDM CPU/GPU support | ProblemIR destination |
|---|---|---|---|---|---|---|---|
| ```fm.ThermalNoise.temperature``` | ```float``` | ```required``` | ```K``` | strictly positive | bath temperature | FEM/FDM CPU/GPU: IR; stochastic runtime qualification required | ```temperature``` |
| ```fm.ThermalNoise.seed``` | ```int or None``` | ```None``` | ```1``` | positive integer when supplied | reproducibility seed | FEM/FDM CPU/GPU: IR; stochastic runtime qualification required | ```seed``` |
...
value = fm.ThermalNoise(temperature=300.0, seed=7)
...
(python-api-interactions-thermal-noise-problem-ir)=
## ProblemIR

value.to_ir() is the canonical serialization boundary. It emits a typed thermal_noise record with the fields listed above; nested geometry, targets, profiles, or material references remain nested typed records rather than opaque Python objects. The IR is the requested intent. Backend resolution must preserve the record or reject an unsupported combination.

(python-api-interactions-thermal-noise-round-trip-and-failure-semantics)=
...
(python-api-interactions-thermal-noise-discrete-realization)=
...
(python-api-interactions-thermal-noise-implementation-mapping)=
## Implementation mapping

The authoritative implementation is packages/fullmag-py/src/fullmag/model/energy.py symbol class ThermalNoise. The public constructor signature, validation branches, defaults, and to_ir() field names are derived from that source, not from a historical example.

(python-api-interactions-thermal-noise-validation)=
## Validation

Focused repository tests covering this contract include: test_thermal_noise_rejects_nonpositive_temperature, test_thermal_noise_serializes_seed. These tests are evidence for authoring/IR behavior; live runtime, device performance, and Control Room browser behavior require separate qualification.

(python-api-interactions-thermal-noise-limitations)=
...
(python-api-interactions-thermal-noise-scientific-bibliography)=
...
(python-api-interactions-thermal-noise-source-code-index)=
## Source code index

| Source path | Symbol | Responsibility |
|---|---|---|
| packages/fullmag-py/src/fullmag/model/energy.py | class ThermalNoise | public constructor and IR lowering |

- Implementation: packages/fullmag-py/src/fullmag/model/energy.py::class ThermalNoise
- Source-map: interactions/thermal-noise.source-map.json
