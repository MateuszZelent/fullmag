...
(python-api-interactions-exchange-problem-statement)=
## Problem statement

The exchange term is a typed zero-parameter interaction; material exchange constants are supplied by the owning material.
...
(python-api-interactions-exchange-governing-equations)=
## Governing equations

```{math}
:label: eq-exchange

q_{\mathrm{IR}} = \mathrm{exchange}(\text{qualified inputs})
```

The physical term or constraint is represented by the canonical IR object exchange. The exact discrete operator, quadrature, mesh treatment, and solver selection are backend responsibilities; this page does not replace their qualification evidence.

(python-api-interactions-exchange-symbols-and-si-units)=
...
(python-api-interactions-exchange-assumptions-and-validity)=
...
(python-api-interactions-exchange-python-api)=
## Python API
...
|---|---|---|---|---|---|---|---|
| ```fm.Exchange``` | ```constructor``` | ```no arguments``` | ```1``` | no parameters | nearest-neighbour exchange term | FEM/FDM CPU/GPU: IR; runtime resolver-specific | ```kind``` |
...
(python-api-interactions-exchange-problem-ir)=
## ProblemIR

value.to_ir() is the canonical serialization boundary. It emits a typed exchange record with the fields listed above; nested geometry, targets, profiles, or material references remain nested typed records rather than opaque Python objects. The IR is the requested intent. Backend resolution must preserve the record or reject an unsupported combination.

(python-api-interactions-exchange-round-trip-and-failure-semantics)=
...
(python-api-interactions-exchange-discrete-realization)=
...
(python-api-interactions-exchange-implementation-mapping)=
## Implementation mapping

The authoritative implementation is packages/fullmag-py/src/fullmag/model/energy.py symbol class Exchange. The public constructor signature, validation branches, defaults, and to_ir() field names are derived from that source, not from a historical example.

(python-api-interactions-exchange-validation)=
## Validation

Focused repository tests covering this contract include: test_class_api_exchange_coupling_lowers_to_ir. These tests are evidence for authoring/IR behavior; live runtime, device performance, and Control Room browser behavior require separate qualification.

(python-api-interactions-exchange-limitations)=
...
(python-api-interactions-exchange-scientific-bibliography)=
...
(python-api-interactions-exchange-source-code-index)=
## Source code index

| Source path | Symbol | Responsibility |
|---|---|---|
| packages/fullmag-py/src/fullmag/model/energy.py | class Exchange | public constructor and IR lowering |

- Implementation: packages/fullmag-py/src/fullmag/model/energy.py::class Exchange
- Source-map: interactions/exchange.source-map.json
