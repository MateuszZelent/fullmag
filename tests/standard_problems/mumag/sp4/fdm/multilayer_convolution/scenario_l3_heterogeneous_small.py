"""Small Appendix-A L=3 heterogeneous FDM push/pull runtime case."""
...
MIDDLE_SIZE = (31.25e-9, 15.625e-9, 6e-9)
TOP_SIZE = (31.25e-9, 15.625e-9, 3e-9)
COMMON_CELLS = (16, 8, 2)
INITIAL_M = (0.9950371902099893, 0.09950371902099893, 0.0)

study = fm.study("mumag_sp4_fdm_multilayer_convolution_l3_heterogeneous_small")
study.engine("fdm")
...
study.universe(
    mode="manual",
    size=(64e-9, 32e-9, 36e-9),
    center=(0.0, 0.0, 10.5e-9),
    padding=(0.0, 0.0, 0.0),
)
study.universe.mesh(maximum_element_size=3e-9, minimum_element_size=3e-9)
bottom = study.geometry(
    fm.Box(size=BOTTOM_SIZE, name="layer_bottom_geom"), name="layer_bottom"
)
middle = study.geometry(
    fm.Box(size=MIDDLE_SIZE, name="layer_middle_geom").translate((0.0, 0.0, 12e-9)),
    name="layer_middle",
)
top = study.geometry(
    fm.Box(size=TOP_SIZE, name="layer_top_geom").translate((0.0, 0.0, 24e-9)),
    name="layer_top",
)
...
study.runtime_metadata(
    "fdm_multilayer_qualification",
    {
        ...
        "appendix_case": "Appendix-A L=3 heterogeneous small; not canonical SP4",
        "backend": "fdm",
        "device": "cpu",
        "precision": "double",
        "variant": "l3_heterogeneous_push_pull_small",
        "native_cells": {
            "layer_bottom": (8, 4, 1),
            "layer_middle": (8, 4, 2),
            "layer_top": (8, 4, 1),
...
study.stages.add_run(until=1e-14, stage_id="l3_heterogeneous_small")
