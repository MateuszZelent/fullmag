...
- On Windows, store every agent-created worktree and all large build outputs, dependency caches, generated evidence, downloads, and temporary task artifacts under `D:\git\fullmag\...`; treat `C:` as nearly full and never create new task storage there. The canonical tracked checkout may remain on `C:`, but active implementation work must use a worktree on `D:`.
