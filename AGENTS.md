...
- Windowsowy FEM/GPU musi przechodzić przez `scripts/windows/run_fullmag_wsl.ps1` i `compose.windows.yaml` z bind mountami na D:; nie uruchamiaj linuksowych recept `just` bezpośrednio w Git Bash z dyskiem `D:\...`, nie używaj named volumes dla buildów/cache i nie dopuszczaj do cichego CPU fallbacku.
