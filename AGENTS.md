...
- Windowsowy FEM CPU/GPU musi przechodzić przez `scripts/windows/run_fullmag_wsl.ps1` i `compose.windows.yaml` z bind mountami poza repo; launcher nie może wywoływać `wsl.exe`, używać named volumes dla buildów/cache ani dopuszczać do cichego CPU fallbacku. Nazwa skryptu jest historyczna; wykonanie należy do Docker Desktop Linux engine.
