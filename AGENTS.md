...
- Na Windowsie checkout repozytorium (`D:\git\fullmag`) nie może być magazynem buildów, cache ani przeglądarek: `CARGO_TARGET_DIR`, pnpm store/home i `PLAYWRIGHT_BROWSERS_PATH` muszą wskazywać bezwzględne ścieżki poza repo (`D:\fullmag-fullmag-target` lub `D:\fullmag-cache\...`). Nie używaj `target`, `cargo-targets`, `pnpm-store`, `pnpm-home`, `playwright-browsers`, `build-contract-wave2` ani `antenna-audit-target` jako lokalnych ścieżek roboczych; jeśli taki katalog powstanie, przenieś go poza repo przed kolejnym `git status` i utrwal dokładną regułę w `.gitignore`.
- Natywny tryb Windows `just fullmag windows=True ...` musi używać `scripts/windows/run_fullmag.ps1`; launcher wymusza `D:\fullmag-cache`/`D:\fullmag-build` dla Cargo, Rustup, pnpm, npm, uv, pip, Python, TEMP/TMP, CUDA i Playwright oraz nie może dziedziczyć cache/buildu z `C:` ani uruchamiać MSI/Dockera.
...
- Windows GPU builds use the WSL2/Linux-container route through `compose.windows.yaml` and `scripts/windows/run_fullmag_wsl.ps1`; keep its source/state/build/cache/temp bind mounts on D:, keep `native=True` as the explicit MSVC `.exe` lane, and require Docker Desktop's internal disk image location to be configured on D: before a real CUDA build.
