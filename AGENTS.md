...
- Windowsowy FEM CPU/GPU musi przechodzić przez `scripts/windows/run_fullmag_docker.ps1` i `compose.windows.yaml` z bind mountami poza repo; launcher używa bezpośrednio Docker Desktop i nie może wywoływać `wsl.exe`, używać ścieżek WSL/Linuxowego checkoutu, używać named volumes dla buildów/cache ani dopuszczać do cichego CPU fallbacku.
