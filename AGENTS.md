...
- Merge na remote nie kończy lokalnego cyklu: po potwierdzeniu wynikowego commita przełącz lokalny główny checkout na `master`; jeżeli główny checkout jest niedostępny, pracę kończ w pierwotnym worktree ustawionym na wynikowy ref. Następnie usuń lokalny zmergowany branch zadania oraz dokładnie ten zweryfikowany worktree. Jeżeli aktywny proces, kontener, mount, dirty/untracked pliki albo obcy właściciel blokują cleanup, zapisz `blocked` z pełną przyczyną i nie przedstawiaj zadania jako zakończonego.
