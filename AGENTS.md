...
- Windowsowy FEM CPU/GPU musi przechodzić przez kanoniczny `scripts/windows/run_fullmag_fem.ps1` i `compose.windows.yaml` z bind mountami poza repo; launcher nie może wywoływać `wsl.exe`, używać named volumes dla buildów/cache ani dopuszczać do cichego CPU fallbacku. `run_fullmag_wsl.ps1` jest wyłącznie historycznym aliasem zgodności; wykonanie należy do Docker Desktop Linux engine.
