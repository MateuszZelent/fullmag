...
void gpu_rk_transaction_telemetry_is_profiled_without_new_sync() {
    ...
    check(
        publication_source.find("fill_step_profiler_timing_stats(ctx, stats)") !=
                std::string::npos &&
            step_metrics_source.find("device_transaction.capture_device_elapsed_ns") !=
                std::string::npos &&
            step_metrics_source.find("device_transaction.capture_bytes") !=
                std::string::npos &&
            step_metrics_source.find("device_transaction.restore_device_elapsed_ns") !=
                std::string::npos &&
            step_metrics_source.find("device_transaction.restore_bytes") !=
                std::string::npos,
        "GPU stats publication must expose device transaction elapsed time and bytes through the shared step-metrics owner");
}
...
void gpu_demag_poisson_is_owned_by_cuda_demag_poisson_module() {
    ...
    const std::string hypre_policy =
        read_text_file(root / "gpu" / "cuda" / "runtime" / "hypre_device_policy.cpp");
    ...
    check(
        hypre_solver.find("#include \"gpu/cuda/runtime/hypre_device_policy.hpp\"") !=
                std::string::npos &&
            hypre_solver.find("configure_hypre_cuda_device_policy()") !=
                std::string::npos &&
            hypre_solver.find("HYPRE_SetMemoryLocation(") == std::string::npos &&
            hypre_solver.find("HYPRE_SetExecutionPolicy(") == std::string::npos &&
            hypre_policy.find("HYPRE_SetMemoryLocation(HYPRE_MEMORY_DEVICE)") !=
                std::string::npos &&
            hypre_policy.find("HYPRE_SetExecutionPolicy(HYPRE_EXEC_DEVICE)") !=
                std::string::npos &&
            hypre_policy.find("HYPRE_SetSpTransUseVendor(1)") !=
                std::string::npos &&
            hypre_policy.find("HYPRE_SetSpMVUseVendor(1)") !=
                std::string::npos &&
            hypre_policy.find("HYPRE_SetSpGemmUseVendor(1)") !=
                std::string::npos &&
            hypre_policy.find("HYPRE_ClearAllErrors()") != std::string::npos,
        "GPU CUDA Poisson demag must consume the shared fail-closed HYPRE CUDA device policy owner");
    check(
