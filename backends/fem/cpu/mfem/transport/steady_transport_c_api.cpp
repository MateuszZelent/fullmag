    ...
    const fullmag_fem_steady_transport_result_v1 &result)
{
    ...
    if (request.mesh.n_nodes == 0 || request.mesh.n_elements == 0 ||
        request.mesh.nodes_xyz == nullptr || request.mesh.elements == nullptr) {
        throw std::invalid_argument("steady transport requires a non-empty tetrahedral mesh");
    }
    if (request.mesh.n_boundary_faces == 0 || request.mesh.boundary_faces == nullptr ||
        request.mesh.boundary_markers == nullptr) {
        throw std::invalid_argument("steady transport requires explicit boundary triangles and attributes");
    }
    if (request.mesh.periodic_node_pairs != nullptr ||
        request.mesh.n_periodic_node_pairs != 0 ||
        request.mesh.periodic_boundary_pair_markers != nullptr ||
        request.mesh.periodic_boundary_pair_count != 0) {
    ...
    if (!pointer_matches_count(
        ...
        request.charge_conductivity_spm_per_element_len != request.mesh.n_elements) {
    ...
    if (request.magnetization_xyz == nullptr ||
        request.magnetization_xyz_len != static_cast<uint64_t>(request.mesh.n_nodes) * 3u) {
    ...
    const uint64_t nodes = request.mesh.n_nodes;
    ...
    for (uint64_t i = 0; i < request.mesh.n_elements; ++i) {
...
std::unique_ptr<mfem::Mesh> import_mesh(const fullmag_fem_mesh_desc &descriptor)
{
    auto mesh = std::make_unique<mfem::Mesh>(
        3,
        static_cast<int>(descriptor.n_nodes),
        static_cast<int>(descriptor.n_elements),
        static_cast<int>(descriptor.n_boundary_faces),
        3);
    for (uint32_t node = 0; node < descriptor.n_nodes; ++node) {
        mesh->AddVertex(descriptor.nodes_xyz + static_cast<size_t>(node) * 3u);
    }
    for (uint32_t element = 0; element < descriptor.n_elements; ++element) {
        const uint32_t *indices = descriptor.elements + static_cast<size_t>(element) * 4u;
        int tetrahedron[4];
        for (int local = 0; local < 4; ++local) {
            if (indices[local] >= descriptor.n_nodes) {
    ...
    for (uint32_t boundary = 0; boundary < descriptor.n_boundary_faces; ++boundary) {
        const uint32_t *indices = descriptor.boundary_faces + static_cast<size_t>(boundary) * 3u;
        int triangle[3];
        ...
        const uint32_t attribute = descriptor.boundary_markers[boundary];
        if (attribute == 0) {
    ...
    fullmag_fem_steady_transport_result_v1 &result)
{
    ...
    auto mesh = import_mesh(request.mesh);

    mfem::Vector conductivity_values(static_cast<int>(request.mesh.n_elements));
    for (uint32_t i = 0; i < request.mesh.n_elements; ++i) {
    ...
    if (magnetization_space.GetNDofs() != static_cast<int>(request.mesh.n_nodes)) {
    ...
    for (uint64_t node = 0; node < request.mesh.n_nodes; ++node) {
        for (int component = 0; component < 3; ++component) {
            magnetization_grid[component * static_cast<int>(request.mesh.n_nodes) +
