...
public:
    explicit ReceiptAttemptGuard(FemGpuExecutionReceiptRuntimeState &receipt)
        : receipt_(receipt)
    {
        gpu_execution_receipt_begin_attempt(receipt_);
    }
    ...
    std::string &reason)
{
    ...
    for (;;) {
        ReceiptAttemptGuard receipt_attempt(ctx.gpu_state.execution_receipt);
