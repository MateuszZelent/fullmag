    ...
    const AdaptiveDeviceControl *adaptive_control)
{
    ...
    // DP45 error weights: E1=71/57600, E3=-71/16695, E4=71/1920, E5=-17253/339200, E6=22/525, E7=-1/40
        ...
        const double ex = dt * (E1*k1x[idx] + E3*k3x[idx] + E4*k4x[idx] + E5*k5x[idx] + E6*k6x[idx] + E7*k7x[idx]);
        const double ey = dt * (E1*k1y[idx] + E3*k3y[idx] + E4*k4y[idx] + E5*k5y[idx] + E6*k6y[idx] + E7*k7y[idx]);
        const double ez = dt * (E1*k1z[idx] + E3*k3z[idx] + E4*k4z[idx] + E5*k5z[idx] + E6*k6z[idx] + E7*k7z[idx]);
...
{
    launch_project_frozen_fp64(ctx, stream);
    ...
    if (fuse_local_pipeline) {
        if (!launch_effective_field_and_base_llg_rhs_fp64(
                ctx, evaluation_time, rhs_out, stream)) return false;
    } else {
...
void launch_dp45_step_fp64(Context &ctx, double dt, fullmag_fdm_step_stats *stats) {
    ...
    for (;;) {
        ...
        dp45_error_kernel<<<grid, 256>>>(
            ...
            ctx.reduction_scratch,
            static_cast<uint64_t>(grid), n, dt,
            ctx.adaptive_atol, ctx.adaptive_rtol, nullptr);
